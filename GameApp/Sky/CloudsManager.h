...
namespace VCloud
{
	...
	public:
		...
		struct Options 
		{
			 ...
			 */
			Options(const float& _Height, 
				    const float& _Scale, 
					const noVec2& _WindDirection, 
					const float& _TimeMultiplier)
				: Height(_Height)
				, Scale(_Scale)
			...
			Options(const float& _Height, 
					...
					const noVec2& _WindDirection, 
					const float& _TimeMultiplier,
					const float& _DistanceAttenuation,
					const float& _DetailAttenuation,
					const float& _HeightVolume,
					const float& _VolumetricDisplacement)
				: Height(_Height)
				, Scale(_Scale)
				, WindDirection(_WindDirection)
				, TimeMultiplier(_TimeMultiplier)
		 ...
		 */
		CloudLayer(VCloudSkySystem *s);

		/** Constructor
		    @param s SkyX parent pointer
		    @param o Cloud layer options
		 */
		CloudLayer(VCloudSkySystem *s, const Options& o);
		 ...
		 */
		//void _registerCloudLayer(Pass* CloudLayerPass);
	...
	public:
		/** Constructor
		    @param s SkyX parent pointer
		 */
		CloudsManager(VCloudSkySystem *h);
