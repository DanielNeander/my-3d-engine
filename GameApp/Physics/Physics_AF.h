...
public:
	...
	virtual void			Translate( const noVec3 &translation );
	virtual void			Rotate( const noRotation &rotation );
	virtual void			GetCenter( noVec3 &center );
...
public:
	...
	void					SetRelativeOrigin( const noVec3 &origin ) { this->offset = origin; }
	void					SetRelativeAxis( const noMat3 &axis ) { this->relAxis = axis; }
	virtual void			SetBody1( idAFBody *body );
	virtual void			SetBody2( idAFBody *body );
	virtual void			DebugDraw( void );
	virtual void			Translate( const noVec3 &translation );
...
protected:
	noVec3					offset;						// offset of body1 relative to body2 in body2 space
	noMat3					relAxis;					// rotation of body1 relative to body2
...
public:
	...
	void					SetAnchor( const noVec3 &worldPosition );
	noVec3					GetAnchor( void ) const;
	void					SetNoLimit( void );
	void					SetConeLimit( const noVec3 &coneAxis, const float coneAngle, const noVec3 &body1Axis );
	void					SetPyramidLimit( const noVec3 &pyramidAxis, const noVec3 &baseAxis,
...
protected:
	noVec3					anchor1;					// anchor in body1 space
	noVec3					anchor2;					// anchor in body2 space
...
public:
	...
	void					SetAnchor( const noVec3 &worldPosition );
	noVec3					GetAnchor( void ) const;
	void					SetShafts( const noVec3 &cardanShaft1, const noVec3 &cardanShaft2 );
	void					GetShafts( noVec3 &cardanShaft1, noVec3 &cardanShaft2 ) { cardanShaft1 = shaft1; cardanShaft2 = shaft2; }
	void					SetNoLimit( void );
	void					SetConeLimit( const noVec3 &coneAxis, const float coneAngle );
	void					SetPyramidLimit( const noVec3 &pyramidAxis, const noVec3 &baseAxis,
...
protected:
	noVec3					anchor1;					// anchor in body1 space
	noVec3					anchor2;					// anchor in body2 space
	noVec3					shaft1;						// body1 cardan shaft in body1 space
	noVec3					shaft2;						// body2 cardan shaft in body2 space
	noVec3					axis1;						// cardan axis in body1 space
	noVec3					axis2;						// cardan axis in body2 space
	float					friction;					// joint friction
	idAFConstraint_ConeLimit *coneLimit;				// cone shaped limit
	idAFConstraint_PyramidLimit *pyramidLimit;			// pyramid shaped limit
...
public:
	...
	void					SetAnchor( const noVec3 &worldPosition );
	noVec3					GetAnchor( void ) const;
	void					SetAxis( const noVec3 &axis );
	void					GetAxis( noVec3 &a1, noVec3 &a2 ) const { a1 = axis1; a2 = axis2; }
	noVec3					GetAxis( void ) const;
	void					SetNoLimit( void );
	void					SetLimit( const noVec3 &axis, const float angle, const noVec3 &body1Axis );
	void					SetLimitEpsilon( const float e );
...
protected:
	noVec3					anchor1;					// anchor in body1 space
	noVec3					anchor2;					// anchor in body2 space
	noVec3					axis1;						// axis in body1 space
	noVec3					axis2;						// axis in body2 space
	noMat3					initialAxis;				// initial axis of body1 relative to body2
...
protected:
	noVec3					axis;						// axis along which body1 slides in body2 space
	noVec3					offset;						// offset of body1 relative to body2
	noMat3					relAxis;					// rotation of body1 relative to body2
...
public:
	...
	void					SetPlane( const noVec3 &normal, const noVec3 &anchor );
	virtual void			DebugDraw( void );
	virtual void			Translate( const noVec3 &translation );
	virtual void			Rotate( const noRotation &rotation );
	//virtual void			Save( idSaveGame *saveFile ) const;
...
protected:
	noVec3					anchor1;					// anchor in body1 space
	noVec3					anchor2;					// anchor in body2 space
	noVec3					planeNormal;				// plane normal in body2 space
...
public:
	...
	void					SetAnchor( const noVec3 &worldAnchor1, const noVec3 &worldAnchor2 );
...
public:
	...
	void					Setup( idAFBody *b1, idAFBody *b2, const noVec3 &coneAnchor, const noVec3 &coneAxis,
									const float coneAngle, const noVec3 &body1Axis );
	void					SetAnchor( const noVec3 &coneAnchor );
	void					SetBody1Axis( const noVec3 &body1Axis );
	void					SetEpsilon( const float e ) { epsilon = e; }
	bool					Add( idPhysics_AF *phys, float invTimeStep );
	virtual void			DebugDraw( void );
	virtual void			Translate( const noVec3 &translation );
...
protected:
	noVec3					coneAnchor;					// top of the cone in body2 space
	noVec3					coneAxis;					// cone axis in body2 space
	noVec3					body1Axis;					// axis in body1 space that should stay within the cone
...
protected:
	noVec3					pyramidAnchor;				// top of the pyramid in body2 space
	noMat3					pyramidBasis;				// pyramid basis in body2 space with base[2] being the pyramid axis
	noVec3					body1Axis;					// axis in body1 space that should stay within the cone
...
public:
	...
	void					Setup( const char *name, idAFBody *body, const noVec3 &origin, const noMat3 &axis, idClipModel *clipModel );
	...
	void					SetEpsilon( const float e ) { epsilon = e; }
	const noVec3			GetWheelOrigin( void ) const;
...
protected:
	noVec3					localOrigin;				// position of suspension relative to body1
	noMat3					localAxis;					// orientation of suspension relative to body1
	...
	noVec3					wheelOffset;				// wheel position relative to body1
...
typedef struct AFBodyPState_s {
	noVec3					worldOrigin;				// position in world space
	noMat3					worldAxis;					// axis at worldOrigin
...
public:
	...
	void					Init( void );
	const idStr &			GetName( void ) const { return name; }
	const noVec3 &			GetWorldOrigin( void ) const { return current->worldOrigin; }
	const noMat3 &			GetWorldAxis( void ) const { return current->worldAxis; }
	const noVec3 &			GetLinearVelocity( void ) const { return current->spatialVelocity.SubVec3(0); }
	const noVec3 &			GetAngularVelocity( void ) const { return current->spatialVelocity.SubVec3(1); }
	noVec3					GetPointVelocity( const noVec3 &point ) const;
	const noVec3 &			GetCenterOfMass( void ) const { return centerOfMass; }
	...
	void					SetWorldOrigin( const noVec3 &origin ) { current->worldOrigin = origin; }
	void					SetWorldAxis( const noMat3 &axis ) { current->worldAxis = axis; }
	void					SetLinearVelocity( const noVec3 &linear ) const { current->spatialVelocity.SubVec3(0) = linear; }
	void					SetAngularVelocity( const noVec3 &angular ) const { current->spatialVelocity.SubVec3(1) = angular; }
	...
	void					SetDensity( float density, const noMat3 &inertiaScale = mat3_identity );
	float					GetInverseMass( void ) const { return invMass; }
	noMat3					GetInverseWorldInertia( void ) const { return current->worldAxis.Transpose() * inverseInertiaTensor * current->worldAxis; }

	void					SetFrictionDirection( const noVec3 &dir );
	bool					GetFrictionDirection( noVec3 &dir ) const;

	void					SetContactMotorDirection( const noVec3 &dir );
	bool					GetContactMotorDirection( noVec3 &dir ) const;
	...
	void					AddForce( const noVec3 &point, const noVec3 &force );
...
private:
	...
	noVec3					frictionDir;				// specifies a single direction of friction in body space
	noVec3					contactMotorDir;			// contact motor direction
	...
	noVec3					centerOfMass;				// center of mass of body
	noMat3					inertiaTensor;				// inertia tensor
	noMat3					inverseInertiaTensor;		// inverse inertia tensor
	...
	noVec3					atRestOrigin;				// origin at rest
	noMat3					atRestAxis;					// axis at rest
...
public:
	...
	void					DebugDraw( const noVec4 &color ) const;
...
public:
	...
	void					SetDefaultFriction( float linear, float angular, float contact );
							// suspend settings
	void					SetSuspendSpeed( const noVec2 &velocity, const noVec2 &acceleration );
...
public:	// common physics interface
	...
	void					GetImpactInfo( const int id, const noVec3 &point, impactInfo_t *info ) const;
	void					ApplyImpulse( const int id, const noVec3 &point, const noVec3 &impulse );
	void					AddForce( const int id, const noVec3 &point, const noVec3 &force );
	...
	void					ClipTranslation( trace_t &results, const noVec3 &translation, const idClipModel *model ) const;
	void					ClipRotation( trace_t &results, const noRotation &rotation, const idClipModel *model ) const;
	...
	const noVec3 &			GetPushedLinearVelocity( const int id = 0 ) const;
	const noVec3 &			GetPushedAngularVelocity( const int id = 0 ) const;
...
private:
	...
	noVec2					suspendVelocity;				// simulation may not be suspended if a body has more velocity
	noVec2					suspendAcceleration;			// simulation may not be suspended if a body has more acceleration
