...
typedef struct impactInfo_s {
	...
	noMat3						invInertiaTensor;	// inverse inertia tensor
	noVec3						position;			// impact position relative to center of mass
	noVec3						velocity;			// velocity at the impact position
} impactInfo_t;
...
public:	// common physics interface
	...
	virtual void				GetImpactInfo( const int id, const noVec3 &point, impactInfo_t *info ) const = 0;
	virtual void				ApplyImpulse( const int id, const noVec3 &point, const noVec3 &impulse ) = 0;
	virtual void				AddForce( const int id, const noVec3 &point, const noVec3 &force ) = 0;
	...
	virtual void				SetOrigin( const noVec3 &newOrigin, int id = -1 ) = 0;
	virtual void				SetAxis( const noMat3 &newAxis, int id = -1 ) = 0;
								// translate or rotate the physics object in world space
	virtual void				Translate( const noVec3 &translation, int id = -1 ) = 0;
	virtual void				Rotate( const noRotation &rotation, int id = -1 ) = 0;
								// get the position and orientation in world space
	virtual const noVec3 &		GetOrigin( int id = 0 ) const = 0;
	virtual const noMat3 &		GetAxis( int id = 0 ) const = 0;
								// set linear and angular velocity
	virtual void				SetLinearVelocity( const noVec3 &newLinearVelocity, int id = 0 ) = 0;
	virtual void				SetAngularVelocity( const noVec3 &newAngularVelocity, int id = 0 ) = 0;
								// get linear and angular velocity
	virtual const noVec3 &		GetLinearVelocity( int id = 0 ) const = 0;
	virtual const noVec3 &		GetAngularVelocity( int id = 0 ) const = 0;
								// gravity
	virtual void				SetGravity( const noVec3 &newGravity ) = 0;
	virtual const noVec3 &		GetGravity( void ) const = 0;
	virtual const noVec3 &		GetGravityNormal( void ) const = 0;
								// get first collision when translating or rotating this physics object
	virtual void				ClipTranslation( trace_t &results, const noVec3 &translation, const idClipModel *model ) const = 0;
	virtual void				ClipRotation( trace_t &results, const noRotation &rotation, const idClipModel *model ) const = 0;
	...
	virtual const noVec3 &		GetPushedLinearVelocity( const int id = 0 ) const = 0;
	virtual const noVec3 &		GetPushedAngularVelocity( const int id = 0 ) const = 0;
