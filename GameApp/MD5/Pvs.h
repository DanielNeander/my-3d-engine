...
public:
	...
	int					GetPVSArea( const noVec3 &point ) const;		// returns the area number
	...
	pvsHandle_t			SetupCurrentPVS( const noVec3 &source, const pvsType_t type = PVS_NORMAL ) const;
	...
	bool				InCurrentPVS( const pvsHandle_t handle, const noVec3 &target ) const;
	...
	void				DrawPVS( const noVec3 &source, const pvsType_t type = PVS_NORMAL ) const;
	void				DrawPVS( const idBounds &source, const pvsType_t type = PVS_NORMAL ) const;
						// visualize the PVS the handle points to
	void				DrawCurrentPVS( const pvsHandle_t handle, const noVec3 &source ) const;
...
#ifdef _D3XP
	bool				CheckAreasForPortalSky( const pvsHandle_t handle, const noVec3 &origin );
#endif
...
private:
	...
	void				AddPassageBoundaries( const idWinding &source, const idWinding &pass, bool flipClip, noPlane *bounds, int &numBounds, int maxBounds ) const;
