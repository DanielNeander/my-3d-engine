...
bool	LUASCRIPT::Run(const char* cmdStr)
{
	 lua_gettable(threadState, LUA_GLOBALSINDEX);
	 //U2ASSERT2(lua_isfunction(threadState, -1), "Error handler not registered!");

	 int errfunc = lua_gettop(threadState);
	 // load chunk
	 int status = luaL_loadbuffer(threadState, cmdStr, strlen(cmdStr), cmdStr);
	 if (0 == status) // parse OK?
	 {
		status = lua_pcall(threadState,
			 0 /* no args */,
			 LUA_MULTRET,
