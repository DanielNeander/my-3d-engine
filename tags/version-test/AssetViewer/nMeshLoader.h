...
public:
	...
	virtual ~nMeshLoader();
	/// set filename
	void SetFilename(const U2String& name);
	/// get filename
	const U2String& GetFilename() const;
...
protected:
	...
	U2ObjVec<nMeshGroupPtr> groupArray;
};
...
inline
void
nMeshLoader::SetFilename(const U2String& name)
{	
	this->filename = name;
}
...
nMeshLoader::Open()
{
	U2ASSERT(0 != this->fileVertexWidth);
	U2ASSERT(0 != this->fileVertexComponents);
	U2ASSERT(0 != this->validVertexComponents);
...
nMeshLoader::Close()
{
	FDebug("nMeshLoader::Close() called!");
}
...
{
	FDebug("nMeshLoader::ReadVertices() called!");
	return false;
}
...
{
	FDebug("nMeshLoader::ReadIndices() called!");
	return false;
}
...
{
	FDebug("nMeshLoader::ReadEdges() called!");
	return false;
}
//------------------------------------------------------------------------------
#endif