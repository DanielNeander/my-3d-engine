#ifndef MODEL_M2_FORMAT_H
#define MODEL_M2_FORMAT_H
...
struct ModelAnimation {
	...
	noVec3 boxA, boxB; // Minimum Extent, Maximum Extend
...
struct ModelLightDef {
	...
	noVec3 pos; // Position, Where is this light?
...
struct ModelCameraDef {
	...
	noVec3 pos; // float, Where the camera is located.
	AnimationBlock transTarget; // How the target moves. Should be 3*3 floats. (?)
	noVec3 target; // float, Where the camera points to.
...
struct ModelParticleEmitterDef {
    int32 id;
	int32 flags;
	noVec3 pos; // The position. Relative to the following bone.
...
struct ModelEventDef {
	...
	noVec3 pos; // Relative to that bone of course.
