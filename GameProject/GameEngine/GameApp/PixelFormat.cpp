...
//-----------------------------------------------------------------------
   ...
   size_t PixelUtil::getNumElemBytes( PixelFormat format )
    ...
    //-----------------------------------------------------------------------
    PixelFormat PixelUtil::getFormatFromName(const std::string& name, bool accessibleOnly, bool caseSensitive)
    {
        std::string tmp = name;
        if (!caseSensitive)
        {
            // We are stored upper-case format names.
            //std::stringUtil::toUpperCase(tmp);
        }
    ...
    //-----------------------------------------------------------------------
    std::string PixelUtil::getBNFExpressionOfPixelFormats(bool accessibleOnly)
    {
        // Collect format names sorted by length, it's required by BNF compiler
        // that similar tokens need longer ones comes first.
        typedef std::multimap<std::string::size_type, std::string> FormatNameMap;
        ...
        for (size_t i = 0; i < PF_COUNT; ++i)
        {
            ...
            if (!accessibleOnly || isAccessible(pf))
            {
                std::string formatName = getFormatName(pf);
