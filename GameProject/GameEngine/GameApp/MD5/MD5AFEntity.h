...
public:
	...
	class idPhysics_AF *	GetAFPhysics( void ) { return af.GetPhysics(); }
...
protected:
	...
	class idClipModel *		combatModel;	// render model for hit detection
	int						combatModelContents;
	noVec3					spawnOrigin;	// spawn origin
	noMat3					spawnAxis;		// rotation axis used when spawned
	int						nextSoundTime;	// next time this can make a sound

	void					Event_SetConstraintPosition( const char *name, const noVec3 &pos );
};
...
public:
	...
	void					Spawn( void );
	void					Damage( idEntity *inflictor, idEntity *attacker, const noVec3 &dir, const char *damageDefName, const float damageScale, const int location );	
