...
Light::Light()
{
  ...
  m_vUpVector = noVec3(0, 1, 0);
  m_fAspectRatio = 1.0f;
  m_vSource = noVec3(0, 200, 0);
  m_vTarget = noVec3(0, 0, 0);
  m_fNear = 50.0f;
  m_fFar = 400.0f;
  m_fFOV = DegreeToRadian(90.0f);
  m_vLightDiffuse = noVec3(0.7f,0.7f,0.7f);
  m_vLightAmbient = noVec3(0.25f,0.25f,0.25f);

  ZeroMemory(&m_ControlState, sizeof(ControlState));
  m_ControlState.m_vRotation = noVec3(-DegreeToRadian(130.0f), -DegreeToRadian(35.0f),0);
}
...
void Light::DoControls(void)
{
  ...
  noVec3 vDist = m_vTarget - m_vSource;
  m_vSource = m_vTarget + noVec3(sh*cp, -sp, cp*ch) * vDist.Length();
...
std::vector<SceneObject *> Light::FindCasters(const Frustum &frustum)
{
  ...
  for(unsigned int i=0; i<g_SceneObjects.size(); i++)
  {
    ...
    else if(m_Type == TYPE_PERSPECTIVE)
    {
      ...
      noVec3 vDirToCenter = Normalize(((pObject->m_AABB.m_vMax + pObject->m_AABB.m_vMin) * 0.5f) - m_vSource);
      if(g_iVisibilityTest == VISTEST_ACCURATE)
      {
        // test accurately
...
inline Matrix Light::BuildCropMatrix(const noVec3 &vMin, const noVec3 &vMax)
{
