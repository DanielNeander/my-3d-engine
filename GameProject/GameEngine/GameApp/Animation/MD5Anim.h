...
===========================================================================
*/
#ifndef __MD5_ANIM_H__
#define __MD5_ANIM_H__
...
typedef struct {
	...
	union {
		...
		const class function_t	*function;
...
public:
	...
	const noVec3			&TotalMovementDelta( void ) const;
	...
	void					GetOrigin( noVec3 &offset, int currentTime, int cyclecount ) const;
	void					GetOriginRotation( noQuat &rotation, int time, int cyclecount ) const;
...
public:
	...
	const noVec3				&TotalMovementDelta( void ) const;
	bool						GetOrigin( noVec3 &offset, int animNum, int time, int cyclecount ) const;
	bool						GetOriginRotation( noQuat &rotation, int animNum, int currentTime, int cyclecount ) const;
	...
	//void						CallFrameCommands( GameObject *ent, int from, int to ) const;
...
public:
	...
	class idRenderModel *		ModelHandle( void ) const;
	...
	const noVec3 &				GetVisualOffset( void ) const;
...
private:
	...
	void						BlendOrigin( int currentTime, noVec3 &blendPos, float &blendWeight, bool removeOriginOffset ) const;
	void						BlendDelta( int fromtime, int totime, noVec3 &blendDelta, float &blendWeight ) const;
	void						BlendDeltaRotation( int fromtime, int totime, noQuat &blendDelta, float &blendWeight ) const;
...
public:
	...
	void						SetEntity( class idEntity *ent );
	...
	void						GetDelta( int fromtime, int totime, noVec3 &delta ) const;
	bool						GetDeltaRotation( int fromtime, int totime, noMat3 &delta ) const;
	void						GetOrigin( int currentTime, noVec3 &pos ) const;
	...
	void						SetJointPos( jointHandle_t jointnum, jointModTransform_t transform_type, const noVec3 &pos );
	void						SetJointAxis( jointHandle_t jointnum, jointModTransform_t transform_type, const noMat3 &mat );
	void						ClearJoint( jointHandle_t jointnum );
	void						ClearAllJoints( void );

	void						InitAFPose( void );
	void						SetAFPoseJointMod( const jointHandle_t jointNum, const AFJointModType_t mod, const noMat3 &axis, const noVec3 &origin );
	...
	bool						GetJointTransform( jointHandle_t jointHandle, int currenttime, noVec3 &offset, noMat3 &axis );
	bool						GetJointLocalTransform( jointHandle_t jointHandle, int currentTime, noVec3 &offset, noMat3 &axis );
	...
	const noVec3				&TotalMovementDelta( int animnum ) const;
...
extern void ANIM_CreateAnimFrame( const idRenderModel *model, const idMD5Anim *anim, int numJoints, idJointMat *joints, int time, const noVec3 &offset, bool remove_origin_offset );
