...
bool idIK::Init( idEntity *self, const char *anim, const noVec3 &modelOffset ) {
	idRenderModel *model;
	if ( self == NULL ) {
		return false;
	}
...
bool idIK::SolveTwoBones( const noVec3 &startPos, const noVec3 &endPos, const noVec3 &dir, float len0, float len1, noVec3 &jointPos )
{
...
float idIK::GetBoneAxis( const noVec3 &startPos, const noVec3 &endPos, const noVec3 &dir, noMat3 &axis )
{
	float length;
...
bool idIK_Walk::Init( idEntity *self, const char *anim, const noVec3 &modelOffset )
{
	int i;
	float footSize;
	noVec3 verts[4];
	...
	noVec3 dir, ankleOrigin, kneeOrigin, hipOrigin, dirOrigin;
	noMat3 axis, ankleAxis, kneeAxis, hipAxis;

	static noVec3 footWinding[4] = {
		noVec3(  1.0f,  1.0f, 0.0f ),
		noVec3( -1.0f,  1.0f, 0.0f ),
		noVec3( -1.0f, -1.0f, 0.0f ),
		noVec3(  1.0f, -1.0f, 0.0f )
	};
	...
	::ANIM_CreateAnimFrame( animator->ModelHandle(), animator->GetAnim( modifiedAnim )->MD5Anim( 0 ), numJoints, joints, 1, animator->ModelDef()->GetVisualOffset() + modelOffset, animator->RemoveOrigin() );
...
void idIK_Walk::Evaluate( void )
{
	...
	noVec3 modelOrigin, normal, hipDir, kneeDir, start, end, jointOrigins[MAX_LEGS];
	noVec3 footOrigin, ankleOrigin, kneeOrigin, hipOrigin, waistOrigin;
	noMat3 modelAxis, waistAxis, axis;
	noMat3 hipAxis[MAX_LEGS], kneeAxis[MAX_LEGS], ankleAxis[MAX_LEGS];
	trace_t results;
	...
	if ( usePivot ) {
		...
		// change pivot foot
		if ( newPivotFoot != pivotFoot || noMath::Fabs( noMath::AngleNormalize180( newPivotYaw - pivotYaw ) ) > 30.0f ) {
	...
	smallestShift = noMath::INFINITY;
	largestAnkleHeight = -noMath::INFINITY;
	for ( i = 0; i < numLegs; i++ ) {
...
bool idIK_Reach::Init( idEntity *self, const char *anim, const noVec3 &modelOffset ) {
	int i;
	const char *jointName;
	idTraceModel trm;
	noVec3 dir, handOrigin, elbowOrigin, shoulderOrigin, dirOrigin;
	noMat3 axis, handAxis, elbowAxis, shoulderAxis;
...
void idIK_Reach::Evaluate( void ) {
	int i;
	noVec3 modelOrigin, shoulderOrigin, elbowOrigin, handOrigin, shoulderDir, elbowDir;
	noMat3 modelAxis, axis;
	noMat3 shoulderAxis[MAX_ARMS], elbowAxis[MAX_ARMS];
	trace_t trace;
