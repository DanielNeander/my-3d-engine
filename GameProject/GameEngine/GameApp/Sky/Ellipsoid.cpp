...
namespace VCloud { 
     ...
     Ellipsoid::Ellipsoid(
		...
		const float& Density)
		: mA(a), mB(b), mC(c)
		, mA2(noMath::Pow(a, 2)), mB2(noMath::Pow(b, 2)), mC2(noMath::Pow(c, 2))
	...
	const float Ellipsoid::_getLength(const int &x, const int &y, const int &z) const
	{
		...
		noVec3 Direction = noVec3(x-mX, y-mY, z-mZ),
					  DirectionNormalized = Direction.NormalizeCopy();

		float a = noMath::Pow(DirectionNormalized.x, 2) / mA2 + 
				       noMath::Pow(DirectionNormalized.y, 2) / mB2 +
				       noMath::Pow(DirectionNormalized.z, 2) / mC2,

		           lambda = 1.0f / noMath::Sqrt(a);

		return noMath::ClampFloat(0, 1, Direction.Length() / lambda);
	}

	const noVec3 Ellipsoid::getProbabilities(const int& x, const int& y, const int& z) const
	{
		float density = noMath::Pow(1-_getLength(x, y, z), 1.0f/mDensity);

		return noVec3(density, 1-density, density);
	}
			...
			for (v = mY-mB; v < mY+mB; v++)
			{
				...
				for (w = mZ-mC; w < mZ+mC; w++)
				{
					...
					if (length < 1)
					{
						...
						if (!delayedResponse)
						{
							c[uu][vv][w].cld = RandInt(0,1) > length ? true : false;
						}
					}
				}
			}
	...
	void Ellipsoid::setDimensions(const noVec3& Dimensions)
	{
		mA = Dimensions.x; 
		mB = Dimensions.y; 
		mC = Dimensions.z;
