...
FormatDesc vbFmt[] = {
	...
	0, TYPE_COLOR, FORMAT_FLOAT, 4,	// color
	0, TYPE_TEXCOORD, FORMAT_FLOAT, 2 // uv
};
...
RadixSort<BillboardSet::ActiveBillboardList, Billboard*, float> BillboardSet::mRadixSorter;
    ...
    BillboardSet::BillboardSet() :
        ...
        mCommonDirection(noVec3(0, 0, 1)),
        mCommonUpVector(noVec3(0, 1, 0)),
        ...
        const ColourValue& colour )
    {
        ...
        noVec3 vecAdjust(adjust, adjust, adjust);
		noVec3 newMin = position - vecAdjust;
		noVec3 newMax = position + vecAdjust;

        mAABB.Set(newMin, newMax);
        
		mBoundingRadius = mAABB.BoundingRadiusFromAABB();
    ...
    Billboard* BillboardSet::createBillboard(
        ...
        const ColourValue& colour )
    {
        return createBillboard( noVec3( x, y, z ), colour );
    }
    ...
    //-----------------------------------------------------------------------
    void BillboardSet::_sortBillboards( BaseCamera* cam)
    {
        switch (_getSortMode())
        {
        case SM_DIRECTION:
    ...
    BillboardSet::SortByDirectionFunctor::SortByDirectionFunctor(const noVec3& dir)
        : sortDir(dir)
    {
    }
    float BillboardSet::SortByDirectionFunctor::operator()(Billboard* bill) const
    {
        return sortDir * bill->getPosition();
    }
    BillboardSet::SortByDistanceFunctor::SortByDistanceFunctor(const noVec3& pos)
        : sortPos(pos)
    {
    }
    float BillboardSet::SortByDistanceFunctor::operator()(Billboard* bill) const
    {
        ...
        return - (sortPos - bill->getPosition()).LengthSqr();
    }
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    void BillboardSet::_notifyCurrentCamera( BaseCamera* cam )
    {
  ...
  //      mCamDir = mCamQ * noVec3::NEGATIVE_UNIT_Z;
    }
    //-----------------------------------------------------------------------
    void BillboardSet::beginBillboards(size_t numBillboards)
    {
        ...
        */
		...
		{
			// clamp to max
			numBillboards = min(mPoolSize, numBillboards);
			...
			mLockPtr = static_cast<float*>(GetRenderer()->LockVB(mVB));
		}
    ...
    {
        //mMainBuf->unlock();
		GetRenderer()->UnlockVB(mVB);
    }
	//-----------------------------------------------------------------------
	void BillboardSet::setBounds(const BoundingBox& box, Real radius)
	{
		mAABB = box;
    ...
    void BillboardSet::_updateBounds(void)
    {
        ...
        else
        {
            ...
            noVec3 min_(noMath::INFINITY, noMath::INFINITY, noMath::INFINITY);
            noVec3 max_(-noMath::INFINITY,-noMath::INFINITY, -noMath::INFINITY);
   ...
   //             max.makeCeil(pos);
			...
			//	maxSqLen = max(maxSqLen, pos.LengthSqr());
   //         }
   ...
   //         mAABB.setExtents(min, max);
    ...
    //-----------------------------------------------------------------------
    const BoundingBox& BillboardSet::getBoundingBox(void) const
    {
        return mAABB;
    }
  ...
  //  }
    ...
    //-----------------------------------------------------------------------
    void BillboardSet::getWorldTransforms( mat4* xform ) const
    {
        if (mWorldSpace)
        {
            //*xform = Identity4();
    ...
    void BillboardSet::_createBuffers(void)
    {
        ...
        */
		...
		BufferAccess access = STATIC;
		if (mAutoUpdate) access = DYNAMIC;
		mVB = renderer->addVertexBuffer( 36 * nVertices , access);

		// Create our internal buffer for manipulations
		uint nIndices = mPoolSize * 6;
		uint16* pIdx = new uint16[nIndices];
		...
		mIB = renderer->addIndexBuffer(nIndices, sizeof(WORD), STATIC, pIdx);
    ...
    //-----------------------------------------------------------------------
    bool BillboardSet::billboardVisible(BaseCamera* cam, const Billboard& bill)
    {
    ...
    //-----------------------------------------------------------------------
    void BillboardSet::genBillboardAxes(noVec3* pX, noVec3 *pY, const Billboard* bb)
    {
        ...
        case BBT_POINT:
            if (mAccurateFacing)
            {
                ...
                *pY = mCamQ * noVec3(0.f, 1.f, 0.f);
                *pX = mCamDir.Cross(*pY);
                pX->Normalize();
                *pY = pX->Cross(mCamDir); // both Normalized already
            }
            else
            {
                // Get camera axes for X and Y (depth is irrelevant)
                *pX = mCamQ * noVec3(1.f, 0.f, 0.f);
                *pY = mCamQ * noVec3(0.f, 1.f, 0.f);
            }
            break;
        ...
        case BBT_PERPENDICULAR_COMMON:
            ...
            *pX = mCommonUpVector.Cross(mCommonDirection);
            *pY = mCommonDirection.Cross(*pX);
            break;

        case BBT_PERPENDICULAR_SELF:
            // X-axis is up-vector cross own direction
            // Y-axis is own direction cross X-axis
            *pX = mCommonUpVector.Cross(bb->mDirection);
            pX->Normalize();
            *pY = bb->mDirection.Cross(*pX); // both should be Normalized
            break;
        }
    ...
    //-----------------------------------------------------------------------
    void BillboardSet::setCommonDirection(const noVec3& vec)
    {
        mCommonDirection = vec;
    }
    //-----------------------------------------------------------------------
    const noVec3& BillboardSet::getCommonDirection(void) const
    {
        return mCommonDirection;
    }
    //-----------------------------------------------------------------------
    void BillboardSet::setCommonUpVector(const noVec3& vec)
    {
        mCommonUpVector = vec;
    }
    //-----------------------------------------------------------------------
    const noVec3& BillboardSet::getCommonUpVector(void) const
    {
    ...
    void BillboardSet::genVertices(
        const noVec3* const offsets, const Billboard& bb)
    {
#if 1
        RGBA colour;
        colour = bb.mColour.getAsARGB();
		RGBA* pCol;
		...
		else if (mAllDefaultRotation || bb.mRotation == 0.f)
        {
            // Left-top
            // Positions
   ...
   //         noVec3 axis = (offsets[3] - offsets[0]).Cross(offsets[2] - offsets[1]).NormalizedCopy();
   ...
   //         *mLockPtr++ = r.bottom;
        ...
        {
            const Real      cos_rot  ( noMath::Cos(bb.mRotation)   );
            const Real      sin_rot  ( noMath::Sin(bb.mRotation)   );
    ...
    void BillboardSet::genVertOffsets(Real inleft, Real inright, Real intop, Real inbottom,
        Real width, Real height, const noVec3& x, const noVec3& y, noVec3* pDestVec)
    {
        noVec3 vLeftOff, vRightOff, vTopOff, vBottomOff;
    ...
    //-----------------------------------------------------------------------
    Real BillboardSet::getSquaredViewDepth(const BaseCamera* const cam) const
    {
      //  assert(mParentNode);
    ...
    void BillboardSet::setTextureStacksAndSlices( uint8 stacks, uint8 slices )
    {
	...
	void BillboardSet::UpdateBillboards()
	{
		// If we're driving this from our own data, update geometry if need to.
		if (!mExternalData && (mAutoUpdate || mBillboardDataChanged || !mBuffersCreated))
		{
