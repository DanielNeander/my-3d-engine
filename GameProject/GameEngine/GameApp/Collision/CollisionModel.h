...
typedef struct {
	...
	noVec3					point;			// point of contact
	noVec3					normal;			// contact plane normal
...
typedef struct trace_s {
	...
	noVec3					endpos;			// final position of trace model
	noMat3					endAxis;		// final axis of trace model
...
public:
	...
	virtual bool			GetModelVertex( cmHandle_t model, int vertexNum, noVec3 &vertex ) const = 0;
	// Gets an edge of a model.
	virtual bool			GetModelEdge( cmHandle_t model, int edgeNum, noVec3 &start, noVec3 &end ) const = 0;
	...
	virtual void			Translation( trace_t *results, const noVec3 &start, const noVec3 &end,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) = 0;
	// Rotates a trace model and reports the first collision if any.
	virtual void			Rotation( trace_t *results, const noVec3 &start, const noRotation &rotation,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) = 0;
	// Returns the contents touched by the trace model or 0 if the trace model is in free space.
	virtual int				Contents( const noVec3 &start,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) = 0;
	// Stores all contact points of the trace model with the model, returns the number of contacts.
	virtual int				Contacts( contactInfo_t *contacts, const int maxContacts, const noVec3 &start, const idVec6 &dir, const float depth,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) = 0;

	// Tests collision detection.
	virtual void			DebugOutput( const noVec3 &origin ) = 0;
	// Draws a model.
	virtual void			DrawModel( cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis,
												const noVec3 &viewOrigin, const float radius ) = 0;
