...
============
*/
int idCollisionModelManagerLocal::R_ChoppedAwayByProcBSP( int nodeNum, idFixedWinding *w, const noVec3 &normal, const noVec3 &origin, const float radius ) {
	int res;
	idFixedWinding back;
	cm_procNode_t *node;
	float dist;
...
============
*/
int idCollisionModelManagerLocal::ChoppedAwayByProcBSP( const idFixedWinding &w, const noPlane &plane, int contents ) {
	idFixedWinding neww;
	idBounds bounds;
	float radius;
	noVec3 origin;
...
============
*/
idFixedWinding *idCollisionModelManagerLocal::WindingOutsideBrushes( idFixedWinding *w, const noPlane &plane, int contents, int primitiveNum, cm_node_t *headNode ) {
	int i, windingLeft;
	...
	cm_windingList->bounds[0] -= noVec3( CHOP_EPSILON, CHOP_EPSILON, CHOP_EPSILON );
	cm_windingList->bounds[1] += noVec3( CHOP_EPSILON, CHOP_EPSILON, CHOP_EPSILON );
...
cm_polygon_t *idCollisionModelManagerLocal::TryMergePolygons( cm_model_t *model, cm_polygon_t *p1, cm_polygon_t *p2 ) {
	...
	if ( noMath::Fabs( p1->plane.Dist() - p2->plane.Dist() ) > NORMAL_EPSILON ) {
		return NULL;
	}
	for ( i = 0; i < 3; i++ ) {
		if ( noMath::Fabs( p1->plane.Normal()[i] - p2->plane.Normal()[i] ) > NORMAL_EPSILON ) {
			return NULL;
		}
...
=============
*/
bool idCollisionModelManagerLocal::PointInsidePolygon( cm_model_t *model, cm_polygon_t *p, noVec3 &v ) {
	int i, edgeNum;
	noVec3 *v1, *v2, dir1, dir2, vec;
	cm_edge_t *edge;
...
void idCollisionModelManagerLocal::FindInternalEdgesOnPolygon( cm_model_t *model, cm_polygon_t *p1, cm_polygon_t *p2 ) {
	...
	for ( i = 0; i < p1->numEdges; i++ ) {
		...
		// the edge was not shared
			...
			if ( noMath::Fabs(d) > VERTEX_EPSILON ) {
				continue;
			}
...
================
*/
ID_INLINE int idCollisionModelManagerLocal::HashVec(const noVec3 &vec) {
	/*
	int x, y;
...
================
*/
int idCollisionModelManagerLocal::GetVertex( cm_model_t *model, const noVec3 &v, int *vertexNum ) {
	int i, hashKey, vn;
	noVec3 vert, *p;
	
	for (i = 0; i < 3; i++) {
		if ( noMath::Fabs(v[i] - noMath::Rint(v[i])) < INTEGRAL_EPSILON )
			vert[i] = noMath::Rint(v[i]);
		else
			vert[i] = v[i];
	}
	...
	for (vn = cm_vertexHash->First( hashKey ); vn >= 0; vn = cm_vertexHash->Next( vn ) ) {
		...
		if (noMath::Fabs(vert[2] - (*p)[2]) < VERTEX_EPSILON &&
			noMath::Fabs(vert[0] - (*p)[0]) < VERTEX_EPSILON &&
			noMath::Fabs(vert[1] - (*p)[1]) < VERTEX_EPSILON )
		{
			*vertexNum = vn;
			return true;
		}
...
================
*/
int idCollisionModelManagerLocal::GetEdge( cm_model_t *model, const noVec3 &v1, const noVec3 &v2, int *edgeNum, int v1num ) {
	int v2num, hashKey, e;
	int found, *vertexNum;
...
================
*/
void idCollisionModelManagerLocal::CreatePolygon( cm_model_t *model, idFixedWinding *w, const noPlane &plane, const idMaterial *material, int primitiveNum ) {
...
================
*/
void idCollisionModelManagerLocal::PolygonFromWinding( cm_model_t *model, idFixedWinding *w, const noPlane &plane, const idMaterial *material, int primitiveNum ) {
	int contents;
...
void idCollisionModelManagerLocal::ConvertBrushSides( cm_model_t *model, const idMapBrush *mapBrush, int primitiveNum ) {
	...
	planes = (noPlane *) _alloca16( mapBrush->GetNumSides() * sizeof( planes[0] ) );
...
===================
*/
bool idCollisionModelManagerLocal::GetModelVertex( cmHandle_t model, int vertexNum, noVec3 &vertex ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
...
===================
*/
bool idCollisionModelManagerLocal::GetModelEdge( cmHandle_t model, int edgeNum, noVec3 &start, noVec3 &end ) const {
	if ( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
