...
================
*/
void CM_RotatePoint( noVec3 &point, const noVec3 &origin, const noVec3 &axis, const float tanHalfAngle ) {
	double d, t, s, c;
	noVec3 proj, v1, v2;
...
================
*/
void CM_RotateEdge( noVec3 &start, noVec3 &end, const noVec3 &origin, const noVec3 &axis, const float tanHalfAngle ) {
	double d, t, s, c;
	noVec3 proj, v1, v2;
...
================
*/
int idCollisionModelManagerLocal::CollisionBetweenEdgeBounds( cm_traceWork_t *tw, const noVec3 &va, const noVec3 &vb,
												   const noVec3 &vc, const noVec3 &vd, float tanHalfAngle,
												   noVec3 &collisionPoint, noVec3 &collisionNormal ) {
	float d1, d2, d;
	noVec3 at, bt, dir, dir1, dir2;
	idPluecker	pl1, pl2;
...
void idCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmEdge_t *trmEdge ) {
	...
	cm_edge_t *edge;
	cm_vertex_t *v1, *v2;
	noVec3 collisionPoint, collisionNormal, origin, epsDir;
	idPluecker epsPl;
	idBounds bounds;
	...
	for ( i = 0; i < poly->numEdges; i++ ) {
		...
		if ( ( f1 < 0.0f && f2 > 0.0f ) || ( f1 > 0.0f && f2 < 0.0f ) ) {
			...
			else if ( noMath::Fabs( startTan ) >= tw->maxTan ) {
				// never going to get beyond the start tangent during the current rotation
				continue;
			}
			else {
				// collide with the epsilon expanded edge
				if ( !RotateEdgeThroughEdge(tw, trmEdge->plzaxis, v1->p + epsDir, v2->p + epsDir, noMath::Fabs( startTan ), tanHalfAngle ) ) {
		...
		if ( noMath::Fabs( tanHalfAngle ) >= tw->maxTan ) {
			continue;
		}
		...
		tw->maxTan = noMath::Fabs( tanHalfAngle );
...
================
*/
int idCollisionModelManagerLocal::RotatePointThroughPlane( const cm_traceWork_t *tw, const noVec3 &point, const noPlane &plane,
...
================
*/
int idCollisionModelManagerLocal::PointFurthestFromPlane( const cm_traceWork_t *tw, const noVec3 &point, const noPlane &plane,
...
================
*/
int idCollisionModelManagerLocal::RotatePointThroughEpsilonPlane( const cm_traceWork_t *tw, const noVec3 &point, const noVec3 &endPoint,
	...
	if ( d <= CM_PL_RANGE_EPSILON ) {
		...
		if ( dir <= 0.0f ) {
			...
			if ( !RotatePointThroughPlane( tw, point, epsPlane, angle, noMath::Fabs( startTan ), tanHalfAngle ) ) {
				tanHalfAngle = startTan;
			}
		}
	}
...
================
*/
void idCollisionModelManagerLocal::RotateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly, cm_vertex_t *v, noVec3 &rotationOrigin ) {
	int i, edgeNum;
	float tanHalfAngle;
	noVec3 dir, endp, endDir, collisionPoint;
	idPluecker pl;
	cm_trmEdge_t *edge;
...
bool idCollisionModelManagerLocal::RotateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	if ( noMath::Fabs(d) > tw->maxDistFromHeartPlane1 ) {
		return false;
	}
...
================
*/
void idCollisionModelManagerLocal::BoundsForRotation( const noVec3 &origin, const noVec3 &axis, const noVec3 &start, const noVec3 &end, idBounds &bounds ) {
	int i;
	float radiusSqr;
	noVec3 v1, v2;
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( ( v1[i] > 0.0f && v2[i] < 0.0f ) || ( v1[i] < 0.0f && v2[i] > 0.0f ) ) {
			if ( ( 0.5f * (start[i] + end[i]) - origin[i] ) > 0.0f ) {
				...
				bounds[1][i] = origin[i] + noMath::Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
			}
			else {
				bounds[0][i] = origin[i] - noMath::Sqrt( radiusSqr * ( 1.0f - axis[i] * axis[i] ) );
...
================
*/
void idCollisionModelManagerLocal::Rotation180( trace_t *results, const noVec3 &rorg, const noVec3 &axis,
	...
	noVec3 dir, dir1, dir2, tmp, vr, vup, org, at, bt;
	noMat3 invModelAxis, endAxis, tmpAxis;
	noRotation startRotation, endRotation;
	idPluecker plaxis;
	...
	tw.maxTan = initialTan = noMath::Fabs( tan( ( noMath::PI / 360.0f ) * tw.angle ) );
	...
	if ( d > 0.0f ) {
		maxErr = tw.radius - noMath::Sqrt( d );
	} else {
		maxErr = tw.radius;
	}
	...
	if ( !trm || ( trm->bounds[1][0] - trm->bounds[0][0] <= 0.0f &&
		...
		*results = tw.trace;
		...
		if ( tw.maxTan == initialTan ) {
			...
			results->fraction = noMath::Fabs( atan( tw.maxTan ) * ( 2.0f * 180.0f / noMath::PI ) / tw.angle );
		}
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( noMath::Fabs( tw.size[0][i] ) > noMath::Fabs( tw.size[1][i] ) ) {
			tw.extents[i] = noMath::Fabs( tw.size[0][i] ) + maxErr + CM_BOX_EPSILON;
		} else {
			tw.extents[i] = noMath::Fabs( tw.size[1][i] ) + maxErr + CM_BOX_EPSILON;
		}
	}
	...
	for ( i = 0; i < tw.numVerts; i++ ) {
		d = noMath::Fabs( tw.heartPlane1.Distance( tw.vertices[i].p ) );
...
void idCollisionModelManagerLocal::Rotation( trace_t *results, const noVec3 &start, const noRotation &rotation,
										const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
										cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) {
	noVec3 tmp;
