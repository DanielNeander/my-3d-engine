...
void PostProcessorBase::Initialize(ID3D11Device* device)
{
    ...
    quadVS.Attach(CompileVSFromFile(device, L"Data\\Shaders\\Quad.hlsl", "QuadVS", "vs_4_0", NULL, NULL, &byteCode));
    ...
    DXCall(device->CreateInputLayout(layout, 2, byteCode->GetBufferPointer(), byteCode->GetBufferSize(), &quadInputLayout));
    ...
    DXCall(device->CreateBuffer(&desc, NULL, &psConstants));
...
void PostProcessorBase::Render(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* input, ID3D11RenderTargetView*)
{
                                                                            ...
                                                                            BOOL generateMipMaps, BOOL useAsUAV)
{
    ...
    DXCall(device->CreateTexture2D(&desc, NULL, &rt->Texture));
    DXCall(device->CreateRenderTargetView(rt->Texture, NULL, &rt->RTView));
    DXCall(device->CreateShaderResourceView(rt->Texture, NULL, &rt->SRView));
