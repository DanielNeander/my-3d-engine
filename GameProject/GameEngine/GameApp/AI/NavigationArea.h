...
{
	public:
		void RectangulariseWorld ( MapCellTypes type, int maxAreaSize );
		void AddMapArea (CMapArea *area ) { areas.push_back(area); }

	private:
		CMapCell mCells[MAP_WIDTH_CELLS][MAP_HEIGHT_CELLS];
		std::list<CMapArea*> areas;
};
...
{
	public:
		void SetBoundaries( int south, int west, int north, int east ) { mNorth = north; mSouth = south; mEast = east, mWest = west; }
		void SetType( MapCellTypes type ) { mType = type; }
