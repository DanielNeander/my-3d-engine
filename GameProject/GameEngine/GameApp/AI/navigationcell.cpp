/*$T navigationcell.cpp GC 1.140 01/03/12 01:39:30 */
...
/**
 *  \ NavigationCell.cpp NavigationCell component implementation for the Navimesh sample program. Included as part of
 *  the Game Programming Gems sample code. Created 3/18/00 by Greg Snook greg@mightystudios.com Notes/Revisions: \
 */
#include "stdafx.h"
...
/*
 =======================================================================================================================
    ClassifyPathToCell Classifies a Path in relationship to this cell. A path is represented by a 2D line where Point A
    is the start of the path and Point B is the desired position. If the path exits this cell on a side which is linked
    to another cell, that cell index is returned in the NextCell parameter and SideHit contains the side number of the
    wall exited through. If the path collides with a side of the cell which has no link (a solid edge), SideHit
    contains the side number (0-2) of the colliding wall. In either case PointOfIntersection will contain the point
    where the path intersected with the wall of the cell if it is provided by the caller.
 =======================================================================================================================
 */
NavigationCell::PATH_RESULT NavigationCell::ClassifyPathToCell
(
...
/*
 =======================================================================================================================
    ProjectPathOnCellWall ProjectPathOnCellWall projects a path intersecting the wall with the wall itself. This can be
    used to convert a path colliding with a cell wall to a resulting path moving along the wall. The input parameter
    MotionPath MUST contain a starting point (EndPointA) which is the point of intersection with the path and cell wall
    number [SideNumber] and an ending point (EndPointB) which resides outside of the cell.
 =======================================================================================================================
 */
void NavigationCell::ProjectPathOnCellWall(CELL_SIDE SideNumber, Line2D &MotionPath) const
{
	...
	noVec2 WallNormal = m_Side[SideNumber].EndPointB() - m_Side[SideNumber].EndPointA();
	WallNormal.Normalize();

	/* determine the vector of our current movement */
	noVec2 MotionVector = MotionPath.EndPointB() - MotionPath.EndPointA();
	 ...
	 */
	float	DotResult = (MotionVector * WallNormal);

	/*
	 * our projected vector is then the normalized wall vector times our new found
	 * magnatude
	 */
	 ...
	 */
	noVec2 NewPoint = MotionPath.EndPointA();
...
/*
 ...
 =======================================================================================================================
 */
bool NavigationCell::ForcePointToWallInterior(CELL_SIDE SideNumber, noVec2 &TestPoint) const
{
	...
	if(Distance <= Epsilon)
	{
		...
		noVec2 Normal = m_Side[SideNumber].Normal();
...
/*
 ...
 =======================================================================================================================
 */
bool NavigationCell::ForcePointToWallInterior(CELL_SIDE SideNumber, noVec3 &TestPoint) const
{
...
/*
 ...
 =======================================================================================================================
 */
bool NavigationCell::ForcePointToCellCollumn(noVec2 &TestPoint) const
{
	bool			PointAltered = false;

	/* create a motion path from the center of the cell to our point */
	Line2D			TestPath(noVec2(m_CenterPoint.x, m_CenterPoint.z), TestPoint);
	noVec2			PointOfIntersection;
	CELL_SIDE		Side;
	...
	{
		noVec2 PathDirection(PointOfIntersection.x - m_CenterPoint.x, PointOfIntersection.y - m_CenterPoint.z);
...
/*
 ...
 =======================================================================================================================
 */
bool NavigationCell::ForcePointToCellCollumn(noVec3 &TestPoint) const
{
	noVec2 TestPoint2D(TestPoint.x, TestPoint.z);
	...
	if(m_SessionID == pHeap->SessionID())
	{
		...
		for(int i = 0; i < 3; ++i)
		{
			if(m_Link[i])
			{
				...
				/*
				 * abs(i-m_ArrivalWall) is a formula to determine which distance measurement to
				 * use. ;
...
/*
 ...
 =======================================================================================================================
 */
void NavigationCell::ComputeHeuristic(const noVec3 &Goal)
{

	/*
	 * our heuristic is the estimated distance (using the longest axis delta) between
	 * our ;
