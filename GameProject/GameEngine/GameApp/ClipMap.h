...
public:
	...
	std::vector<ClipStackEntry> mLevels;
	...
	/// want bound.
	CShaderLoader* getMaterialAndTextures(U32 layer1, U32 layer2, U32 layer3=-1, U32 layer4=-1, bool doTerrainRenderHack = true);

	// Same as above, but for FF.
	CShaderLoader* bindTexturesFF(U32 curLevel, Point4F &pt, bool firstLayer, bool doTerrainRenderHack = true);

	/// Reinitialize the cache with our current settings.
	void refreshCache();

	void purgeNextUpdate() { mForceClipmapPurge = true; }

	/// Shader constants used by clipmap shaders
	static const std::string smMorphT;
	static const std::string smMapInfo;
	static const std::string smBrightnessScale;
	static const std::string smDetData;
...
private:
	...
	CShaderLoader* mClipmapMat[MAX_LEVELS];   
	std::vector<noVec4> mMapInfoConst;

	CShaderConstantDirectX10 mMorphTSC[MAX_LEVELS];
	CShaderConstantDirectX10 mMapInfoTC[MAX_LEVELS];
	CShaderConstantDirectX10 mDiffuseMap0TC[MAX_LEVELS];
	CShaderConstantDirectX10 mDiffuseMap1TC[MAX_LEVELS];
	CShaderConstantDirectX10 mDiffuseMap2TC[MAX_LEVELS];
	CShaderConstantDirectX10 mDiffuseMap3TC[MAX_LEVELS];
