...
typedef struct trmCache_s {
	...
	float					volume;
	noVec3					centerOfMass;
	noMat3					inertiaTensor;
} trmCache_t;

noVec3 vec3_boxEpsilon( CM_BOX_EPSILON, CM_BOX_EPSILON, CM_BOX_EPSILON );
...
int idClipModel::GetTraceModelHashKey( const idTraceModel &trm ) {
	const noVec3 &v = trm.bounds[0];
	return ( trm.type << 8 ) ^ ( trm.numVerts << 4 ) ^ ( trm.numEdges << 2 ) ^ ( trm.numPolys << 0 ) ^ noMath::FloatHash( v.ToFloatPtr(), v.GetDimension() );
}
...
================
*/
void idClipModel::SetPosition( const noVec3 &newOrigin, const noMat3 &newAxis ) {
	if ( clipLinks ) {
...
================
*/
void idClipModel::GetMassProperties( const float density, float &mass, noVec3 &centerOfMass, noMat3 &inertiaTensor ) const {
	if ( traceModelIndex == -1 ) {
...
===============
*/
void idClipModel::Link( idClip &clp, idEntity *ent, int newId, const noVec3 &newOrigin, const noMat3 &newAxis, int renderModelHandle ) {
...
===============
*/
clipSector_t *idClip::CreateClipSectors_r( const int depth, const idBounds &bounds, noVec3 &maxSector ) {
	int				i;
	clipSector_t	*anode;
	noVec3			size;
	idBounds		front, back;
...
void idClip::Init( void ) {
	cmHandle_t h;
	noVec3 size, maxSector = vec3_origin;
	...
	defaultClipModel.LoadModel( idTraceModel( idBounds( noVec3( 0, 0, 0 ) ).Expand( 8 ) ) );
...
============
*/
void idClip::TraceRenderModel( trace_t &trace, const noVec3 &start, const noVec3 &end, const float radius, const noMat3 &axis, idClipModel *touch ) const {
	trace.fraction = 1.0f;
...
============
*/
ID_INLINE bool TestHugeTranslation( trace_t &results, const idClipModel *mdl, const noVec3 &start, const noVec3 &end, const noMat3 &trmAxis ) {
...
============
*/
void idClip::TranslationEntities( trace_t &results, const noVec3 &start, const noVec3 &end,
...
============
*/
bool idClip::Translation( trace_t &results, const noVec3 &start, const noVec3 &end,
						const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity ) {
	int i, num;
	idClipModel *touch, *clipModelList[MAX_GENTITIES];
	idBounds traceBounds;
...
============
*/
bool idClip::Rotation( trace_t &results, const noVec3 &start, const noRotation &rotation,
					const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity ) {
	int i, num;
	idClipModel *touch, *clipModelList[MAX_GENTITIES];
	idBounds traceBounds;
...
============
*/
bool idClip::Motion( trace_t &results, const noVec3 &start, const noVec3 &end, const noRotation &rotation,
					const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity ) {
	int i, num;
	idClipModel *touch, *clipModelList[MAX_GENTITIES];
	noVec3 dir, endPosition;
	idBounds traceBounds;
	float radius;
	trace_t translationalTrace, rotationalTrace, trace;
	noRotation endRotation;
	const idTraceModel *trm;
...
============
*/
int idClip::Contacts( contactInfo_t *contacts, const int maxContacts, const noVec3 &start, const idVec6 &dir, const float depth,
					 const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity ) {
...
============
*/
int idClip::Contents( const noVec3 &start, const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity ) {
	int i, num, contents;
	idClipModel *touch, *clipModelList[MAX_GENTITIES];
	idBounds traceBounds;
	const idTraceModel *trm;
...
============
*/
void idClip::TranslationModel( trace_t &results, const noVec3 &start, const noVec3 &end,
...
============
*/
void idClip::RotationModel( trace_t &results, const noVec3 &start, const noRotation &rotation,
					const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
					cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) {
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numRotations++;
...
============
*/
int idClip::ContactsModel( contactInfo_t *contacts, const int maxContacts, const noVec3 &start, const idVec6 &dir, const float depth,
					const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
					cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) {
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numContacts++;
...
============
*/
int idClip::ContentsModel( const noVec3 &start,
					const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
					cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis ) {
	const idTraceModel *trm = TraceModelForClipModel( mdl );
	idClip::numContents++;
...
============
*/
void idClip::DrawClipModels( const noVec3 &eye, const float radius, const idEntity *passEntity ) {
	int				i, num;
	idBounds		bounds;
	idClipModel		*clipModelList[MAX_GENTITIES];
	idClipModel		*clipModel;
