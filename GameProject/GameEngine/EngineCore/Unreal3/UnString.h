...
//
class FString : protected TArray<TCHAR, FDefaultAllocator>
{
public:
	FString()
	: TArray<TCHAR, FDefaultAllocator>()
	{}
	FString( const FString& Other )
	: TArray<TCHAR, FDefaultAllocator>( Other.ArrayNum )
	{
		if( ArrayNum )
		{
	...
	{
		TArray<TCHAR, FDefaultAllocator>::Empty(Other.ArrayNum + ExtraSlack);
		if( Other.ArrayNum )
		{
			Add(Other.ArrayNum);
			appMemcpy( GetData(), Other.GetData(), ArrayNum*sizeof(TCHAR) );
	...
	: TArray<TCHAR, FDefaultAllocator>( In && *In ? (appStrlen(In)+1) : 0 )
	{
		if( ArrayNum )
		{
	...
	:	TArray<TCHAR, FDefaultAllocator>( InCount ? InCount+1 : 0 )
	{
		if( ArrayNum )
		{
	...
	FString( ENoInit )
	: TArray<TCHAR, FDefaultAllocator>( E_NoInit )
	{}
	...
	{
		assert(ArrayNum>=0);
		assert(!ArrayNum || !(GetTypedData()[ArrayNum-1]));
		assert(ArrayMax>=ArrayNum);
	}
	void Empty( INT Slack=0 )
	{
		TArray<TCHAR, FDefaultAllocator>::Empty(Slack > 0 ? Slack + 1 : Slack);
	}
	void Shrink()
	{
		TArray<TCHAR, FDefaultAllocator>::Shrink();
	}
	const TCHAR* operator*() const
	{
		return Num() ? &(*this)(0) : TEXT("");
	}
	TArray<TCHAR, FDefaultAllocator>& GetCharArray()
	{
		//warning: Operations on the TArray<CHAR> can be unsafe, such as adding
		// non-terminating 0's or removing the terminating zero.
		return (TArray<TCHAR, FDefaultAllocator>&)*this;
	}
	 ...
	 */
	INT ParseIntoArray( TArray<FString, FDefaultAllocator>* InArray, const TCHAR* pchDelim, UBOOL InCullEmpty ) const;
	 ...
	 * @return	The number of elements in InArray
	 */
	INT ParseIntoArrayWS( TArray<FString, FDefaultAllocator>* InArray, const TCHAR* pchExtraDelim = NULL ) const;
	 ...
	 */
	static INT CullArray( TArray<FString, FDefaultAllocator>* InArray )
	{
		assert(InArray);
	 ...
	 */
	FString ReplaceCharWithEscapedChar( const TArray<TCHAR, FDefaultAllocator>* Chars=NULL ) const;
	/**
	 ...
	 */
	FString ReplaceEscapedCharWithChar( const TArray<TCHAR, FDefaultAllocator>* Chars=NULL ) const;
