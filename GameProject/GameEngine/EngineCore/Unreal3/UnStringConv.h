...
public:
	 ...
	 */
	FORCEINLINE ANSICHAR* Convert(const UNICHAR* Source,ANSICHAR* Dest,DWORD Size)
	{
...
public:
	 ...
	 * @param Size the size of the dest buffer in bytes
	 */
	FORCEINLINE UNICHAR* Convert(const ANSICHAR* Source,UNICHAR* Dest,DWORD Size)
	{
		// Determine whether we need to allocate memory or not
#if _MSC_VER
		DWORD Length = (DWORD)lstrlenA(Source) + 1;
#else
		DWORD Length = (DWORD)wcslen(Source) + 1;
#endif
		if (Length > Size)
		{
			// Need to allocate memory because the string is too big
			Dest = new UNICHAR[Length * sizeof(TCHAR)];
		}
		// Now do the conversion
#if _MSC_VER
...
public:
	 ...
	 * @param Size the size of the dest buffer in bytes
	 */
	FORCEINLINE ANSICHAR* Convert(const WCHAR* Source,ANSICHAR* Dest,DWORD Size)
	{
		// Determine whether we need to allocate memory or not
#if _MSC_VER
		DWORD LengthW = (DWORD)lstrlenW(Source) + 1;
...
typedef TStringConversion<ANSICHAR,WCHAR,FTCHARToANSI_Convert> FTCHARToANSI;
typedef TStringConversion<ANSICHAR,WCHAR,FTCHARToOEM_Convert> FTCHARToOEM;
