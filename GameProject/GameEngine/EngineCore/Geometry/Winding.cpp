...
=============
*/
void idWinding::BaseForPlane( const noVec3 &normal, const float dist ) {
	noVec3 org, vright, vup;
...
=============
*/
int idWinding::Split( const noPlane &plane, const float epsilon, idWinding **front, idWinding **back ) const {
	float *			dists;
	byte *			sides;
	int				counts[3];
	float			dot;
...
=============
*/
idWinding *idWinding::Clip( const noPlane &plane, const float epsilon, const bool keepOn ) {
	float *		dists;
	byte *		sides;
	idVec5 *	newPoints;
	int			newNumPoints;
...
=============
*/
bool idWinding::ClipInPlace( const noPlane &plane, const float epsilon, const bool keepOn ) {
	float*		dists;
	byte *		sides;
	idVec5 *	newPoints;
	int			newNumPoints;
...
=============
*/
float idWinding::GetRadius( const noVec3 &center ) const {
	int i;
	float radius, r;
	noVec3 dir;
...
=============
*/
noVec3 idWinding::GetCenter( void ) const {
	int i;
	noVec3 center;
...
=============
*/
void idWinding::GetPlane( noVec3 &normal, float &dist ) const {
	noVec3 v1, v2, center;
...
=============
*/
void idWinding::GetPlane( noPlane &plane ) const {
	noVec3 v1, v2;
	noVec3 center;
...
=============
*/
void idWinding::RemoveColinearPoints( const noVec3 &normal, const float epsilon ) {
	int i, j;
	noVec3 edgeNormal;
	float dist;
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( noMath::Fabs( edgeNormal * p[(i+1)%numPoints].ToVec3() - dist ) > epsilon ) {
			continue;
		}
...
=============
*/
void idWinding::AddToConvexHull( const idWinding *winding, const noVec3 &normal, const float epsilon ) {
	int				i, j, k;
	noVec3			dir;
	float			d;
	int				maxPts;
	...
	hullDirs = (noVec3 *) _alloca( maxPts * sizeof( noVec3 ) );
...
=============
*/
void idWinding::AddToConvexHull( const noVec3 &point, const noVec3 &normal, const float epsilon ) {
	...
	hullDirs = (noVec3 *) _alloca( numPoints * sizeof( noVec3 ) );
...
idWinding *idWinding::TryMerge( const idWinding &w, const noVec3 &planenormal, int keep ) const {
	noVec3			*p1, *p2, *p3, *p4, *back;
	idWinding		*newf;
	...
	for ( i = 0; i < f1->numPoints; i++ ) {
		...
		for ( j = 0; j < f2->numPoints; j++ ) {
			...
			for (k = 0; k < 3; k++ ) {
				if ( noMath::Fabs((*p1)[k] - (*p4)[k]) > 0.1f ) {
					break;
				}
				if ( noMath::Fabs((*p2)[k] - (*p3)[k]) > 0.1f ) {
					break;
				}
			}
			if ( k == 3 ) {
...
=============
*/
void idWinding::InsertPoint( const noVec3 &point, int spot ) {
	int i;
...
=============
*/
bool idWinding::InsertPointIfOnEdge( const noVec3 &point, const noPlane &plane, const float epsilon ) {
	int i;
	float dist, dot;
	noVec3 normal;

	// point may not be too far from the winding plane
	if ( noMath::Fabs( plane.Distance( point ) ) > epsilon ) {
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( noMath::Fabs( normal * point - dist ) > epsilon ) {
			continue;
		}
...
=============
*/
float idWinding::PlaneDistance( const noPlane &plane ) const {
	int		i;
	float	d, min, max;
...
=============
*/
int idWinding::PlaneSide( const noPlane &plane, const float epsilon ) const {
	bool	front, back;
	int		i;
	float	d;
...
bool idWinding::PlanesConcave( const idWinding &w2, const noVec3 &normal1, const noVec3 &normal2, float dist1, float dist2 ) const {
	int i;
...
=============
*/
bool idWinding::PointInside( const noVec3 &normal, const noVec3 &point, const float epsilon ) const {
	int i;
	noVec3 dir, n, pointvec;
...
=============
*/
bool idWinding::LineIntersection( const noPlane &windingPlane, const noVec3 &start, const noVec3 &end, bool backFaceCull ) const {
	...
	if ( noMath::Fabs(front - back) < 0.0001f ) {
		mid = end;
	}
	else {
...
=============
*/
bool idWinding::RayIntersection( const noPlane &windingPlane, const noVec3 &start, const noVec3 &dir, float &scale, bool backFaceCull ) const {
	int i;
...
=================
*/
float idWinding::TriangleArea( const noVec3 &a, const noVec3 &b, const noVec3 &c ) {
	noVec3	v1, v2;
	noVec3	cross;
...
=============
*/
int idFixedWinding::Split( idFixedWinding *back, const noPlane &plane, const float epsilon ) {
	int		counts[3];
