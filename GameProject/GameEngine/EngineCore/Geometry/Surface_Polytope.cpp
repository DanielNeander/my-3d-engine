...
====================
*/
void idSurface_Polytope::FromPlanes( const noPlane *planes, const int numPlanes ) {
...
void idSurface_Polytope::SetupTetrahedron( const idBounds &bounds ) {
	...
	verts.SetNum( 4 );
	verts[0].xyz = center + noVec3( 0.0f, 0.0f, scale.z );
	verts[1].xyz = center + noVec3( 2.0f * c1 * scale.x, 0.0f, c3 * scale.z );
	verts[2].xyz = center + noVec3( -c1 * scale.x, c2 * scale.y, c3 * scale.z );
	verts[3].xyz = center + noVec3( -c1 * scale.x, -c2 * scale.y, c3 * scale.z );
...
void idSurface_Polytope::SetupHexahedron( const idBounds &bounds ) {
	...
	verts.SetNum( 8 );
	verts[0].xyz = center + noVec3( -scale.x, -scale.y, -scale.z );
	verts[1].xyz = center + noVec3(  scale.x, -scale.y, -scale.z );
	verts[2].xyz = center + noVec3(  scale.x,  scale.y, -scale.z );
	verts[3].xyz = center + noVec3( -scale.x,  scale.y, -scale.z );
	verts[4].xyz = center + noVec3( -scale.x, -scale.y,  scale.z );
	verts[5].xyz = center + noVec3(  scale.x, -scale.y,  scale.z );
	verts[6].xyz = center + noVec3(  scale.x,  scale.y,  scale.z );
	verts[7].xyz = center + noVec3( -scale.x,  scale.y,  scale.z );
...
void idSurface_Polytope::SetupOctahedron( const idBounds &bounds ) {
	...
	verts.SetNum( 6 );
	verts[0].xyz = center + noVec3(  scale.x, 0.0f, 0.0f );
	verts[1].xyz = center + noVec3( -scale.x, 0.0f, 0.0f );
	verts[2].xyz = center + noVec3( 0.0f,  scale.y, 0.0f );
	verts[3].xyz = center + noVec3( 0.0f, -scale.y, 0.0f );
	verts[4].xyz = center + noVec3( 0.0f, 0.0f,  scale.z );
	verts[5].xyz = center + noVec3( 0.0f, 0.0f, -scale.z );
...
====================
*/
int idSurface_Polytope::SplitPolytope( const noPlane &plane, const float epsilon, idSurface_Polytope **front, idSurface_Polytope **back ) const {
