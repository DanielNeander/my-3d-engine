...
typedef noVec3 traceModelVert_t;
...
public:
	...
	noVec3				offset;			// offset to center of model
...
public:
	...
	void				SetupPolygon( const noVec3 *v, const int count );
	void				SetupPolygon( const idWinding &w );
						// generate edge normals
	int					GenerateEdgeNormals( void );
						// translate the trm
	void				Translate( const noVec3 &translation );
						// rotate the trm
	void				Rotate( const noMat3 &rotation );
	...
	int					GetProjectionSilhouetteEdges( const noVec3 &projectionOrigin, int silEdges[MAX_TRACEMODEL_EDGES] ) const;
	int					GetParallelProjectionSilhouetteEdges( const noVec3 &projectionDir, int silEdges[MAX_TRACEMODEL_EDGES] ) const;
