...
public:
					...
					explicit idWinding( const noVec3 *verts, const int n );			// winding from points
					explicit idWinding( const noVec3 &normal, const float dist );	// base winding for plane
					explicit idWinding( const noPlane &plane );						// base winding for plane
	...
	idWinding &		operator+=( const noVec3 &v );
	idWinding &		operator+=( const idVec5 &v );
	void			AddPoint( const noVec3 &v );
	...
	void			BaseForPlane( const noVec3 &normal, const float dist );
	void			BaseForPlane( const noPlane &plane );

					// splits the winding into a front and back winding, the winding itself stays unchanged
					// returns a SIDE_?
	int				Split( const noPlane &plane, const float epsilon, idWinding **front, idWinding **back ) const;
	...
	idWinding *		Clip( const noPlane &plane, const float epsilon = ON_EPSILON, const bool keepOn = false );
					// cuts off the part at the back side of the plane, returns true if some part was at the front
					// if there is nothing at the front the number of points is set to zero
	bool			ClipInPlace( const noPlane &plane, const float epsilon = ON_EPSILON, const bool keepOn = false );
	...
	void			RemoveColinearPoints( const noVec3 &normal, const float epsilon = ON_EPSILON );
	void			RemovePoint( int point );
	void			InsertPoint( const noVec3 &point, int spot );
	bool			InsertPointIfOnEdge( const noVec3 &point, const noPlane &plane, const float epsilon = ON_EPSILON );
					// add a winding to the convex hull
	void			AddToConvexHull( const idWinding *winding, const noVec3 &normal, const float epsilon = ON_EPSILON );
					// add a point to the convex hull
	void			AddToConvexHull( const noVec3 &point, const noVec3 &normal, const float epsilon = ON_EPSILON );
	...
	idWinding *		TryMerge( const idWinding &w, const noVec3 &normal, int keep = false ) const;
					// check whether the winding is valid or not
	bool			Check( bool print = true ) const;

	float			GetArea( void ) const;
	noVec3			GetCenter( void ) const;
	float			GetRadius( const noVec3 &center ) const;
	void			GetPlane( noVec3 &normal, float &dist ) const;
	void			GetPlane( noPlane &plane ) const;
	...
	bool			PlanesConcave( const idWinding &w2, const noVec3 &normal1, const noVec3 &normal2, float dist1, float dist2 ) const;

	bool			PointInside( const noVec3 &normal, const noVec3 &point, const float epsilon ) const;
					// returns true if the line or ray intersects the winding
	bool			LineIntersection( const noPlane &windingPlane, const noVec3 &start, const noVec3 &end, bool backFaceCull = false ) const;
					// intersection point is start + dir * scale
	bool			RayIntersection( const noPlane &windingPlane, const noVec3 &start, const noVec3 &dir, float &scale, bool backFaceCull = false ) const;

	static float	TriangleArea( const noVec3 &a, const noVec3 &b, const noVec3 &c );
...
ID_INLINE idWinding::idWinding( const noVec3 *verts, const int n ) {
	int i;
...
ID_INLINE idWinding::idWinding( const noVec3 &normal, const float dist ) {
	numPoints = allocedSize = 0;
	p = NULL;
	BaseForPlane( normal, dist );
}

ID_INLINE idWinding::idWinding( const noPlane &plane ) {
	numPoints = allocedSize = 0;
	p = NULL;
	BaseForPlane( plane );
}
...
ID_INLINE idWinding &idWinding::operator+=( const noVec3 &v ) {
	AddPoint( v );
	return *this;
}
...
ID_INLINE void idWinding::AddPoint( const noVec3 &v ) {
...
ID_INLINE void idWinding::BaseForPlane( const noPlane &plane ) {
...
public:
					...
					explicit idFixedWinding( const noVec3 *verts, const int n );
					explicit idFixedWinding( const noVec3 &normal, const float dist );
					explicit idFixedWinding( const noPlane &plane );
	...
	virtual void	Clear( void );
					...
					// returns a SIDE_?
	int				Split( idFixedWinding *back, const noPlane &plane, const float epsilon = ON_EPSILON );
...
ID_INLINE idFixedWinding::idFixedWinding( const noVec3 *verts, const int n ) {
	int i;
...
ID_INLINE idFixedWinding::idFixedWinding( const noVec3 &normal, const float dist ) {
	numPoints = 0;
	p = data;
	allocedSize = MAX_POINTS_ON_WINDING;
	BaseForPlane( normal, dist );
...
ID_INLINE idFixedWinding::idFixedWinding( const noPlane &plane ) {
	numPoints = 0;
	p = data;
	allocedSize = MAX_POINTS_ON_WINDING;
	BaseForPlane( plane );
