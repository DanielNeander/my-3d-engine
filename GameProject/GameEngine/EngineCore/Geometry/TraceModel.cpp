...
============
*/
void idTraceModel::SetupPolygon( const noVec3 *v, const int count ) {
	int i, j;
	noVec3 mid;
...
void idTraceModel::SetupPolygon( const idWinding &w ) {
	...
	verts = (noVec3 *) _alloca16( w.GetNumPoints() * sizeof( noVec3 ) );
...
============
*/
void idTraceModel::Translate( const noVec3 &translation ) {
	int i;
...
============
*/
void idTraceModel::Rotate( const noMat3 &rotation ) {
	int i, j, edgeNum;
...
============
*/
int idTraceModel::GetProjectionSilhouetteEdges( const noVec3 &projectionOrigin, int silEdges[MAX_TRACEMODEL_EDGES] ) const {
	int i, j, edgeNum;
...
============
*/
int idTraceModel::GetParallelProjectionSilhouetteEdges( const noVec3 &projectionDir, int silEdges[MAX_TRACEMODEL_EDGES] ) const {
	int i, j, edgeNum;
	int edgeIsSilEdge[MAX_TRACEMODEL_EDGES+1];
	const traceModelPoly_t *poly;
...
void idTraceModel::VolumeIntegrals( struct volumeIntegrals_s &integrals ) const {
	...
	for ( i = 0; i < numPolys; i++ ) {
		...
		nx = noMath::Fabs( poly->normal[0] );
		ny = noMath::Fabs( poly->normal[1] );
		nz = noMath::Fabs( poly->normal[2] );
		if ( nx > ny && nx > nz ) {
			c = 0;
		}
		else {
...
============
*/
void idTraceModel::GetMassProperties( const float density, float &mass, noVec3 &centerOfMass, noMat3 &inertiaTensor ) const {
