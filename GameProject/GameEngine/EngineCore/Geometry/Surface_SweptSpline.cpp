...
====================
*/
void idSurface_SweptSpline::SetSpline( idCurve_Spline<noVec4> *spline ) {
	if ( this->spline ) {
		delete this->spline;
	}
	this->spline = spline;
...
====================
*/
void idSurface_SweptSpline::SetSweptSpline( idCurve_Spline<noVec4> *sweptSpline ) {
	if ( this->sweptSpline ) {
...
void idSurface_SweptSpline::SetSweptCircle( const float radius ) {
	idCurve_NURBS<noVec4> *nurbs = new idCurve_NURBS<noVec4>();
	nurbs->Clear();
	nurbs->AddValue(   0.0f, noVec4(  radius,  radius, 0.0f, 0.00f ) );
	nurbs->AddValue( 100.0f, noVec4( -radius,  radius, 0.0f, 0.25f ) );
	nurbs->AddValue( 200.0f, noVec4( -radius, -radius, 0.0f, 0.50f ) );
	nurbs->AddValue( 300.0f, noVec4(  radius, -radius, 0.0f, 0.75f ) );
	nurbs->SetBoundaryType( idCurve_NURBS<noVec4>::BT_CLOSED );
...
====================
*/
void idSurface_SweptSpline::GetFrame( const noMat3 &previousFrame, const noVec3 dir, noMat3 &newFrame ) {
	float wx, wy, wz;
	float xx, yy, yz;
	float xy, xz, zz;
	float x2, y2, z2;
	...
	a = noMath::ACos( previousFrame[2] * d ) * 0.5f;
	c = noMath::Cos( a );
	s = noMath::Sqrt( 1.0f - c * c );
...
void idSurface_SweptSpline::Tessellate( const int splineSubdivisions, const int sweptSplineSubdivisions ) {
	...
	sweptSplineDiv = sweptSpline->GetBoundaryType() == idCurve_Spline<noVec3>::BT_CLOSED ? sweptSplineSubdivisions : sweptSplineSubdivisions - 1;
	...
	splineDiv = spline->GetBoundaryType() == idCurve_Spline<noVec3>::BT_CLOSED ? splineSubdivisions : splineSubdivisions - 1;
	splineMat.Identity();
