...
=================
*/
int idSurface::Split( const noPlane &plane, const float epsilon, idSurface **front, idSurface **back, int *frontOnPlaneEdges, int *backOnPlaneEdges ) const {
	float *			dists;
	float			f;
	byte *			sides;
	int				counts[3];
...
=================
*/
bool idSurface::ClipInPlace( const noPlane &plane, const float epsilon, const bool keepOn ) {
	float *			dists;
	float			f;
	byte *			sides;
	int				counts[3];
...
=============
*/
float idSurface::PlaneDistance( const noPlane &plane ) const {
	int		i;
	float	d, min, max;
...
=============
*/
int idSurface::PlaneSide( const noPlane &plane, const float epsilon ) const {
	bool	front, back;
	int		i;
	float	d;
...
=================
*/
bool idSurface::LineIntersection( const noVec3 &start, const noVec3 &end, bool backFaceCull ) const {
	float scale;
...
=================
*/
bool idSurface::RayIntersection( const noVec3 &start, const noVec3 &dir, float &scale, bool backFaceCull ) const {
	...
	for ( i = 0; i < edgeIndexes.Num(); i += 3 ) {
		...
		if ( s0 & s1 & s2 ) {
			...
			if ( noMath::Fabs( s ) < noMath::Fabs( scale ) ) {
				scale = s;
			}
	...
	if ( noMath::Fabs( scale ) < noMath::INFINITY ) {
		return true;
	}
	return false;
}
