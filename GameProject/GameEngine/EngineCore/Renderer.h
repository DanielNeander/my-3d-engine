...
public:
	...
	virtual void changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture, const SamplerStateID samplerState)  {}
	virtual void changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture) {}
	virtual void changeVertexFormat2(const VertexFormatID vertexFormatID) {}

	virtual	void* LockVB( VertexBufferID vb ) {return NULL; }
	virtual void UnlockVB( VertexBufferID vb) {}
...
*/
	...
	void setTarget( const char* shadermodel)	{ 
		resetTarget();
		vstarget_ += "vs_";
		vstarget_ += shadermodel; 
		pstarget_ += "ps_";
		pstarget_ += shadermodel; 
		gstarget_ += "gs_";
		gstarget_ += shadermodel; 
	}
	void resetTarget() {
		vstarget_.clear();
		gstarget_.clear();
...
//	float *textureLod;
	std::string vstarget_;	// shader model
	std::string gstarget_;	// shader model
	std::string pstarget_;	// shader model
