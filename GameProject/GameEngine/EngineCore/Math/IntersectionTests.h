...
{
  // min and max vectors
  const noVec3 &vFrustumMin = frustumBB.m_vMin;
  const noVec3 &vFrustumMax = frustumBB.m_vMax;
  const noVec3 &vObjectMin = objectBB.m_vMin;
  const noVec3 &vObjectMax = objectBB.m_vMax;
...
//
//
inline bool SweepIntersectionTest(const BoundingBox &objectBB, const BoundingBox &frustumBB, const noVec3 &vSweepDir)
{
  ...
  noVec3 vFrustumCenter = (vFrustumMin + vFrustumMax) * 0.5f;
  noVec3 vFrustumHalfSize = (vFrustumMax - vFrustumMin) * 0.5f;
  ...
  noVec3 vObjectCenter = (vObjectMin + vObjectMax) * 0.5f;
  noVec3 vObjectHalfSize = (vObjectMax - vObjectMin) * 0.5f;
  ...
  if(fDistMin > fDistMax) UnSwap(fDistMin, fDistMax);
  ...
  else
  {
    ...
    if(fDistMinNew > fDistMaxNew) UnSwap(fDistMinNew, fDistMaxNew);
...
//
//
inline bool ProjectedIntersection(const noVec3 &vHalfSize, const noVec3 &vCenter,
                                  const noVec3 *pFrustumPoints,
                                  const noVec3 &vDir)
{
...
inline bool IntersectionTest(const BoundingBox &objectBB, const Frustum &frustum)
{
  ...
  //
  //
  noVec3 vHalfSize = (objectBB.m_vMax - objectBB.m_vMin) * 0.5f;
  noVec3 vCenter = (objectBB.m_vMin + objectBB.m_vMax) * 0.5f;

  // AABB face normals
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(1,0,0))) return false;
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(0,1,0))) return false;
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(0,0,1))) return false;

  // frustum face normals
  //

  // front and back faces:
  noVec3 vNorm1 = Normalize(Cross(frustum.m_pPoints[1] - frustum.m_pPoints[0],
                                  frustum.m_pPoints[3] - frustum.m_pPoints[0]));
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm1)) return false;

  // left face:
  noVec3 vNorm2 = Normalize(Cross(frustum.m_pPoints[1] - frustum.m_pPoints[0],
                                  frustum.m_pPoints[4] - frustum.m_pPoints[0]));
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm2)) return false;

  // right face:
  noVec3 vNorm3 = Normalize(Cross(frustum.m_pPoints[2] - frustum.m_pPoints[3],
                                  frustum.m_pPoints[7] - frustum.m_pPoints[3]));
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm3)) return false;

  // top face:
  noVec3 vNorm4 = Normalize(Cross(frustum.m_pPoints[2] - frustum.m_pPoints[1],
                                  frustum.m_pPoints[5] - frustum.m_pPoints[1]));
  if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm4)) return false;

  // bottom face:
  noVec3 vNorm5 = Normalize(Cross(frustum.m_pPoints[3] - frustum.m_pPoints[0],
                                  frustum.m_pPoints[4] - frustum.m_pPoints[0]));
  ...
  //
  noVec3 pBoxEdges[3] = {noVec3(1,0,0), noVec3(0,1,0), noVec3(0,0,1)};
  for(int i=0;i<3;i++)
  {
    // edge up-down
    noVec3 vNorm1 = Normalize(Cross(frustum.m_pPoints[1] - frustum.m_pPoints[0], pBoxEdges[i]));
    if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm1)) return false;

    // edge left-right
    noVec3 vNorm2 = Normalize(Cross(frustum.m_pPoints[3] - frustum.m_pPoints[0], pBoxEdges[i]));
    if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm2)) return false;

    // edge bottom left
    noVec3 vNorm3 = Normalize(Cross(frustum.m_pPoints[4] - frustum.m_pPoints[0], pBoxEdges[i]));
    if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm3)) return false;

    // edge top left
    noVec3 vNorm4 = Normalize(Cross(frustum.m_pPoints[5] - frustum.m_pPoints[1], pBoxEdges[i]));
    if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm4)) return false;

    // edge top right
    noVec3 vNorm5 = Normalize(Cross(frustum.m_pPoints[6] - frustum.m_pPoints[2], pBoxEdges[i]));
    if(!ProjectedIntersection(vHalfSize, vCenter, frustum.m_pPoints, vNorm5)) return false;

    // edge bottom right
    noVec3 vNorm6 = Normalize(Cross(frustum.m_pPoints[7] - frustum.m_pPoints[3], pBoxEdges[i]));
...
//
//
inline bool ProjectedSweepIntersection(const noVec3 &vHalfSize, const noVec3 &vCenter,
                                  const noVec3 *pFrustumPoints,
                                  const noVec3 &vDir,
                                  float &fDistMin, float &fDistMax, const noVec3 &vSweepDir)
{
  ...
  if(fSweepDir != 0)
  {
    ...
    if(fIntersectionStart > fIntersectionEnd) UnSwap(fIntersectionStart, fIntersectionEnd);
...
//
//
inline bool SweepIntersectionTest(const BoundingBox &objectBB, const Frustum &frustum, const noVec3 &vSweepDir)
{
  // Note that this code is very unoptimal
  //
  //
  ...
  // AABB face normals
  if(!ProjectedSweepIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(1,0,0), fDistMin, fDistMax, vSweepDir)) return false;
  if(!ProjectedSweepIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(0,1,0), fDistMin, fDistMax, vSweepDir)) return false;
  if(!ProjectedSweepIntersection(vHalfSize, vCenter, frustum.m_pPoints, noVec3(0,0,1), fDistMin, fDistMax, vSweepDir)) return false;
