...
===============
*/
// calculates arc tangent of a vector with 16 bits of precision, based on atan16 in noMath
...
inline void FastScalarInvSqrt_x3( float *arg1, float *arg2, float *arg3 ) {
	...
	*arg3 = estimate3;
#else
	*arg1 = noMath::InvSqrt( *arg1 );
	*arg2 = noMath::InvSqrt( *arg2 );
	*arg3 = noMath::InvSqrt( *arg3 );
#endif
}
...
inline void FastScalarInvSqrt_x6( float *arg1, float *arg2, float *arg3, float *arg4, float *arg5, float *arg6 ) {
	...
	*arg2 = noMath::InvSqrt( *arg2 );
	*arg3 = noMath::InvSqrt( *arg3 );
	*arg4 = noMath::InvSqrt( *arg4 );
	*arg5 = noMath::InvSqrt( *arg5 );
	*arg6 = noMath::InvSqrt( *arg6 );	
#endif
}
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noVec3 &constant, const noVec3 *src, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noVec3 &constant, const noPlane *src, const int count ) {
//#define OPER(X) dst[(X)] = constant * src[(X)].Normal() + src[(X)][3];

	assert( sizeof(noPlane)  == PLANE_OFFSET * sizeof(float) );
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noVec3 &constant, const idDrawVert *src, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noPlane &constant, const noVec3 *src, const int count ) {
//#define OPER(X) dst[(X)] = constant.Normal() * src[(X)] + constant[3];
		...
		noVec3 constNormal = constant.Normal();
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noPlane &constant, const noPlane *src, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noPlane &constant, const idDrawVert *src, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::Dot( float *dst, const noVec3 *src0, const noVec3 *src1, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::MinMax( noVec2 &min, noVec2 &max, const noVec2 *src, const int count ) {
	min[0] = min[1] = noMath::INFINITY; max[0] = max[1] = -noMath::INFINITY;
//#define OPER(X) const noVec2 &v = src[(X)]; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; }
...
============
*/
void VPCALL idSIMD_AltiVec::MinMax( noVec3 &min, noVec3 &max, const noVec3 *src, const int count ) {
	min[0] = min[1] = min[2] = noMath::INFINITY; max[0] = max[1] = max[2] = -noMath::INFINITY;
//#define OPER(X) const noVec3 &v = src[(X)]; if ( v[0] < min[0] ) { min[0] = v[0]; } if ( v[0] > max[0] ) { max[0] = v[0]; } if ( v[1] < min[1] ) { min[1] = v[1]; } if ( v[1] > max[1] ) { max[1] = v[1]; } if ( v[2] < min[2] ) { min[2] = v[2]; } if ( v[2] > max[2] ) { max[2] = v[2]; }
	...
	if ( count >= 4 ) {
		...
		for ( ; i+3 < count; i += 4 ) {
			...
			// put each noVec3 into its own vector as X Y Z (crap)
			vecSrc1 = vecLd1;
...
============
*/
void VPCALL idSIMD_AltiVec::MinMax( noVec3 &min, noVec3 &max, const idDrawVert *src, const int count ) {
...
============
*/
void VPCALL idSIMD_AltiVec::MinMax( noVec3 &min, noVec3 &max, const idDrawVert *src, const int *indexes, const int count ) {
	min[0] = min[1] = min[2] = noMath::INFINITY; max[0] = max[1] = max[2] = -noMath::INFINITY;
...
void VPCALL idSIMD_AltiVec::BlendJoints( idJointQuat *joints, const idJointQuat *blendJoints, const float lerp, const int *index, const int numJoints ) {
	...
	for ( i = 0; i+3 < numJoints; i+=4 ) {
		...
		// lerp is  v1 + l * ( v2 - v1 );	
		// the noVec3 T is going to be 12 bytes after the Q, so we can do this without calling ToFloatPtr() again. since its
...
void VPCALL idSIMD_AltiVec::ConvertJointMatsToJointQuats( idJointQuat *jointQuats, const idJointMat *jointMats, const int numJoints ) {
	...
	// bigger than 50% speedup, which is not too shabby. Should really replace noMath::InvSqrt with
...
void VPCALL idSIMD_AltiVec::TransformVerts( idDrawVert *verts, const int numVerts, const idJointMat *joints, const noVec4 *weights, const int *index, int numWeights ) {
	int i, j;
...
============
*/
void VPCALL idSIMD_AltiVec::TracePointCull( byte *cullBits, byte &totalOr, const float radius, const noPlane *planes, const idDrawVert *verts, const int numVerts ) {
...
============
*/
void VPCALL idSIMD_AltiVec::DecalPointCull( byte *cullBits, const noPlane *planes, const idDrawVert *verts, const int numVerts ) {
...
============
*/
void VPCALL idSIMD_AltiVec::OverlayPointCull( byte *cullBits, noVec2 *texCoords, const noPlane *planes, const idDrawVert *verts, const int numVerts ) {
...
============
*/
void VPCALL idSIMD_AltiVec::DeriveTriPlanes( noPlane *planes, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
	...
	for ( ; i < numIndexes; i += 3, j++ ) {
		...
		//noMath::RSqrt( n.x * n.x + n.y * n.y + n.z * n.z );
...
============
*/
void VPCALL idSIMD_AltiVec::DeriveTangents( noPlane *planes, idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
	int i;

	bool *used = (bool *)_alloca16( numVerts * sizeof( used[0] ) );
	memset( used, 0, numVerts * sizeof( used[0] ) );
	
	noPlane *planesPtr = planes;
...
void VPCALL idSIMD_AltiVec::NormalizeTangents( idDrawVert *verts, const int numVerts ) {
	...
	// cleanup
	for ( ; i < numVerts; i++ ) {
		noVec3 &v = verts[i].normal;
		float f;

		//f = noMath::RSqrt( v.x * v.x + v.y * v.y + v.z * v.z );
		f = FastScalarInvSqrt( v.x * v.x + v.y * v.y + v.z * v.z );
		v.x *= f; v.y *= f; v.z *= f;

		for ( int j = 0; j < 2; j++ ) {
			noVec3 &t = verts[i].tangents[j];

			t -= ( t * v ) * v;
		//	f = noMath::RSqrt( t.x * t.x + t.y * t.y + t.z * t.z );
...
void VPCALL idSIMD_AltiVec::CreateTextureSpaceLightVectors( noVec3 *lightVectors, const noVec3 &lightOrigin, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
...
============
*/
void VPCALL idSIMD_AltiVec::CreateSpecularTextureCoords( noVec4 *texCoords, const noVec3 &lightOrigin, const noVec3 &viewOrigin, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes ) {
...
============
*/
int VPCALL idSIMD_AltiVec::CreateShadowCache( noVec4 *vertexCache, int *vertRemap, const noVec3 &lightOrigin, const idDrawVert *verts, const int numVerts ) {
	int outVerts = 0;
	int i = 0;
...
============
*/
int VPCALL idSIMD_AltiVec::CreateVertexProgramShadowCache( noVec4 *vertexCache, const idDrawVert *verts, const int numVerts ) {
	...
	assert( sizeof(idDrawVert) == DRAWVERT_OFFSET * sizeof(float) );
	// noVec4 size
	assert( sizeof(noVec4) == noVec4_OFFSET * sizeof(float) );
