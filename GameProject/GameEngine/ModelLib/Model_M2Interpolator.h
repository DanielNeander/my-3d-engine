#ifndef MODEL_M2_INTERPOLATOR_H
#define MODEL_M2_INTERPOLATOR_H
...
// "linear" interpolation for noQuats should be slerp by default
template<>
inline noQuat interpolate<noQuat>(const float t, const noQuat &v1, const noQuat &v2)
{
	
	return noQuat::Slerp(v1, v2, t);	
}
...
public:
	static const noQuat conv(const PACK_QUATERNION t)
	{
		return noQuat(
...
//#endif
	...
	void init(AnimationBlock &b, MPQFile &f, uint32 *gs, MPQFile *animfiles)
	{
		...
		//times = new std::vector<unsigned int>[b.nTimes];		
		...
		for(size_t j=0; j < b.nKeys; j++) {
			...
			if (pHeadKeys->nEntrys == 0)
				continue;
			switch (type) {
				case INTERPOLATION_NONE:
				case INTERPOLATION_LINEAR:
					//data[j].resize(pHeadKeys->nEntrys);
					for (size_t i = 0; i < pHeadKeys->nEntrys; i++) 
						data[j].push_back(Conv::conv(keys[i]));
					break;
				case INTERPOLATION_HERMITE:					
