...
public:
	...
	void					Link( idClip &clp, idEntity *ent, int newId, const noVec3 &newOrigin, const noMat3 &newAxis, int renderModelHandle = -1 );
	void					Unlink( void );						// unlink from sectors
	void					SetPosition( const noVec3 &newOrigin, const noMat3 &newAxis );	// unlinks the clip model
	void					Translate( const noVec3 &translation );							// unlinks the clip model
	void					Rotate( const noRotation &rotation );							// unlinks the clip model
	...
	const noMat3 &			GetAxis( void ) const;
	...
	void					GetMassProperties( const float density, float &mass, noVec3 &centerOfMass, noMat3 &inertiaTensor ) const;
...
private:
	...
	noVec3					origin;					// origin of clip model
	noMat3					axis;					// orientation of clip model
...
ID_INLINE void idClipModel::Translate( const noVec3 &translation ) {
	Unlink();
	origin += translation;
}

ID_INLINE void idClipModel::Rotate( const noRotation &rotation ) {
	Unlink();
	origin *= rotation;
...
ID_INLINE const noVec3 &idClipModel::GetOrigin( void ) const {
	return origin;
}

ID_INLINE const noMat3 &idClipModel::GetAxis( void ) const {
	return axis;
}
...
public:
	...
	bool					Translation( trace_t &results, const noVec3 &start, const noVec3 &end,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity );
	bool					Rotation( trace_t &results, const noVec3 &start, const noRotation &rotation,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity );
	bool					Motion( trace_t &results, const noVec3 &start, const noVec3 &end, const noRotation &rotation,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity );
	...
	bool					TracePoint( trace_t &results, const noVec3 &start, const noVec3 &end,
								int contentMask, const idEntity *passEntity );
	bool					TraceBounds( trace_t &results, const noVec3 &start, const noVec3 &end, const idBounds &bounds,
								int contentMask, const idEntity *passEntity );

	// clip versus a specific model
	void					TranslationModel( trace_t &results, const noVec3 &start, const noVec3 &end,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	void					RotationModel( trace_t &results, const noVec3 &start, const noRotation &rotation,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	int						ContactsModel( contactInfo_t *contacts, const int maxContacts, const noVec3 &start, const idVec6 &dir, const float depth,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	int						ContentsModel( const noVec3 &start,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );

	// clip versus all entities but not the world
	void					TranslationEntities( trace_t &results, const noVec3 &start, const noVec3 &end,
								const idClipModel *mdl, const noMat3 &trmAxis, int contentMask, const idEntity *passEntity );
	...
	void					DrawClipModels( const noVec3 &eye, const float radius, const idEntity *passEntity );
...
private:
	struct clipSector_s *	CreateClipSectors_r( const int depth, const idBounds &bounds, noVec3 &maxSector );
	...
	void					TraceRenderModel( trace_t &trace, const noVec3 &start, const noVec3 &end, const float radius, const noMat3 &axis, idClipModel *touch ) const;
};


ID_INLINE bool idClip::TracePoint( trace_t &results, const noVec3 &start, const noVec3 &end, int contentMask, const idEntity *passEntity ) {
	Translation( results, start, end, NULL, mat3_identity, contentMask, passEntity );
	return ( results.fraction < 1.0f );
}

ID_INLINE bool idClip::TraceBounds( trace_t &results, const noVec3 &start, const noVec3 &end, const idBounds &bounds, int contentMask, const idEntity *passEntity ) {
