...
void input(const char * filename, const int width, const int height) ; /* Read in light
							probe */
...
*/
void prefilter(int width, int height) ;         /* The main integration routine */
...
void prefilter(int width, int height) {
  ...
  for (i = 0 ; i < height ; i++)
    for (j = 0 ; j < width ; j++) {
	 ...
	 if the form of the input changes */
	  ...
	  //formula for sphere-map format
      domega = (2*PI/width)*(2*PI/width)*sinc(theta) ;

	  //formula for lat-long maps
	  //domega = (2*PI*PI/(width*height))*sinf(theta) ;
