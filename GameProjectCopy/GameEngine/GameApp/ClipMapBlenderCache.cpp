...
ClipMapBlenderCache::ClipMapBlenderCache( IClipMapImageSource *opacitySource, IClipMapImageSource *lightmapSource, bool LM1 /*= false*/ )
{
...
void ClipMapBlenderCache::doRectUpdate( U32 mipLevel, ClipMap::ClipStackEntry &cse, const RectI &srcRegion, const RectI &dstRegion )
{
	...
	if (mFixedfunction)
	{
		...
		// Draw first layer
		GFX->setStateBlock(mFFBaseLayerSB);
		...
		GFX->setStateBlock(mFFAdditionalLayersSB);
		...
		if (mLightmapScratchTextures.size() > 0)
		{
			...
			GFX->setStateBlock(mFFLightmapSB);
...
#  pragma message("Gotta do a stretchrect here pat, Love Pat." )
	...
	if (GFX->getActiveRenderTarget() != NULL && viewPort.len_x() > 0 && viewPort.len_y() > 0)
...
void ClipMapBlenderCache::registerSourceImage( const String &imgPath )
{
	...
	if(filename.isEmpty())
	{
		...
		mSourceImages.last().set(bmp, &GFXDefaultStaticDiffuseProfile, true, String("ClipmapBlender Source Image"));
		return;
	}    
	...
	if(bmp != NULL)
	{
		// GFXTextureHandle is going to own this, so let's copy it.
		GBitmap  *newBM = new GBitmap(*bmp);
		newBM->extrudeMipLevels();
		mSourceImages.last().set(newBM, &GFXDefaultStaticDiffuseProfile, true, String("ClipmapBlender Source File Image"));
	}
...
void ClipMapBlenderCache::createOpacityScratchTextures()
{
	...
	U32 numExistingScratchTextures = mOpacityScratchTextures.size();
	if( numScratchTextures < numExistingScratchTextures )
	{
		for( U32 i = ( numExistingScratchTextures - 1 ); i >= numScratchTextures; -- i )
			mOpacityScratchTextures.decrement();
	}
	else
	{
		for (U32 i = numExistingScratchTextures; i < numScratchTextures; i++)
		{
			mOpacityScratchTextures.increment();
			mOpacityScratchTextures.back().Set( mClipMapSize, mClipMapSize, GFXFormatR8G8B8A8, &ClipMapTextureProfile, avar("%s() - mOpacityScratchTextures (line %d)", __FUNCTION__, __LINE__), 1);
		}
	}
}
...
void ClipMapBlenderCache::setupGeometry( const RectF &srcRect0, const RectF &srcRect1, const RectF &masterCoords, const RectI &dstRect )
{
...
void ClipMapBlenderCache::setupGeometryFF( const RectF &srcRect0, const RectF &masterCoords, const F32 scaleFactor, const RectI &dstRect )
{
