...
namespace VCloud { 
	...
	void DataManager::remove()
	{
		...
		for (int k = 0; k < 2; k++)
		{
			//TextureManager::getSingleton().remove(mVolTextures[k]->getName());
	...
	void DataManager::update(const float &timeSinceLastFrame)
	{
		if (mVolTexToUpdate)
		{
	...
	void DataManager::create(const int &nx, const int &ny, const int &nz)
	{
		...
		mNoiseTex = GetRenderer()->addTexture("Textures/Noise.png", true);
	...
	void DataManager::setWheater(const float& Humidity, const float& AverageCloudsSize, const bool& delayedResponse)
	{
		...
		noVec3 maxcloudsize = AverageCloudsSize*noVec3(mNx/14, mNy/14, static_cast<int>(static_cast<float>(mNz)/2.75));

		// Update old clouds with new parameters
		noVec3 currentdimensions, currentPosition;
		...
		for(mEllipsoidsIt = mEllipsoids.begin(); mEllipsoidsIt != mEllipsoids.end(); mEllipsoidsIt++)
		{
			...
			{
				currentdimensions.x = maxcloudsize.x + randfloat(-0.2,0.2)*maxcloudsize.x;
			}
			if (currentdimensions.y / maxcloudsize.y < 0.5 || currentdimensions.y / maxcloudsize.y > 2)
			{
				currentdimensions.y = maxcloudsize.y + randfloat(-0.2,0.2)*maxcloudsize.y;
			}
			if (currentdimensions.z / maxcloudsize.z < 0.5 || currentdimensions.z / maxcloudsize.z > 2)
			{
				currentdimensions.z = maxcloudsize.z + randfloat(-0.2,0.15)*maxcloudsize.z;
			}
			...
			(*mEllipsoidsIt)->setPosition(noVec3(currentPosition.x,currentPosition.y,static_cast<int>(randfloat(currentdimensions.z+2,mNz-currentdimensions.z-2))));
		}
		...
		noVec3 newclouddimensions;
		while (static_cast<unsigned int>(numberofclouds) > mEllipsoids.size())
		{
			newclouddimensions = maxcloudsize.Multiply(noVec3(randfloat(0.5, 2), randfloat(0.5, 2), randfloat(0.75, 1)));
			addEllipsoid(new Ellipsoid(newclouddimensions.x,  newclouddimensions.y,  newclouddimensions.z, mNx, mNy, mNz, (int)randfloat(0, mNx), (int)randfloat(0, mNy), static_cast<int>(randfloat(newclouddimensions.z+2,mNz-newclouddimensions.z-2)), randfloat(1,5.0f)), false);
		}
	...
	const float DataManager::_getLightAbsorcionAt(Cell*** c, const int& nx, const int& ny, const int& nz, const int& x, const int& y, const int& z, const noVec3& d, const float& att) const
	{
		float step = 1, factor = 1;
		noVec3 pos = noVec3(x, y, z);
		...
		return noMath::ClampFloat(0,1, factor);
	}
	...
	void DataManager::_performCalculations(const int& nx, const int& ny, const int& nz, const int& step, const int& xStart, const int& xEnd)
	{
		...
		switch (step)
		{
			...
			{
				// Light scattering
				noVec3 SunDir = noVec3(mVClouds->getSunDirection().x, mVClouds->getSunDirection().z, mVClouds->getSunDirection().y);
	...
	{		
		...
		return noMath::ClampFloat(0, 1, strength*((float)clouds)/div);
	}
	...
	{
		mVolTex3D[TexId] = new D3D11Texture3D;
		mVolTex3D[TexId]->Initialize(D3D11Dev(), nx, ny, nz, DXGI_FORMAT_B8G8R8X8_UNORM_SRGB);
		
		ID3D11Texture3D* tex = mVolTex3D[TexId]->Texture;
		Direct3D11Renderer* renderer = (Direct3D11Renderer*)GetApp()->getRenderer();
		mVolTextures[static_cast<int>(TexId)] =renderer->addTexture(tex);
				
		/*= TextureManager::getSingleton().
				createManual("_SkyX_VolCloudsData"+std::stringConverter::toString(TexId),
				SKYX_RESOURCE_GROUP, 
				TEX_TYPE_3D,
				nx, ny, nz, 0,
				PF_BYTE_RGB);
*/
	/*	static_cast<MaterialPtr>(
			MaterialManager::getSingleton().getByName("SkyX_VolClouds"))
			->getTechnique(0)->getPass(0)->getTextureUnitState(static_cast<int>(TexId))
				->setTextureName("_SkyX_VolCloudsData"+std::stringConverter::toString(TexId), TEX_TYPE_3D);
		static_cast<MaterialPtr>(
			MaterialManager::getSingleton().getByName("SkyX_VolClouds_Lightning"))
			->getTechnique(0)->getPass(0)->getTextureUnitState(static_cast<int>(TexId))
				->setTextureName("_SkyX_VolCloudsData"+std::stringConverter::toString(TexId), TEX_TYPE_3D);
	}
*/
	...
	void DataManager::_updateVolTextureData(Cell ***c, const VolTextureId& TexId, const int& nx, const int& ny, const int& nz)
	{
		...
		PixelBox pb(mVolTex3D[TexId]->Width, mVolTex3D[TexId]->Height, mVolTex3D[TexId]->Depth, PF_X8R8G8B8);
		pb.data = mVolTex3D[TexId]->Map(D3D11Context(), 0, pitch);
		...
		for (z=pb.front; z<pb.back; z++) 
        {
            for (y=pb.top; y<pb.bottom; y++)
            {
				...
				{
					PixelUtil::packColour(c[x][y][z].dens/* TODO!!!! */, c[x][y][z].light, 0, 0, pb.format, &pbptr[x]);
                } 
                pbptr += pb.rowPitch;
            }
		...
		mVolTex3D[TexId]->Unmap(D3D11Context(), 0);
		//buffer->unlock();
	}
}