	...
	public:
		...
		struct Options 
		{
			...
			Options()
				...
				, SunIntensity(30)
				, WaveLength(noVec3(0.57f, 0.54f, 0.44f))
				, G(-0.991f)
				, Exposure(2.0f)
				, NumberOfSamples(4)
			{
			 ...
			 */
			Options(const float&	 _InnerRadius,
				    const float&	 _OuterRadius,
					const float&	 _HeightPosition,
					const float&	 _RayleighMultiplier,
					const float&	 _MieMultiplier,
					const float&    _SunIntensity,
					const noVec3& _WaveLength,
					const float&    _G,
		 ...
		 */
		AtmosphereManager(VCloudSkySystem *sys);
		 ...
		 */
		const noVec3 getColorAt(const noVec3& Direction) const;
