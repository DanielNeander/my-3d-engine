...
{
  m_vSource = noVec3(0,20,-110);
  m_vTarget = noVec3(0,20,-109);
  m_fNearMin = 1.0f;
  m_fFarMax = 400.0f;
  m_fNear = 1.0f;
  m_fFar = 400.0f;
...
void ShadowCamera::CalculateSplitPositions(float *pDistances)
{
...
Frustum ShadowCamera::CalculateFrustum(float fNear, float fFar)
{
  ...
  noVec3 vNearPlaneCenter = m_vSource + vZ * fNear;
  noVec3 vFarPlaneCenter = m_vSource + vZ * fFar;
