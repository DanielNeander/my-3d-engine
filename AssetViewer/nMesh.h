...
//------------------------------------------------------------------------------
class nMesh2 : public U2RefObject
{
public:
	...
	struct Edge
	{
		uint16 fIndex[2];  // face indices - the 2nd face index could be = InvalidIndex when the edge is a geometry border
		uint16 vIndex[2];  // vertex indices
	};
	...
	virtual bool OptimizeMesh(OptimizationFlag flags, float * vertices, int numVertices, uint16 * indices, int numIndices);
...
protected:
	...
	void UpdateGroupBoundingBoxes(float* vertexBufferData, uint16* indexBufferData);
...
nMesh2::GetVertexComponentOffset(VertexComponent component) const
{
	...
	FDebug("Requested component('%i') was not found!\n", (int) component);
	return -1;
}
...
{
	FDebug("nMesh2: pure virtual function CreateVertexBuffer() called!\n");
}
...
{
	FDebug("nMesh2: pure virtual function CreateIndexBuffer() called!\n");
}
