...
*/
bool U2WinFile::Exists(const U2DynString& fileName) const 
{
	HANDLE fh = CreateFile(fileName.Str(),       // filename
...
*/
bool U2WinFile::Open(const U2DynString &fileName, const U2DynString &accessMode)
{
	U2ASSERT(!IsOpen());
	lineNumber = 0;
	...
	DWORD disposition = 0;
	DWORD shareMode = 0;
	if (accessMode.Contains(_T("rR")))
	{
		access |= GENERIC_READ;
	}
	if (accessMode.Contains(_T("wW")))
	{
	...
	this->handle = CreateFile(fileName,    // filename
		access,               // access mode
...
*/
bool U2WinFile::PutS(const U2String& buffer)
{
	U2ASSERT(this->IsOpen());
...
int U2WinFile::AppendFile(U2WinFile* other)
{
	...
	char* buffer = U2_ALLOC(char, numBytes);
	U2ASSERT(buffer);
	int numBytesRead = other->Read(buffer, numBytes);
	U2ASSERT(numBytesRead == numBytes);

	// write to this file
	int numBytesWritten = this->Write(buffer, numBytes);
	U2ASSERT(numBytesWritten == numBytes);
