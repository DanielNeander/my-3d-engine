...
public:
	...
	void LoadAndFindDecl( const char* filename, declType_t defaultType);
...
void idDeclManagerLocal::LoadAndFindDecl( const char* filename, declType_t defaultType )
{
	idDeclFile* df = new idDeclFile( filename, defaultType );
	loadedFiles.Append( df );
	df->LoadAndParse();	
}
	...
	*/
	float idDeclTable::TableLookup( float index ) const {
		...
		if ( clamp ) {
			...
			iIndex = noMath::Ftoi( index );
			iFrac = index - iIndex;
		} else {
			index *= domain;

			if ( index < 0 ) {
				index += domain * noMath::Ceil( -index / domain );
			}

			iIndex = noMath::FtoiFast( noMath::Floor( index ) );
			iFrac = index - iIndex;
