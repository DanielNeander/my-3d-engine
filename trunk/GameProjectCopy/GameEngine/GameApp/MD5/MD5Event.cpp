...
idEvent *idEvent::Alloc( const idEventDef *evdef, int numargs, va_list args ) {
	...
	for( i = 0; i < numargs; i++ ) {
		...
		case D_EVENT_VECTOR :
			if ( arg->value ) {
				*reinterpret_cast<noVec3 *>( dataPtr ) = *reinterpret_cast<const noVec3 *>( arg->value );
			}
			break;
...
void idEvent::Schedule( idClass *obj, const idTypeInfo *type, int time ) {
	...
	this->time = GetApp()->globalTime + time;
...
void idEvent::ServiceEvents( void ) {
	...
	while( !EventQueue.IsListEmpty() ) {
		...
		if ( event->time > GetApp()->globalTime ) {
			break;
		}
		...
		for( i = 0; i < numargs; i++ ) {
			...
			case D_EVENT_VECTOR :
				*reinterpret_cast<noVec3 **>( &args[ i ] ) = reinterpret_cast<noVec3 *>( &data[ offset ] );
				break;
...
//					break;
//				case D_EVENT_VECTOR :
//					savefile->WriteVec3( *reinterpret_cast<noVec3 *>( dataPtr ) );
//					size += sizeof( noVec3 );
//					break;
//				case D_EVENT_TRACE :
...
//						break;
//					case D_EVENT_VECTOR :
//						savefile->ReadVec3( *reinterpret_cast<noVec3 *>( dataPtr ) );
//						size += sizeof( noVec3 );
//						break;
//					case D_EVENT_TRACE :
