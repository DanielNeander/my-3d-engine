...
public:
	...
	void					Init( class idActor *owner, idAnimator *_animator, int animchannel );
	void					Shutdown( void );
...
public:
	...
	noMat3					viewAxis;			// view axis of the actor
...
public:
	...
	noVec3					EyeOffset( void ) const;
	noVec3					GetEyePosition( void ) const;
	virtual void			GetViewPos( noVec3 &origin, noMat3 &axis ) const;
	void					SetFOV( float fov );
	bool					CheckFOV( const noVec3 &pos ) const;
	bool					CanSee( idEntity *ent, bool useFOV ) const;
	
	const noAngles &		GetDeltaViewAngles( void ) const	{ return deltaViewAngles; }
	void					SetDeltaViewAngles( const noAngles &delta ) { deltaViewAngles = delta; }
	...
	int GetAnim( int channel, const char *animname );
	...
	virtual void			SpawnGibs( const noVec3 &dir, const char *damageDefName );
	...
	void Event_SetAnimPrefix( const char *prefix );
	idStr Event_GetPainAnim( void );
	int Event_GetBlendFrames( int channel );
	...
	bool Event_IdleAnim( int channel, const char *animname );
	bool Event_PlayCycle( int channel, const char *animname );
...
protected:
	...
	noVec3					eyeOffset;			// offset of eye relative to physics origin
	noVec3					modelOffset;		// offset of visual model relative to the physics origin

	noAngles				deltaViewAngles;	// delta angles relative to view input angles
	...
	virtual void			Gib( const noVec3 &dir, const char *damageDefName );
