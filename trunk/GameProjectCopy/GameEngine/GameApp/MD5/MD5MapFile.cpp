...
=================
*/
static void ComputeAxisBase( const noVec3 &normal, noVec3 &texS, noVec3 &texT ) {
	float RotY, RotZ;
	noVec3 n;

	// do some cleaning
	n[0] = ( noMath::Fabs( normal[0] ) < 1e-6f ) ? 0.0f : normal[0];
	n[1] = ( noMath::Fabs( normal[1] ) < 1e-6f ) ? 0.0f : normal[1];
	n[2] = ( noMath::Fabs( normal[2] ) < 1e-6f ) ? 0.0f : normal[2];

	RotY = -atan2( n[2], noMath::Sqrt( n[1] * n[1] + n[0] * n[0]) );
	RotZ = atan2( n[1], n[0] );
...
=================
*/
void idMapBrushSide::GetTextureVectors( noVec4 v[2] ) const {
	int i;
	noVec3 texX, texY;
...
=================
*/
idMapPatch *idMapPatch::Parse( idLexer &src, const noVec3 &origin, bool patchDef3, float version ) {
	float		info[7];
	idDrawVert *vert;
	idToken		token;
	int			i, j;
...
============
*/
bool idMapPatch::Write( idFile *fp, int primitiveNum, const noVec3 &origin ) const {
	int i, j;
	const idDrawVert *v;
...
=================
*/
idMapBrush *idMapBrush::Parse( idLexer &src, const noVec3 &origin, bool newFormat, float version ) {
	int i;
	noVec3 planepts[3];
	idToken token;
...
=================
*/
idMapBrush *idMapBrush::ParseQ3( idLexer &src, const noVec3 &origin ) {
	...
	do {
		...
		side->texMat[0] = noVec3( 0.03125f, 0.0f, 0.0f );
		side->texMat[1] = noVec3( 0.0f, 0.03125f, 0.0f );
		side->origin = origin;
...
============
*/
bool idMapBrush::Write( idFile *fp, int primitiveNum, const noVec3 &origin ) const {
	int i;
	idMapBrushSide *side;
