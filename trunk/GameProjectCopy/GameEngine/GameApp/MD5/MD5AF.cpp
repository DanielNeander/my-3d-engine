...
bool idAF::UpdateAnimation( void ) {
	int i;
	noVec3 origin, renderOrigin, bodyOrigin;
	noMat3 axis, renderAxis, bodyAxis;
...
idBounds idAF::GetBounds( void ) const {
	int i;
	idAFBody *body;
	noVec3 origin, entityOrigin;
	noMat3 axis, entityAxis;
	idBounds bounds, b;
...
================
*/
void idAF::GetPhysicsToVisualTransform( noVec3 &origin, noMat3 &axis ) const {
	origin = - baseOrigin;
...
================
*/
void idAF::GetImpactInfo( idEntity *ent, int id, const noVec3 &point, impactInfo_t *info ) {
...
================
*/
void idAF::ApplyImpulse( idEntity *ent, int id, const noVec3 &point, const noVec3 &impulse ) {
	SetupPose( self, gameLocal.time );
...
================
*/
void idAF::AddForce( idEntity *ent, int id, const noVec3 &point, const noVec3 &force ) {
	SetupPose( self, gameLocal.time );
...
bool idAF::LoadBody( const idDeclAF_Body *fb, const idJointMat *joints ) {
	...
	idClipModel *clip;
	idAFBody *body;
	noMat3 axis, inertiaTensor;
	noVec3 centerOfMass, origin;
	idBounds bounds;
...
bool idAF::LoadConstraint( const idDeclAF_Constraint *fc ) {
	...
	switch( fc->type ) {
		...
		case DECLAF_CONSTRAINT_HINGE: {
			...
			switch( fc->limit ) {
				case idDeclAF_Constraint::LIMIT_CONE: {
					noVec3 left, up, axis, shaft;
					fc->axis.ToVec3().OrthogonalBasis( left, up );
					axis = left * noRotation( vec3_origin, fc->axis.ToVec3(), fc->limitAngles[0] );
					shaft = left * noRotation( vec3_origin, fc->axis.ToVec3(), fc->limitAngles[2] );
...
================
*/
static bool GetJointTransform( void *model, const idJointMat *frame, const char *jointName, noVec3 &origin, noMat3 &axis ) {
	jointHandle_t	joint;
...
bool idAF::Load( idEntity *ent, const char *fileName ) {
	...
	::ANIM_CreateAnimFrame( model, animator->GetAnim( modifiedAnim )->MD5Anim( 0 ), numJoints, joints, 1, animator->ModelDef()->GetVisualOffset(), animator->RemoveOrigin() );
...
bool idAF::TestSolid( void ) const {
	...
	for ( i = 0; i < physicsObj.GetNumBodies(); i++ ) {
		...
		if ( gameLocal.clip.Translation( trace, body->GetWorldOrigin(), body->GetWorldOrigin(), body->GetClipModel(), body->GetWorldAxis(), body->GetClipMask(), self ) ) {
			float depth = noMath::Fabs( trace.c.point * trace.c.normal - trace.c.dist );
...
================
*/
void idAF::SetConstraintPosition( const char *name, const noVec3 &pos ) {
...
void idAF::AddBindConstraints( void ) {
	...
	noVec3 origin, renderOrigin;
	noMat3 axis, renderAxis;
	...
	for ( kv = args.MatchPrefix( "bindConstraint ", NULL ); kv; kv = args.MatchPrefix( "bindConstraint ", kv ) ) {
		...
		if ( type.Icmp( "fixed" ) == 0 ) {
			...
			animator->GetJointTransform( joint, gameLocal.time, origin, axis );
			c->SetAnchor( renderOrigin + origin * renderAxis );
			c->SetShafts( noVec3( 0, 0, 1 ), noVec3( 0, 0, -1 ) );
		}
		else {
