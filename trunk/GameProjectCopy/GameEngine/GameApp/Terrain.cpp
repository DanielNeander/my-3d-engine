...
static bool g_FrustumCullInHS			= true;

int CTerrain::gp_wrap( int a)
{
...
CTerrain::CTerrain() {
	m_pOwnPatch = NULL;
	initialized = false;
	terrain_gridpoints = 512;
	terrain_geometry_scale = 10.0f;
}

void CTerrain::CreateSkyBox() {
	CreateSkyInput();
	CreateSkyVertexBuffer();
}

void CTerrain::CreateSkyInput() {
...
void CTerrain::InitializeWater(ID3D11Device* device, ID3DX11Effect * effect)
{
	pEffect = effect;
	pDevice = device;
	...
	D3DX11_PASS_DESC passDesc;
	effect->GetTechniqueByName("RenderWater")->GetPassByIndex(0)->GetDesc(&passDesc);
...
void CTerrain::RecreateWaterBuffers() {
...
void CTerrain::CreateWater() {
	...
	for(int i=0;i<terrain_numpatches_1d;i++)
		...
		loadTextureFromFile("Terrain/water_heightMap.dds", "g_HeightfieldTexture", D3D11Dev(), pEffect, &heightmap_textureSRV);

		// DXUTFindDXSDKMediaFileCch(path, MAX_PATH, L"TerrainTextures/water_bump.dds");
		D3DX11GetImageInfoFromFile("TerrainTextures/water_bump.dds", NULL, &imageInfo, NULL);
		...
		D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/water_bump.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&water_bump_texture, NULL);
...
void CTerrain::CreateTerrain()
{
	...
	height.resize(terrain_gridpoints+1);
	for (i=0;i<terrain_gridpoints+1;i++)
		height[i].resize(terrain_gridpoints+1);

	normal.resize(terrain_gridpoints+1);
	for (i=0;i<terrain_gridpoints+1;i++)
		normal[i].resize(terrain_gridpoints+1);

	tangent.resize(terrain_gridpoints+1);
	for (i=0;i<terrain_gridpoints+1;i++)
		tangent[i].resize(terrain_gridpoints+1);

	binormal.resize(terrain_gridpoints+1);
	for (i=0;i<terrain_gridpoints+1;i++)
		binormal[i].resize(terrain_gridpoints+1);
...
void CTerrain::CreateSkyVertexBuffer()  {
...
void CTerrain::LoadTextures()
{
	...
	//FindMediaFileCch(path, MAX_PATH, L"TerrainTextures/rock_bump6.dds");
	D3DX11GetImageInfoFromFile("TerrainTextures/rock_bump6.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/rock_bump6.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&rock_bump_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/terrain_rock4.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/terrain_rock4.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&rock_diffuse_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/sand_diffuse.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/sand_diffuse.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&sand_diffuse_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/rock_bump4.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/rock_bump4.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&sand_bump_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/terrain_grass.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/terrain_grass.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&grass_diffuse_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/terrain_slope.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/terrain_slope.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&slope_diffuse_texture, NULL);
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/lichen1_normal.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/lichen1_normal.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&sand_microbump_texture, NULL);
	...
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/rock_bump4.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&rock_microbump_texture, NULL);
...
void CTerrain::LoadSkyTexture() {
	...
	D3DX11GetImageInfoFromFile("TerrainTextures/sky.dds", NULL, &imageInfo, NULL);
	memset(&imageLoadInfo, 0, sizeof(imageLoadInfo));
	imageLoadInfo.Width = imageInfo.Width;
	imageLoadInfo.Height = imageInfo.Height;
	imageLoadInfo.MipLevels = imageInfo.MipLevels;
	imageLoadInfo.Format = imageInfo.Format;
	imageLoadInfo.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	D3DX11CreateTextureFromFile(pDevice, "TerrainTextures/sky.dds", &imageLoadInfo, NULL, (ID3D11Resource**)&sky_texture, NULL);
...
void CTerrain::RenderPass_Reflection() {	
...
void CTerrain::CommitShaderConstants() {
	float origin[2]={0,0};
	D3DXVECTOR2 WaterTexcoordShift(GetApp()->GetSecTime()*1.5f,GetApp()->GetSecTime()*0.75f);
	D3DXVECTOR2 ScreenSizeInv(1.0f/(BackbufferWidth*main_buffer_size_multiplier),1.0f/(BackbufferHeight*main_buffer_size_multiplier));
	...
	pEffect->GetVariableByName("g_ZNear")->AsScalar()->SetFloat(GetCamera()->GetNear());
	pEffect->GetVariableByName("g_ZFar")->AsScalar()->SetFloat(GetCamera()->GetFar());
	pEffect->GetVariableByName("g_LightPosition")->AsVector()->SetFloatVector(gSun.m_vLitPos.ToFloatPtr());
	pEffect->GetVariableByName("g_WaterBumpTexcoordShift")->AsVector()->SetFloatVector(WaterTexcoordShift);
	pEffect->GetVariableByName("g_ScreenSizeInv")->AsVector()->SetFloatVector(ScreenSizeInv);

	pEffect->GetVariableByName("g_DynamicTessFactor")->AsScalar()->SetFloat(g_DynamicTessellationFactor);
	pEffect->GetVariableByName("g_StaticTessFactor")->AsScalar()->SetFloat(g_StaticTessellationFactor);
	pEffect->GetVariableByName("g_UseDynamicLOD")->AsScalar()->SetFloat(g_UseDynamicLOD?1.0f:0.0f);
	pEffect->GetVariableByName("g_RenderCaustics")->AsScalar()->SetFloat(g_RenderCaustics?1.0f:0.0f);
	pEffect->GetVariableByName("g_FrustumCullInHS")->AsScalar()->SetFloat(g_FrustumCullInHS?1.0f:0.0f);
}


void CTerrain::RenderPass_Caustics()
{	
	if (!initialized) return;

	if(!g_RenderCaustics)  return;
	...
	pContext->IASetVertexBuffers(0,1,null,&stride,&offset);
	SAFE_RELEASE(pContext);	
	
}


void CTerrain::ResetConstantBuffers() {

	
}

void CTerrain::RenderWater(BaseCamera *cam) {
	if (!initialized) return;
	...
	//ID3D11RenderTargetView *colorBuffer = renderer->getBackBuffer();
	//ID3D11DepthStencilView  *backBuffer = renderer->getDepthBuffer();	
	...
	pContext->ResolveSubresource(refraction_color_resource,0,GetApp()->main_color_resource,0,DXGI_FORMAT_R8G8B8A8_UNORM);
	...
	case 1:
		...
		tex_variable->SetResource(GetApp()->main_depth_resourceSRV);
...
void CTerrain::Render(BaseCamera *cam)
{
	ID3D11DeviceContext* pContext;
	ID3DX11EffectShaderResourceVariable* tex_variable;
	float origin[2]={0,0};
...
void CTerrain::SetupReflectionView(BaseCamera *cam)
{
	...
	D3DXMatrixPerspectiveFovLH(&mProj,camera_fov * PI / 360.0f,aspectRatio, GetCamera()->GetNear(), GetCamera()->GetFar());
	ReflectViewProj=mView*mProj;
	D3DXMatrixInverse(&mViewProjInv, NULL, &ReflectViewProj);

	pEffect->GetVariableByName("g_mWorldViewProj")->AsMatrix()->SetMatrix(ReflectViewProj);
	pEffect->GetVariableByName("g_CameraPosition")->AsVector()->SetFloatVector(EyePoint);
	ReflectCamPos = EyePoint;
	...
	ReflectCamDir = normalized_direction;
...
void CTerrain::SetupRefractionView(BaseCamera *cam)
{
	pEffect->GetVariableByName("g_HalfSpaceCullSign")->AsScalar()->SetFloat(-1.0f);
	pEffect->GetVariableByName("g_HalfSpaceCullPosition")->AsScalar()->SetFloat(terrain_minheight);
}
void CTerrain::SetupLightView(BaseCamera *cam)
{
...
void CTerrain::SetupNormalView(BaseCamera *cam)
{
	if (!initialized) return;
	...
	pEffect->GetVariableByName("View")->AsMatrix()->SetMatrix(mView);
	pEffect->GetVariableByName("g_mWorldViewProj")->AsMatrix()->SetMatrix(mViewProj);
	...
	pEffect->GetVariableByName("g_WaterNormalMapTexture")->AsShaderResource()->SetResource(water_normalmap_resourceSRV);	
}
