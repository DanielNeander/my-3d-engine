/*$T navigationmesh.cpp GC 1.140 01/03/12 01:39:30 */
...
/**
 *  \ NavigationMesh.cpp NavigationMesh component implementation for the Navimesh sample program. Included as part of
...
/*
 ...
 =======================================================================================================================
 */
noVec3 NavigationMesh::SnapPointToCell(NavigationCell *Cell, const noVec3 &Point)
{
	/*~~~~~~~~~~~~~~~~~~~~~*/
	noVec3 PointOut = Point;
	/*~~~~~~~~~~~~~~~~~~~~~*/
...
/*
 ...
 =======================================================================================================================
 */
noVec3 NavigationMesh::SnapPointToMesh(NavigationCell **CellOut, const noVec3 &Point)
{
	noVec3 PointOut = Point;
...
/*
 ...
 =======================================================================================================================
 */
NavigationCell *NavigationMesh::FindClosestCell(const noVec3 &Point) const
{
	...
	for(; CellIter != m_CellArray.end(); ++CellIter)
	{
		...
		if(!FoundHomeCell)
		{
			noVec2						Start(pCell->CenterPoint().x, pCell->CenterPoint().z);
			noVec2						End(Point.x, Point.z);
			Line2D						MotionPath(Start, End);
			NavigationCell				*NextCell;
			NavigationCell::CELL_SIDE	WallHit;
			noVec2						PointOfIntersection;

			NavigationCell::PATH_RESULT Result = pCell->ClassifyPathToCell
				(
					MotionPath,
					&NextCell,
					WallHit,
			...
			{
				noVec3 ClosestPoint3D(PointOfIntersection.x, 0.0f, PointOfIntersection.y);
...
)
{
	...
	/*
	 * create a 2D motion path from our Start and End positions, tossing out their Y
	 * values to project them ;
	 * down to the XZ plane.
	 */
	Line2D						MotionPath(noVec2(StartPos.x, StartPos.z), noVec2(EndPos.x, EndPos.z));
	...
	while((Result != NavigationCell::ENDING_CELL) && (MotionPath.EndPointA() != MotionPath.EndPointB()))
	{
		...
		if(Result == NavigationCell::EXITING_CELL)
		{
			...
			else
			{
				 ...
				 */
				noVec2 Direction = MotionPath.EndPointB() - MotionPath.EndPointA();
				Direction *= 0.9f;
		...
		else if(Result == NavigationCell::NO_RELATIONSHIP)
		{
			 ...
			 */
			noVec2 NewOrigin = MotionPath.EndPointA();
	...
	while(IterA != m_CellArray.end())
	{
		...
		while(IterB != m_CellArray.end())
		{
			...
			if(IterA != IterB)
			{
				if
				(
					!pCellA->Link(NavigationCell::SIDE_AB)
				&&	pCellB->RequestLink(pCellA->Vertex(0), pCellA->Vertex(1), pCellA)
				)
				{
					...
					(
						!pCellA->Link(NavigationCell::SIDE_BC)
					&&	pCellB->RequestLink(pCellA->Vertex(1), pCellA->Vertex(2), pCellA)
					)
				{
					...
					(
						!pCellA->Link(NavigationCell::SIDE_CA)
					&&	pCellB->RequestLink(pCellA->Vertex(2), pCellA->Vertex(0), pCellA)
					)
				{
