...
class PrimData
{
	// struct _BoundingBox { noVec3 p1; noVec3 p2; };
public:
	PrimData();
	...
	bool RebuildAndClip( BoundingBox& a_Box );
	void UpdateBBox();
	// data members
	static noVec3* s_TVert;
	int m_Verts;
	Primitive* m_Prim;
	noVec3* m_Vertex;
	BoundingBox bbox;
...
public:
	...
	void InitBuild( const std::list<Primitive*>& Prim);
	void Build( const std::list<Primitive*>& Prim );
	...
	void BuildTree( ObjectList* a_List, KdTreeNode* a_Node, BoundingBox& a_Box, int a_Depth, int a_Prims );
	void SubdivNewONlogN( KdTreeNode* a_Node, BoundingBox& a_Box, int a_Depth, int a_Prims );
	KdTreeNode* m_Root;
	Primitive** m_ObjList;
