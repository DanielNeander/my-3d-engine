...
struct MeshSizeSort
{
  ...
  {
    noVec3 A = a->m_OOBB.GetSize();
    A.y = 0;
    noVec3 B = b->m_OOBB.GetSize();
    B.y = 0;
...
void CreateScene(void)
{
  ...
  for(int i = 0; i < g_iNumSceneObjects; i++)
  {
    ...
    {
      pObject->m_mWorld.SetRotation(noVec3(DegreeToRadian((rand()%4)*90), 0, 0));
      pObject->CalculateAABB();

      // position is quantized by size of mesh
      noVec3 vSize = pObject->m_AABB.GetSize();
      ...
      pObject->m_mWorld.SetTranslation(noVec3(fPosX, 0, fPosZ));
      pObject->CalculateAABB();
  ...
  pFloor->m_mWorld.Scale(noVec3(1.0f, 1.0f, 1.0f) * (g_fSceneArea + 8.0f));
  ...
  // set camera
  g_ShadowCamera.m_fFarMax = fMaxLength + 300.0f;
}
