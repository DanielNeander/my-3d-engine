...
public:
	...
	virtual bool Init( idEntity *self, const char *anim, const noVec3 &modelOffset );
	virtual void			Evaluate( void );
	virtual void			ClearJointMods( void );

	bool					SolveTwoBones( const noVec3 &startPos, const noVec3 &endPos, const noVec3 &dir, float len0, float len1, noVec3 &jointPos );
	float					GetBoneAxis( const noVec3 &startPos, const noVec3 &endPos, const noVec3 &dir, noMat3 &axis );

	class idAnimator *		animator;			// animator on entity
protected:
	bool					initialized;
	bool					ik_activate;
	idEntity*				self;
...
private:
	...
	noVec3					hipForward[MAX_LEGS];
	noVec3					kneeForward[MAX_LEGS];

	float					upperLegLength[MAX_LEGS];
	float					lowerLegLength[MAX_LEGS];

	noMat3					upperLegToHipJoint[MAX_LEGS];
	noMat3					lowerLegToKneeJoint[MAX_LEGS];
...
private:
	...
	noVec3					shoulderForward[MAX_ARMS];
	noVec3					elbowForward[MAX_ARMS];

	float					upperArmLength[MAX_ARMS];
	float					lowerArmLength[MAX_ARMS];

	noMat3					upperArmToShoulderJoint[MAX_ARMS];
	noMat3					lowerArmToElbowJoint[MAX_ARMS];
};
