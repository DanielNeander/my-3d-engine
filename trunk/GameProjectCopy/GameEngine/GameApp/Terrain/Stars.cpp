...
extern HRESULT loadTextureFromFile(LPCSTR file, LPCSTR shaderTextureName, ID3D11Device* pd3dDevice, ID3DX11Effect* pEffect);
...
StarData::StarData(const char* line): gLon(0), gLat(0), magnitude(0), BminusV(0)
{
	...
	gLon      = (StrToFloat(line,  91, 96) / 180.0f) * noMath::PI;
	gLat      = (StrToFloat(line,  97,102) / 180.0f) * noMath::PI;
...
HRESULT CreateStarEffect(ID3D11Device* pd3dDevice)
{
    ...
    CHAR path[MAX_PATH] = "Data\\Shaders\\TerrainTessellation\\Stars.fx";
    ID3DXBuffer* pShader = NULL;
    /*if (FAILED (hr = DXUTFindDXSDKMediaFileCch(path, MAX_PATH, "Stars.fx"))) {
        MessageBox (NULL, "Stars.fx cannot be found.", "Error", MB_OK);
        return hr;
    }*/
	...
	loadTextureFromFile("Data/TerrainTessellation/StarColours.jpg", "g_StarColours", pd3dDevice, g_pStarEffect);
	loadTextureFromFile("Data/TerrainTessellation/StarShape1.dds",  "g_StarShape",   pd3dDevice, g_pStarEffect);
	loadTextureFromFile("Data/TerrainTessellation/fBm5Octaves.dds", "g_fBmTexture",  pd3dDevice, g_pStarEffect);
...
void ReadStars()
{
	const CHAR* file = "Data\\TerrainTessellation\\BrightStarCatalog.txt";
...
D3DXMATRIX StarWorldMatrix(float starRotation = 0)
{
	...
	D3DXMatrixRotationY(&mWorld1, -123.3f / 180.0f * noMath::PI);
	D3DXMatrixRotationX(&mWorld2,   54.0f / 180.0f * noMath::PI);
	D3DXMatrixRotationY(&mWorld3, starRotation / 180.0f * noMath::PI);
