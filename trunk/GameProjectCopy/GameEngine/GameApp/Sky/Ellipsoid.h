...
namespace VCloud { 
	...
	public:
		...
		Ellipsoid(const int& a, const int& b, const int& c, 
				  ...
				  const float& DynLibManager = 1.0f);
		 ...
		 */
		const noVec3 getProbabilities(const int& x, const int& y, const int& z) const;
		 ...
		 */
		inline const noVec3 getDimensions() const
		{
			return noVec3(mA, mB, mC);
		}

		/** Get position
		    @return Position
		 */
		inline const noVec3 getPosition() const
		{
			return noVec3(mX, mY, mZ);
		}

		/** Set dimensions
		    @param Dimensions New dimensions
		 */
		void setDimensions(const noVec3& Dimensions);

		/** Set position
		    @param Position New position
		 */
		inline void setPosition(const noVec3& Position)
		{
			mX = Position.x;
			mY = Position.y;
			mZ = Position.z;
