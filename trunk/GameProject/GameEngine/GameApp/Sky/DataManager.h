...
namespace VCloud {
	...
	public:
		...
		/** Get update time
		    @return Update time
		 */
		inline const float& getUpdateTime() const
		{
			return mUpdateTime;
		}
		 ...
		 */
		inline const float _getInterpolation() const
		{
	...
	private:
		...
		*/
		 ...
		 */
		const float _getLightAbsorcionAt(Cell*** c, const int& nx, const int& ny, const int& nz, const int& x, const int& y, const int& z, const noVec3& d, const float& att) const;
