...
float Gauss()
{
	...
	if (u1 < 1e-6f)
		u1 = 1e-6f;
	return sqrtf(-2 * logf(u1)) * cosf(2*noMath::PI * u2);
}
...
OceanSimulator::OceanSimulator(OceanParameter& params, ID3D11Device* pd3dDevice)
{
    ...
    CompileShaderFromFile(L"Data/Shaders/ocean_simulator_cs.hlsl", "UpdateSpectrumCS", "cs_4_0", &pBlobUpdateSpectrumCS);
    ...
    CompileShaderFromFile(L"Data/Shaders/ocean_simulator_vs_ps.hlsl", "QuadVS", "vs_4_0", &pBlobQuadVS);
    CompileShaderFromFile(L"Data/Shaders/ocean_simulator_vs_ps.hlsl", "UpdateDisplacementPS", "ps_4_0", &pBlobUpdateDisplacementPS);
    CompileShaderFromFile(L"Data/Shaders/ocean_simulator_vs_ps.hlsl", "GenGradientFoldingPS", "ps_4_0", &pBlobGenGradientFoldingPS);
	assert(pBlobQuadVS);
...
void OceanSimulator::initHeightMap(OceanParameter& params, D3DXVECTOR2* out_h0, float* out_omega)
{
	...
	for (i = 0; i <= height_map_dim; i++)
	{
		...
		K.y = (-height_map_dim / 2.0f + i) * (2 * noMath::PI / patch_length);

		for (j = 0; j <= height_map_dim; j++)
		{
			K.x = (-height_map_dim / 2.0f + j) * (2 * noMath::PI / patch_length);
...
HRESULT CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
    ...
    // find the file
    WCHAR str[MAX_PATH];
    FindMediaFileCch( str, MAX_PATH, szFileName );
