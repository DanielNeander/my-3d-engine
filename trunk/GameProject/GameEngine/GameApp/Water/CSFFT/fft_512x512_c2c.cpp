...
void fft_512x512_c2c(CSFFT512x512_Plan* fft_plan,
					 ...
					 ID3D11ShaderResourceView* pSRV_Src)
{
	const UINT thread_count = fft_plan->slices * (64 * 64) / 8;
...
void create_cbuffers_512x512(CSFFT512x512_Plan* plan, ID3D11Device* pd3dDevice, UINT slices)
{
	...
	// Buffer 0
	const UINT thread_count = slices * (64 * 64) / 8;
	UINT ostride = 64 * 64 / 8;
	UINT istride = ostride;
	double phase_base = -TWO_PI / (64.0 * 64.0);
	
	CB_Structure cb_data_buf0 = {thread_count, ostride, istride, 64, (float)phase_base};
	...
	CB_Structure cb_data_buf1 = {thread_count, ostride, istride, 64, (float)phase_base};
	...
	CB_Structure cb_data_buf2 = {thread_count, ostride, istride, 64, (float)phase_base};
...
void fft512x512_create_plan(CSFFT512x512_Plan* plan, ID3D11Device* pd3dDevice, UINT slices)
{
    ...
    CompileShaderFromFile(L"Data/Shaders/fft_512x512_c2c.hlsl", "Radix008A_CS", "cs_4_0", &pBlobCS);
    CompileShaderFromFile(L"Data/Shaders/fft_512x512_c2c.hlsl", "Radix008A_CS2", "cs_4_0", &pBlobCS2);
	assert(pBlobCS);
	assert(pBlobCS2);
	...
	buf_desc.ByteWidth = sizeof(float) * 2 * (64 * slices) * 64;
	...
	uav_desc.Buffer.NumElements = (64 * slices) * 64;
	...
	srv_desc.Buffer.NumElements = (64 * slices) * 64;
