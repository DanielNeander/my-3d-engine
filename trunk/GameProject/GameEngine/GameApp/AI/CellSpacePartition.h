...
struct Cell
{
	...
	Cell(noVec3 topleft,
		noVec3 botright):BBox(InvertedAABBox2D(topleft, botright))
	{}
};
...
private:
	...
	inline int  PositionToIndex(const noVec3& pos)const;

public:

	CellSpacePartition(float width,        //width of the environment
		float height,       //height ...
	...
	inline void UpdateEntity(const entity& ent, noVec3 OldPos);
	...
	inline void CalculateNeighbors(noVec3 TargetPos, float QueryRadius);
	...
	void SetCellHeight(float minh, float maxh);

	//call this to use the gdi to render the cell edges
	inline void RenderCells(NavDebugDraw* dd, float offSetX = 0.0, float offSetY = 0.0)const;
};

template <class entity>
void CellSpacePartition<entity>::SetCellHeight(float minHeight, float maxHeight)
{
	std::vector<Cell<entity> >::iterator it = m_Cells.begin();

	for (it; it!=m_Cells.end(); ++it)
	{
		it->BBox.SetMinHeight(minHeight);
		it->BBox.SetMaxHeight(maxHeight);
	}
}
...
//--------------------------------------------------------------------------
template<class entity>
CellSpacePartition<entity>::CellSpacePartition(float  width,        //width of 2D space
	float  height,       //height...
	...
	m_OffsetY(offsetY)
{
	...
	for (int y=0; y<m_iNumCellsY; ++y)
	{
		for (int x=0; x<m_iNumCellsX; ++x)
		{
			...
			m_Cells.push_back(Cell<entity>(noVec3(left, 0.0f, top), noVec3(right, 0.0f, bot)));
		}
	}
}
...
//------------------------------------------------------------------------
template<class entity>
void CellSpacePartition<entity>::CalculateNeighbors(noVec3 TargetPos,
	float   QueryRadius)
{
	const noVec3 offSet(m_OffsetX, 0.0f, m_OffsetY);
	const noVec3 newPos = (TargetPos + offSet);
	...
	//area
	InvertedAABBox2D QueryBox(TargetPos - noVec3(QueryRadius, TargetPos.y,  QueryRadius), 
		TargetPos + noVec3(QueryRadius, TargetPos.y, QueryRadius));
	...
	for (curCell=m_Cells.begin(); curCell!=m_Cells.end(); ++curCell)
	{
			...
			!curCell->Members.empty())
		{
			...
			{     
				if (((*it)->Pos() +offSet- TargetPos + offSet).LengthSqr() < QueryRadius*QueryRadius)
				{
					*curNbor++ = *it;
				}
			}    
...
//------------------------------------------------------------------------
template<class entity>
inline int CellSpacePartition<entity>::PositionToIndex(const noVec3& pos)const
{
	const noVec3 offSet(m_OffsetX, 0.0f,  m_OffsetY);
	const noVec3 newPos = (pos + offSet);
	int idx = (int)(m_iNumCellsX * newPos.x / m_dSpaceWidth) + 
		((int)((m_iNumCellsY) * newPos.y / m_dSpaceHeight) * m_iNumCellsX);

	//if the entity's position is equal to noVec3(m_dSpaceWidth, m_dSpaceHeight)
...
//--------------------------------------------------------------------------
template<class entity>
inline void CellSpacePartition<entity>::RenderCells(NavDebugDraw* dd, float offSetX, float offSetY)const
{
	std::vector<Cell<entity> >::const_iterator curCell;
	for (curCell=m_Cells.begin(); curCell!=m_Cells.end(); ++curCell)
	{
		curCell->BBox.Render(dd, false, offSetX, offSetY);
	}
