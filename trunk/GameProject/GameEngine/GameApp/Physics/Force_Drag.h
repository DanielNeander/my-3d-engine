...
public:
	...
	void				SetPhysics( idPhysics *physics, int id, const noVec3 &p );
						// set position to drag towards
	void				SetDragPosition( const noVec3 &pos );
						// get the position dragged towards
	const noVec3 &		GetDragPosition( void ) const;
						// get the position on the dragged physics object
	const noVec3		GetDraggedPosition( void ) const;
...
private:
	...
	noVec3				p;				// position on clip model
	noVec3				dragPosition;	// drag towards this position
};
