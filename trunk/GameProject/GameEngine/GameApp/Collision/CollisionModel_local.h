...
typedef struct cm_windingList_s {
	...
	noVec3				normal;					// normal for all windings
	idBounds			bounds;					// bounds of all windings in list
	noVec3				origin;					// origin for radius
...
typedef struct cm_brush_s {
	...
	noPlane					planes[1];			// variable sized
} cm_brush_t;
...
typedef struct cm_trmVertex_s {
	...
	noVec3 p;										// vertex position
	noVec3 endp;									// end point of vertex after movement
	...
	noVec3 rotationOrigin;							// rotation origin for this vertex
...
typedef struct cm_trmEdge_s {
	...
	noVec3 start;									// start of edge
	noVec3 end;										// end of edge
	int vertexNum[2];								// indexes into cm_traceWork_t->vertices
	idPluecker pl;									// pluecker coordinate for edge
	noVec3 cross;									// (z,-y,x) of cross product between edge dir and movement dir
...
typedef struct cm_traceWork_s {
	...
	noVec3 start;									// start of trace
	noVec3 end;										// end of trace
	noVec3 dir;										// trace direction
	idBounds bounds;								// bounds of full trace
	idBounds size;									// bounds of transformed trm relative to start
	noVec3 extents;									// largest of abs(size[0]) and abs(size[1]) for BSP trace
	...
	noVec3 origin;									// origin of rotation in model space
	noVec3 axis;									// rotation axis in model space
	noMat3 matrix;									// rotates axis of rotation to the z-axis
	...
	noRotation modelVertexRotation;					// inverse rotation for model vertices
	...
	noPlane heartPlane1;							// polygons should be near anough the trace heart planes
	...
	noVec3 polygonRotationOriginCache[CM_MAX_POLYGON_EDGES];
} cm_traceWork_t;
...
public:
	...
	bool			GetModelVertex( cmHandle_t model, int vertexNum, noVec3 &vertex ) const;
	// get the edge of a model
	bool			GetModelEdge( cmHandle_t model, int edgeNum, noVec3 &start, noVec3 &end ) const;
	...
	void			Translation( trace_t *results, const noVec3 &start, const noVec3 &end,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	// rotates a trm and reports the first collision if any
	void			Rotation( trace_t *results, const noVec3 &start, const noRotation &rotation,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	// returns the contents the trm is stuck in or 0 if the trm is in free space
	int				Contents( const noVec3 &start,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	// stores all contact points of the trm with the model, returns the number of contacts
	int				Contacts( contactInfo_t *contacts, const int maxContacts, const noVec3 &start, const idVec6 &dir, const float depth,
								const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
								cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );
	// test collision detection
	void			DebugOutput( const noVec3 &origin );
	// draw a model
	void			DrawModel( cmHandle_t model, const noVec3 &origin, const noMat3 &axis,
											const noVec3 &viewOrigin, const float radius );
...
private:			// CollisionMap_translate.cpp
	int				TranslateEdgeThroughEdge( noVec3 &cross, idPluecker &l1, idPluecker &l2, float *fraction );
	...
	void			TranslateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly, cm_vertex_t *v, noVec3 &endp, idPluecker &pl );
...
private:			// CollisionMap_rotate.cpp
	int				CollisionBetweenEdgeBounds( cm_traceWork_t *tw, const noVec3 &va, const noVec3 &vb,
											const noVec3 &vc, const noVec3 &vd, float tanHalfAngle,
											noVec3 &collisionPoint, noVec3 &collisionNormal );
	int				RotateEdgeThroughEdge( cm_traceWork_t *tw, const idPluecker &pl1,
											const noVec3 &vc, const noVec3 &vd,
	...
	int				RotatePointThroughPlane( const cm_traceWork_t *tw, const noVec3 &point, const noPlane &plane,
											const float angle, const float minTan, float &tanHalfAngle );
	int				PointFurthestFromPlane( const cm_traceWork_t *tw, const noVec3 &point, const noPlane &plane,
											const float angle, float &tanHalfAngle, float &dir );
	int				RotatePointThroughEpsilonPlane( const cm_traceWork_t *tw, const noVec3 &point, const noVec3 &endPoint,
											const noPlane &plane, const float angle, const noVec3 &origin,
											float &tanHalfAngle, noVec3 &collisionPoint, noVec3 &endDir );
	...
	void			RotateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly,
											cm_vertex_t *v, noVec3 &rotationOrigin );
	bool			RotateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p );
	void			BoundsForRotation( const noVec3 &origin, const noVec3 &axis, const noVec3 &start, const noVec3 &end, idBounds &bounds );
	void			Rotation180( trace_t *results, const noVec3 &rorg, const noVec3 &axis,
									const float startAngle, const float endAngle, const noVec3 &start,
									const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
									cmHandle_t model, const noVec3 &origin, const noMat3 &modelAxis );
...
private:			// CollisionMap_contents.cpp
	...
	cm_node_t *		PointNode( const noVec3 &p, cm_model_t *model );
	int				PointContents( const noVec3 p, cmHandle_t model );
	int				TransformedPointContents( const noVec3 &p, cmHandle_t model, const noVec3 &origin, const noMat3 &modelAxis );
	int				ContentsTrm( trace_t *results, const noVec3 &start,
									const idTraceModel *trm, const noMat3 &trmAxis, int contentMask,
									cmHandle_t model, const noVec3 &modelOrigin, const noMat3 &modelAxis );

private:			// CollisionMap_trace.cpp
	void			TraceTrmThroughNode( cm_traceWork_t *tw, cm_node_t *node );
	void			TraceThroughAxialBSPTree_r( cm_traceWork_t *tw, cm_node_t *node, float p1f, float p2f, noVec3 &p1, noVec3 &p2);
	void			TraceThroughModel( cm_traceWork_t *tw );
	void			RecurseProcBSP_r( trace_t *results, int parentNodeNum, int nodeNum, float p1f, float p2f, const noVec3 &p1, const noVec3 &p2 );
...
private:			// CollisionMap_load.cpp
	...
	bool			PointInsidePolygon( cm_model_t *model, cm_polygon_t *p, noVec3 &v );
	...
	int				R_ChoppedAwayByProcBSP( int nodeNum, idFixedWinding *w, const noVec3 &normal, const noVec3 &origin, const float radius );
	int				ChoppedAwayByProcBSP( const idFixedWinding &w, const noPlane &plane, int contents );
	...
	idFixedWinding *WindingOutsideBrushes( idFixedWinding *w, const noPlane &plane, int contents, int patch, cm_node_t *headNode );
	...
	int				HashVec(const noVec3 &vec);
	int				GetVertex( cm_model_t *model, const noVec3 &v, int *vertexNum );
	int				GetEdge( cm_model_t *model, const noVec3 &v1, const noVec3 &v2, int *edgeNum, int v1num );
	void			CreatePolygon( cm_model_t *model, idFixedWinding *w, const noPlane &plane, const idMaterial *material, int primitiveNum );
	void			PolygonFromWinding( cm_model_t *model, idFixedWinding *w, const noPlane &plane, const idMaterial *material, int primitiveNum );
...
private:			// CollisionMap_debug
	...
	void			DrawEdge( cm_model_t *model, int edgeNum, const noVec3 &origin, const noMat3 &axis );
	void			DrawPolygon( cm_model_t *model, cm_polygon_t *p, const noVec3 &origin, const noMat3 &axis,
								const noVec3 &viewOrigin );
	void			DrawNodePolygons( cm_model_t *model, cm_node_t *node, const noVec3 &origin, const noMat3 &axis,
								const noVec3 &viewOrigin, const float radius );
