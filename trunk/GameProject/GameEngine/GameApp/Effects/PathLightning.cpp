...
{
PathLightning::PathLightning(ID3DX11Effect* effect, const std::vector<LightningPathSegment>& segments, int pattern_mask, unsigned int subdivisions):
	LightningSeed
	(
		effect,
...

{
	...
	std::copy(segments.begin(), segments.end(), back_inserter(vertices));
	m_path_segments = new Geometry::SimpleVertexBuffer<SubdivideVertex>(m_device, m_context, vertices);
