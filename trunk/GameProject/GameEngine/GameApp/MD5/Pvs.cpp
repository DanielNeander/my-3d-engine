...
typedef struct pvsPortal_s {
	...
	noPlane				plane;		// winding plane, normal points towards the area this portal leads to
...
===============
*/
void idPVS::AddPassageBoundaries( const idWinding &source, const idWinding &pass, bool flipClip, noPlane *bounds, int &numBounds, int maxBounds ) const {
	int			i, j, k, l;
	noVec3		v1, v2, normal;
	float		d, dist;
	bool		flipTest, front;
...
void idPVS::CreatePassages( void ) const {
	...
	noPlane passageBounds[MAX_PASSAGE_BOUNDS];
...
================
*/
int idPVS::GetPVSArea( const noVec3 &point ) const {
...
================
*/
pvsHandle_t idPVS::SetupCurrentPVS( const noVec3 &source, const pvsType_t type ) const {
	int sourceArea;
...
================
*/
bool idPVS::InCurrentPVS( const pvsHandle_t handle, const noVec3 &target ) const {
	int targetArea;
...
================
*/
void idPVS::DrawPVS( const noVec3 &source, const pvsType_t type ) const {
...
================
*/
void idPVS::DrawCurrentPVS( const pvsHandle_t handle, const noVec3 &source ) const {
	int i, j, k, numPoints, n, sourceArea;
	exitPortal_t portal;
	noPlane plane;
	noVec3 offset;
...
================
*/
bool idPVS::CheckAreasForPortalSky( const pvsHandle_t handle, const noVec3 &origin ) {
	int j, sourceArea;
