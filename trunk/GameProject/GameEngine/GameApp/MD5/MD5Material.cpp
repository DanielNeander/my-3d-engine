...
void idMaterial::CommonInit() {
	...
	bumpImg = diffuseImg = specularImg = -1;
}
...
void idMaterial::ParseStage( idLexer &src, const textureRepeat_t trpDefault ) {
	...
	if ( imageName[0] ) {
		...
		//}
		if (td ==  TD_DIFFUSE && diffuseImg == -1) {
			ImageFromFile( imageName, tf, allowPicmip, trp, td, cubeMap );		
			diffuseImg = gTex;
		} else if (td ==  TD_BUMP && bumpImg == -1) {
			ImageFromFile( imageName, tf, allowPicmip, trp, td, cubeMap );		
			bumpImg = gTex;
		} else if (td == TD_SPECULAR && specularImg == -1) {
			ImageFromFile( imageName, tf, allowPicmip, trp, td, cubeMap );		
			specularImg = gTex;
		}		
		if (gTex == -1)		
			LOG_DEBUG << "Doom3 Material : " << imageName;		
