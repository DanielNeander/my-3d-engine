...
================
*/
int idBitMsg::DirToBits( const noVec3 &dir, int numBits ) {
	int max, bits;
	float bias;
	...
	bits |= ( noMath::Ftoi( ( noMath::Fabs( dir.x ) + bias ) * max ) ) << ( numBits * 2 );
	bits |= FLOATSIGNBITSET( dir.y ) << ( numBits * 2 - 1 );
	bits |= ( noMath::Ftoi( ( noMath::Fabs( dir.y ) + bias ) * max ) ) << ( numBits * 1 );
	bits |= FLOATSIGNBITSET( dir.z ) << ( numBits * 1 - 1 );
	bits |= ( noMath::Ftoi( ( noMath::Fabs( dir.z ) + bias ) * max ) ) << ( numBits * 0 );
	return bits;
}
...
================
*/
noVec3 idBitMsg::BitsToDir( int bits, int numBits ) {
