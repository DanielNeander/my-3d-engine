...
UBOOL FDockingParent::LoadDockingLayout()
{
	...
	if( bOldVersion == FALSE)
	{
		...
		if(bFileExists == TRUE)
		{
			...
			if( InFile.Ok() && InFile.GetLength() > 0 )
			{
				// Load the layout info
				FString LoadedPerspectiveString;
				const INT NumCharsToLoad = InFile.GetLength() / sizeof( TCHAR );
				for( INT CurCharIndex = 0; CurCharIndex < NumCharsToLoad; ++CurCharIndex )
				{
					// NOTE: Loaded string is always in TCHAR format
					INT CurTChar = ( InFile.GetC() & 0xff );
					if( sizeof( TCHAR ) > 1 )
					{
						CurTChar |= ( CurTChar & 0xff << 8 ) | ( InFile.GetC() & 0xff );
					}

					LoadedPerspectiveString.AppendChar( CurTChar );
				}
...
void FDockingParent::SaveDockingLayout()
{
	...
	{
		// Save layout info
		wxString SavedPerspectiveString = AUIManager.SavePerspective();
		if( SavedPerspectiveString.Len() > 0 )
		{
			// NOTE: We always save in TCHAR format
			OutFile.Write( &SavedPerspectiveString[ 0 ], SavedPerspectiveString.Len() * sizeof( SavedPerspectiveString[ 0 ] ) );
		}

		// Save the current version of the file to a INI file.
		GConfig->SetInt(TEXT("DockingLayoutVersionsAUI"), GetDockingParentName(), GetDockingParentVersion(), GEditorUserSettingsIni);
	}
}
 ...
 */
void FDockingParent::AddDockingWindow( wxWindow* ClientWindow, EDockHost DockHost, const TCHAR* WindowTitle, const TCHAR* WindowName )
{
	...
	else
	{
		...
		PaneInfo.best_size.Set( 300, 240 );	// @todo AUI
...
{
	OutFileName = FString::Printf(TEXT("%s%s%s_layoutAUI.txt"),*appGameConfigDir(), GGameName,GetDockingParentName());
}
