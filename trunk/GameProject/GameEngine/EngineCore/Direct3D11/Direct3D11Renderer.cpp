									   ...
									   const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags)
{
	...
	CShaderInclude shaderInclude("Data/Shaders", "Data/Shaders");
	...
	if (vsText != NULL)
	{
		...
		if (vstarget_.empty())
			vstarget_= (feature_level == D3D_FEATURE_LEVEL_11_0)? "vs_5_0" : (feature_level == D3D_FEATURE_LEVEL_10_1)? "vs_4_1" : "vs_4_0";
				
		if (SUCCEEDED(D3DCompile(shaderString, shaderString.getLength(), fileName, NULL, &shaderInclude, "main", vstarget_.c_str(), compileFlags, 0, &shaderBuf, &errorsBuf)))
		{
	...
	if (gsText != NULL)
	{
		...
		if (gstarget_.empty())
			gstarget_ = (feature_level == D3D_FEATURE_LEVEL_11_0)? "gs_5_0" : (feature_level == D3D_FEATURE_LEVEL_10_1)? "gs_4_1" : "gs_4_0";

		if (SUCCEEDED(D3DCompile(shaderString, shaderString.getLength(), fileName, NULL, &shaderInclude, "main", gstarget_.c_str(), compileFlags, 0, &shaderBuf, &errorsBuf)))
		{
	...
	if (fsText != NULL)
	{
		...
		if (pstarget_.empty())
			pstarget_ = (feature_level == D3D_FEATURE_LEVEL_11_0)? "ps_5_0" : (feature_level == D3D_FEATURE_LEVEL_10_1)? "ps_4_1" : "ps_4_0";

		if (SUCCEEDED(D3DCompile(shaderString, shaderString.getLength(), fileName, NULL, &shaderInclude, "main", pstarget_.c_str(), compileFlags, 0, &shaderBuf, &errorsBuf)))
		{
...
void Direct3D11Renderer::changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture)
{
	const Sampler *s = getSampler(shaders[shdaerId].textures, shaders[shdaerId].nTextures, textureName);
	if (s)
	{
		if (s->vsIndex >= 0)
		{
...
void Direct3D11Renderer::changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture, const SamplerStateID samplerState)
{	
	const Sampler *s = getSampler(shaders[shdaerId].textures, shaders[shdaerId].nTextures, textureName);
	if (s)
	{
...
ID3D11RenderTargetView* Direct3D11Renderer::getBackBuffer()
{
	return backBufferRTV;
}

ID3D11DepthStencilView* Direct3D11Renderer::getDepthBuffer()
{
	return depthBufferDSV;
}
...
void Direct3D11Renderer::changeVertexFormat2(const VertexFormatID vertexFormatID)
{
...
void* Direct3D11Renderer::LockVB( VertexBufferID vb )
{

	D3D11_MAPPED_SUBRESOURCE ms;	
	D3D11_MAP flag = D3D11_MAP_WRITE_NO_OVERWRITE;
	context->Map(vertexBuffers[vb].vertexBuffer, 0, flag, 0, &ms);	
	
	
	return ms.pData;
}

void Direct3D11Renderer::UnlockVB( VertexBufferID vb)
{
	context->Unmap(vertexBuffers[vb].vertexBuffer, 0);
}

int Direct3D11Renderer::CopyToVB( VertexBufferID vb, const void *src, const uint size, uint vbOffset /*= 0*/ )
{
	uint offset = vbOffset;

	D3D11_MAPPED_SUBRESOURCE ms;	
	D3D11_MAP flag = D3D11_MAP_WRITE_DISCARD;
	if (FAILED(context->Map(vertexBuffers[vb].vertexBuffer, 0, flag, 0, &ms)))
	{
		ASSERT(false);
	}
...
ID3D11DepthStencilView* Direct3D11Renderer::getTextureDSV( const TextureID texture ) const
{
	return textures[texture].dsv;
}

ID3D11ShaderResourceView* Direct3D11Renderer::getTextureSRV( const TextureID texture ) const
{
	return textures[texture].srv;
}
...
RenderData::RenderData()
{
	...
	vertexEnd = vertexStart = 0;
	diffuseSampler = normalSampler = decalSampler = -1;
	blendMode = -1;
	depthMode = -1;
	cull = -1;
	vb = vf = ib = -1;
