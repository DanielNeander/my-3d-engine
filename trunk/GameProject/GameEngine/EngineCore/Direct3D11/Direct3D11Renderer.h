...
struct RenderData {
	...
	VertexBufferID vb;
	IndexBufferID ib;
	SamplerStateID	diffuseSampler;
	SamplerStateID	normalSampler;
	SamplerStateID	decalSampler;
	TextureID	baseTex;
	TextureID  bumpTex;
	TextureID	normalTex;
	TextureID	decalTex;
	uint32 startIndex, endIndex;	
	uint32 vertexStart, vertexEnd;
	int colorIndex;
	int opacity;
	BlendStateID blendMode;
	DepthStateID depthMode;
	bool unlit;
	RasterizerStateID cull;
	Primitives prim;
	bool visible;
};
...
//	void changeSamplerState(const uint samplerUnit, const SamplerStateID samplerState);
	...
	ID3D11DepthStencilView*	getTextureDSV(const TextureID texture) const;
	ID3D11ShaderResourceView*	getTextureSRV(const TextureID texture) const;
	ID3D11RenderTargetView* getBackBuffer();
	ID3D11DepthStencilView* getDepthBuffer();

	void changeVertexFormat2(const VertexFormatID vertexFormatID);
	void changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture, const SamplerStateID samplerState);
	void changeTexture(ShaderID shdaerId, const char *textureName, const TextureID texture);

	void* LockVB( VertexBufferID vb );
	void UnlockVB( VertexBufferID vb);
	int CopyToVB(VertexBufferID vb, const void *src, const uint size, uint vbOffset = 0);
