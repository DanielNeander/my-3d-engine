	...
	if (format <= FORMAT_I16 || format == FORMAT_RGB10A2){
		...
		switch (format){
			...
			default:
				...
				headerDX10.miscFlag = (depth == 0)? D3D11_RESOURCE_MISC_TEXTURECUBE : 0;
				headerDX10.resourceDimension = is1D()? D3D11_RESOURCE_DIMENSION_TEXTURE1D : is3D()? D3D11_RESOURCE_DIMENSION_TEXTURE3D : D3D11_RESOURCE_DIMENSION_TEXTURE2D;
				switch (format){
