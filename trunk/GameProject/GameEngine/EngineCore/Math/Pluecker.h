...
public:	
					...
					explicit idPluecker( const noVec3 &start, const noVec3 &end );
	...
	void			FromLine( const noVec3 &start, const noVec3 &end );			// pluecker from line
	void			FromRay( const noVec3 &start, const noVec3 &dir );			// pluecker from ray
	bool			FromPlanes( const noPlane &p1, const noPlane &p2 );			// pluecker from intersection of planes
	bool			ToLine( noVec3 &start, noVec3 &end ) const;					// pluecker to line
	bool			ToRay( noVec3 &start, noVec3 &dir ) const;					// pluecker to ray
	void			ToDir( noVec3 &dir ) const;									// pluecker to direction
...
ID_INLINE idPluecker::idPluecker( const noVec3 &start, const noVec3 &end ) {
	FromLine( start, end );
}
...
ID_INLINE void idPluecker::FromLine( const noVec3 &start, const noVec3 &end ) {
...
ID_INLINE void idPluecker::FromRay( const noVec3 &start, const noVec3 &dir ) {
...
ID_INLINE bool idPluecker::ToLine( noVec3 &start, noVec3 &end ) const {
	noVec3 dir1, dir2;
	float d;
...
ID_INLINE bool idPluecker::ToRay( noVec3 &start, noVec3 &dir ) const {
	noVec3 dir1;
	float d;
...
ID_INLINE void idPluecker::ToDir( noVec3 &dir ) const {
	dir[0] = -p[2];
	dir[1] = p[5];
	dir[2] = -p[4];
}
...
ID_INLINE float idPluecker::Length( void ) const {
	return ( float )noMath::Sqrt( p[5] * p[5] + p[4] * p[4] + p[2] * p[2] );
}
