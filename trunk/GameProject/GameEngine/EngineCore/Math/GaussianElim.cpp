...
Solve( float* b, float* A, unsigned int n )
{
    ...
    for ( unsigned int pivot = 0; pivot < n; ++pivot )
    {
        ...
        float maxelem = noMath::Abs( A[ maxrow + n*pivot ] );
        for ( row = pivot+1; row < n; ++row )
        {
            float elem = noMath::Abs( A[ row + n*pivot ] );
            if ( elem > maxelem )
            {
                maxelem = elem;
                maxrow = row;
...
Determinant( float* A, unsigned int n )
{
    ...
    for ( unsigned int pivot = 0; pivot < n; ++pivot )
    {
        ...
        // find the largest magnitude element in the current column
        unsigned int maxrow = pivot;
        float maxelem = noMath::Abs( copy[ maxrow + n*pivot ] );
        for ( row = pivot+1; row < n; ++row )
        {
            float elem = noMath::Abs( copy[ row + n*pivot ] );
            if ( elem > maxelem )
            {
                maxelem = elem;
                maxrow = row;
