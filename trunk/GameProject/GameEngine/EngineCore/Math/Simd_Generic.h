...
public:
	...
	virtual void VPCALL Dot( float *dst,			const noVec3 &constant,	const noVec3 *src,		const int count );
	virtual void VPCALL Dot( float *dst,			const noVec3 &constant,	const noPlane *src,		const int count );
	virtual void VPCALL Dot( float *dst,			const noVec3 &constant,	const idDrawVert *src,	const int count );
	virtual void VPCALL Dot( float *dst,			const noPlane &constant,const noVec3 *src,		const int count );
	virtual void VPCALL Dot( float *dst,			const noPlane &constant,const noPlane *src,		const int count );
	virtual void VPCALL Dot( float *dst,			const noPlane &constant,const idDrawVert *src,	const int count );
	virtual void VPCALL Dot( float *dst,			const noVec3 *src0,		const noVec3 *src1,		const int count );
	...
	virtual	void VPCALL MinMax( noVec2 &min,		noVec2 &max,			const noVec2 *src,		const int count );
	virtual void VPCALL MinMax( noVec3 &min,		noVec3 &max,			const noVec3 *src,		const int count );
	virtual	void VPCALL MinMax( noVec3 &min,		noVec3 &max,			const idDrawVert *src,	const int count );
	virtual	void VPCALL MinMax( noVec3 &min,		noVec3 &max,			const idDrawVert *src,	const int *indexes,		const int count );
	...
	virtual void VPCALL TransformVerts( idDrawVert *verts, const int numVerts, const idJointMat *joints, const noVec4 *weights, const int *index, const int numWeights );
	virtual void VPCALL TracePointCull( byte *cullBits, byte &totalOr, const float radius, const noPlane *planes, const idDrawVert *verts, const int numVerts );
	virtual void VPCALL DecalPointCull( byte *cullBits, const noPlane *planes, const idDrawVert *verts, const int numVerts );
	virtual void VPCALL OverlayPointCull( byte *cullBits, noVec2 *texCoords, const noPlane *planes, const idDrawVert *verts, const int numVerts );
	virtual void VPCALL DeriveTriPlanes( noPlane *planes, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes );
	virtual void VPCALL DeriveTangents( noPlane *planes, idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes );
	...
	virtual void VPCALL CreateTextureSpaceLightVectors( noVec3 *lightVectors, const noVec3 &lightOrigin, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes );
	virtual void VPCALL CreateSpecularTextureCoords( noVec4 *texCoords, const noVec3 &lightOrigin, const noVec3 &viewOrigin, const idDrawVert *verts, const int numVerts, const int *indexes, const int numIndexes );
	virtual int  VPCALL CreateShadowCache( noVec4 *vertexCache, int *vertRemap, const noVec3 &lightOrigin, const idDrawVert *verts, const int numVerts );
	virtual int  VPCALL CreateVertexProgramShadowCache( noVec4 *vertexCache, const idDrawVert *verts, const int numVerts );
