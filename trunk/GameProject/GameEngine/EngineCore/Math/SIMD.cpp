...
void idSIMD::InitProcessor( const char *module, bool forceGeneric ) {
	...
	cpuid = Sys_GetCPUId(); //idLib::sys->GetProcessorId();
...
void TestAdd( void ) {
	...
	for ( i = 0; i < COUNT; i++ ) {
		if ( noMath::Fabs( fdst0[i] - fdst1[i] ) > 1e-5f ) {
			break;
		}
	}
...
void TestDiv( void ) {
	...
	for ( i = 0; i < COUNT; i++ ) {
		...
		do {
			fsrc1[i] = srnd.CRandomFloat() * 10.0f;
		} while( noMath::Fabs( fsrc1[i] ) < 0.1f );
	}
	...
	for ( i = 0; i < COUNT; i++ ) {
		if ( noMath::Fabs( fdst0[i] - fdst1[i] ) > 1e-3f ) {
			break;
		}
	}
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
...
void TestDot( void ) {
	...
	ALIGN16( float fsrc0[COUNT] );
	ALIGN16( float fsrc1[COUNT] );
	ALIGN16( noVec3 v3src0[COUNT] );
	ALIGN16( noVec3 v3src1[COUNT] );
	ALIGN16( noVec3 v3constant ) ( 1.0f, 2.0f, 3.0f );
	ALIGN16( noPlane v4src0[COUNT] );
	ALIGN16( noPlane v4constant ) (1.0f, 2.0f, 3.0f, 4.0f);
	...
	PrintClocks( "generic->Dot( noVec3 * noVec3[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noVec3 * noVec3[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->Dot( noVec3 * noPlane[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noVec3 * noPlane[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->Dot( noVec3 * idDrawVert[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noVec3 * idDrawVert[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->Dot( noPlane * noVec3[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noPlane * noVec3[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->Dot( noPlane * noPlane[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noPlane * noPlane[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->Dot( noPlane * idDrawVert[] )", COUNT, bestClocksGeneric );
	...
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	PrintClocks( va( "   simd->Dot( noPlane * idDrawVert[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	for ( i = 0; i < COUNT; i++ ) {
		if ( noMath::Fabs( fdst0[i] - fdst1[i] ) > 1e-4f ) {
			break;
		}
	}
	result = ( i >= COUNT ) ? "ok" : S_COLOR_RED"X";
	...
	for ( j = 0; j < 50 && j < COUNT; j++ ) {
		...
		result = noMath::Fabs( dot1 - dot2 ) < 1e-4f ? "ok" : S_COLOR_RED"X";
...
void TestMinMax( void ) {
	...
	TIME_TYPE start, end, bestClocksGeneric, bestClocksSIMD;
	ALIGN16( float fsrc0[COUNT] );
	ALIGN16( noVec2 v2src0[COUNT] );
	ALIGN16( noVec3 v3src0[COUNT] );
	ALIGN16( idDrawVert drawVerts[COUNT] );
	ALIGN16( int indexes[COUNT] );
	float min = 0.0f, max = 0.0f, min2 = 0.0f, max2 = 0.0f;
	noVec2 v2min, v2max, v2min2, v2max2;
	noVec3 vmin, vmax, vmin2, vmax2;
	const char *result;
	...
	PrintClocks( "generic->MinMax( noVec2[] )", COUNT, bestClocksGeneric );
	...
	PrintClocks( va( "   simd->MinMax( noVec2[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
	...
	PrintClocks( "generic->MinMax( noVec3[] )", COUNT, bestClocksGeneric );
	...
	PrintClocks( va( "   simd->MinMax( noVec3[] ) %s", result ), COUNT, bestClocksSIMD, bestClocksGeneric );
...
void TestTracePointCull( void ) {
	...
	planes[0].SetNormal( noVec3(  1,  0, 0 ) );
	planes[1].SetNormal( noVec3( -1,  0, 0 ) );
	planes[2].SetNormal( noVec3(  0,  1, 0 ) );
	planes[3].SetNormal( noVec3(  0, -1, 0 ) );
	planes[0][3] = -5.3f;
	planes[1][3] = 5.3f;
	planes[2][3] = -3.4f;
	planes[3][3] = 3.4f;
...
void TestDecalPointCull( void ) {
	...
	planes[2].SetNormal( noVec3(  0,  1,  0 ) );
	planes[3].SetNormal( noVec3(  0, -1,  0 ) );
	planes[4].SetNormal( noVec3(  0,  0,  1 ) );
	planes[5].SetNormal( noVec3(  0,  0, -1 ) );
	planes[0][3] = -5.3f;
	planes[1][3] = 5.3f;
	planes[2][3] = -4.4f;
	planes[3][3] = 4.4f;
...
void TestOverlayPointCull( void ) {
	int i, j;
	TIME_TYPE start, end, bestClocksGeneric, bestClocksSIMD;
	ALIGN16( noPlane planes[2] );
	ALIGN16( idDrawVert drawVerts[COUNT] );
	ALIGN16( byte cullBits1[COUNT] );
	ALIGN16( byte cullBits2[COUNT] );
	ALIGN16( noVec2 texCoords1[COUNT] );
	ALIGN16( noVec2 texCoords2[COUNT] );
	const char *result;

	idRandom srnd( RANDOM_SEED );

	planes[0].SetNormal( noVec3( 0.3f, 0.2f, 0.9f ) );
	planes[1].SetNormal( noVec3( 0.9f, 0.2f, 0.3f ) );
	planes[0][3] = -5.3f;
	planes[1][3] = -4.3f;
...
void TestDeriveTriPlanes( void ) {
	...
	ALIGN16( noPlane planes1[COUNT] );
	ALIGN16( noPlane planes2[COUNT] );
...
void TestGetTextureSpaceLightVectors( void ) {
	...
	TIME_TYPE start, end, bestClocksGeneric, bestClocksSIMD;
	ALIGN16( idDrawVert drawVerts[COUNT] );
	ALIGN16( noVec4 texCoords1[COUNT] );
	ALIGN16( noVec4 texCoords2[COUNT] );
	ALIGN16( int indexes[COUNT*3] );
	ALIGN16( noVec3 lightVectors1[COUNT] );
	ALIGN16( noVec3 lightVectors2[COUNT] );
	noVec3 lightOrigin;
	const char *result;
...
void TestCreateShadowCache( void ) {
	...
	TIME_TYPE start, end, bestClocksGeneric, bestClocksSIMD;
	ALIGN16( idDrawVert drawVerts[COUNT] );
	ALIGN16( noVec4 vertexCache1[COUNT*2] );
	ALIGN16( noVec4 vertexCache2[COUNT*2] );
	ALIGN16( int originalVertRemap[COUNT] );
	ALIGN16( int vertRemap1[COUNT] );
	ALIGN16( int vertRemap2[COUNT] );
	ALIGN16( noVec3 lightOrigin );
	...
	for ( numSpeakers = 1; numSpeakers <= 2; numSpeakers++ ) {
		...
		for ( kHz = 11025; kHz <= 44100; kHz *= 2 ) {
			...
			for ( i = 0; i < MIXBUFFER_SAMPLES*numSpeakers; i++ ) {
				if ( noMath::Fabs( samples1[i] - samples2[i] ) > SOUND_UPSAMPLE_EPSILON ) {
					break;
				}
			}
...
void TestSoundMixing( void ) {
	...
	for ( i = 0; i < MIXBUFFER_SAMPLES*6; i++ ) {
		if ( noMath::Fabs( mixBuffer1[i] - mixBuffer2[i] ) > SOUND_MIX_EPSILON ) {
			break;
		}
	}
...
void TestMath( void ) {
	...
	PrintClocks( "    noMath::Fabs( tst )", 1, bestClocks );
	...
	PrintClocks( "    noMath::Sqrt( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.RandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Sqrt16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.RandomFloat();
	}
	PrintClocks( "  noMath::Sqrt16( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.RandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Sqrt64( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.RandomFloat();
	}
	PrintClocks( "  noMath::Sqrt64( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.RandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = tst * noMath::RSqrt( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.RandomFloat();
	}
	PrintClocks( "   noMath::RSqrt( tst )", 1, bestClocks );
	...
	PrintClocks( "     noMath::Sin( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Sin16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "   noMath::Sin16( tst )", 1, bestClocks );
	...
	PrintClocks( "     noMath::Cos( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Cos16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "   noMath::Cos16( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		noMath::SinCos( tst, tst, tst2 );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::SinCos( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		noMath::SinCos16( tst, tst, tst2 );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "noMath::SinCos16( tst )", 1, bestClocks );
	...
	PrintClocks( "     noMath::Tan( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Tan16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "   noMath::Tan16( tst )", 1, bestClocks );
	...
	for ( i = 0; i < NUMTESTS; i++ ) {
		...
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * ( 1.0f / noMath::PI );
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "    noMath::ASin( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::ASin16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * ( 1.0f / noMath::PI );
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::ASin16( tst )", 1, bestClocks );
	...
	PrintClocks( "    noMath::ACos( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::ACos16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * ( 1.0f / noMath::PI );
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::ACos16( tst )", 1, bestClocks );
	...
	PrintClocks( "    noMath::ATan( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::ATan16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::ATan16( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Pow( 2.7f, tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * 0.1f;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "    noMath::Pow( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Pow16( 2.7f, tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * 0.1f;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::Pow16( tst )", 1, bestClocks );
	...
	PrintClocks( "    noMath::Exp( tst )", 1, bestClocks );

	bestClocks = 0;
	tst = rnd.CRandomFloat();
	for ( i = 0; i < NUMTESTS; i++ ) {
		StartRecordTime( start );
		tst = noMath::Exp16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst * 0.1f;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::Exp16( tst )", 1, bestClocks );
	...
	PrintClocks( "    noMath::Log( tst )", 1, bestClocks );
	...
	for ( i = 0; i < NUMTESTS; i++ ) {
		tst = fabs( tst ) + 1.0f;
		StartRecordTime( start );
		tst = noMath::Log16( tst );
		StopRecordTime( end );
		GetBest( start, end, bestClocks );
		testvar = ( testvar + tst ) * tst;
		tst = rnd.CRandomFloat();
	}
	PrintClocks( "  noMath::Log16( tst )", 1, bestClocks );

	////idLib::common->Printf( "testvar = %f\n", testvar );

	noMat3 resultMat3;
	noQuat fromQuat, toQuat, resultQuat;
	idCQuat cq;
	noAngles ang;

	fromQuat = noAngles( 30, 45, 0 ).ToQuat();
	toQuat = noAngles( 45, 0, 0 ).ToQuat();
	cq = noAngles( 30, 45, 0 ).ToQuat().ToCQuat();
	ang = noAngles( 30, 40, 50 );
	...
	PrintClocks( "       noQuat::ToMat3()", 1, bestClocks );
	...
	PrintClocks( "        noQuat::Slerp()", 1, bestClocks );
	...
	PrintClocks( "     noAngles::ToQuat()", 1, bestClocks );
	...
	PrintClocks( "     noAngles::ToMat3()", 1, bestClocks );
}
