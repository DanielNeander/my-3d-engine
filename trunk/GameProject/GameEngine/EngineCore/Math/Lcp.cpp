...
void idLCP_Square::GetMaxStep( int d, float dir, float &maxStep, int &limit, int &limitSide ) const {
	...
	if ( noMath::Fabs( delta_a[d] ) > LCP_DELTA_ACCEL_EPSILON ) {
...
bool idLCP_Square::Solve( const idMatX &o_m, idVecX &o_x, const idVecX &o_b, const idVecX &o_lo, const idVecX &o_hi, const int *o_boxIndex ) {
	...
	numUnbounded = 0;
	for ( i = 0; i < m.GetNumRows(); i++ ) {
		if ( lo[i] == -noMath::INFINITY && hi[i] == noMath::INFINITY ) {
			if ( numUnbounded != i ) {
				Swap( numUnbounded, i );
			}
			numUnbounded++;
	...
	if ( boxIndex ) {
		for ( i = m.GetNumRows() - 1; i >= numUnbounded; i-- ) {
			if ( boxIndex[i] >= 0 && ( lo[i] != -noMath::INFINITY || hi[i] != noMath::INFINITY ) ) {
				boxStartIndex--;
				if ( boxStartIndex != i ) {
	...
	for ( i = numUnbounded; i < m.GetNumRows(); i++ ) {
		...
		if ( i == boxStartIndex ) {
			...
			for ( j = boxStartIndex; j < m.GetNumRows(); j++ ) {
				s = o_x[boxIndex[j]];
				if ( lo[j] != -noMath::INFINITY ) {
					lo[j] = - noMath::Fabs( lo[j] * s );
				}
				if ( hi[j] != noMath::INFINITY ) {
					hi[j] = noMath::Fabs( hi[j] * s );
				}
			}
		}
		...
		if ( noMath::Fabs( a[i] ) <= LCP_ACCEL_EPSILON ) {
			side[i] = 0;
			AddClamped( i );
			continue;
		}
	...
	if ( !failed ) {
		...
		for ( i = 0; i < m.GetNumRows(); i++ ) {
			...
			} else if ( f[i] < lo[i] || f[i] > hi[i] || noMath::Fabs( a[i] ) > 1.0f ) {
				int bah3 = 1;
			}
		}
	}
