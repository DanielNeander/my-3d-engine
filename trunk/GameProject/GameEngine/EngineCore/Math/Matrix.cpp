...
noMat2 mat2_zero( noVec2( 0, 0 ), noVec2( 0, 0 ) );
noMat2 mat2_identity( noVec2( 1, 0 ), noVec2( 0, 1 ) );
...
bool noMat2::InverseSelf( void ) {
	...
	if ( noMath::Fabs( det ) < MATRIX_INVERSE_EPSILON ) {
		return false;
	}
...
/*
============
noMat2::InverseFastSelf
============
*/
bool noMat2::InverseFastSelf( void ) {
#if 1
	// 2+4 = 6 multiplications
	//		 1 division
	double det, invDet, a;
...
=============
*/
const char *noMat2::ToString( int precision ) const {
	return idStr::FloatArrayToString( ToFloatPtr(), GetDimension(), precision );	
}


noMat3 mat3_zero( noVec3( 0, 0, 0 ), noVec3( 0, 0, 0 ), noVec3( 0, 0, 0 ) );
noMat3 mat3_identity( noVec3( 1, 0, 0 ), noVec3( 0, 1, 0 ), noVec3( 0, 0, 1 ) );

noMat4 mat4_zero(noVec4( 0, 0, 0, 0 ), noVec4( 0, 0, 0, 0 ), noVec4( 0, 0, 0, 0 ), noVec4(0, 0, 0, 0) );
noMat4 mat4_identity( noVec4( 1, 0, 0, 0 ), noVec4( 0, 1, 0, 0 ), noVec4( 0, 0, 1, 0 ), noVec4(0, 0, 0, 1) );
...
============
*/
idCQuat noMat3::ToCQuat( void ) const {
	noQuat q = ToQuat();
	if ( q.w < 0.0f ) {
...
noMat4 noMat4::Transpose( void ) const
{
	noMat4	transpose;
	int		i, j;
...
noMat4 &noMat4::TransposeSelf( void ) {
	float	temp;
	int		i, j;
...
float noMat4::Determinant( void ) const {
...
bool noMat4::InverseFastSelf( void )
{
...
float noMat3::Determinant( void ) const
{
...
bool noMat3::InverseFastSelf( void )
{
#if 1
	// 18+3+9 = 30 multiplications
	//			 1 division
...
noMat3 noMat3::InertiaTranslate( const float mass, const noVec3 &centerOfMass, const noVec3 &translation ) const
{
	noMat3 m;
	noVec3 newCenter;
...
noMat3 & noMat3::InertiaTranslateSelf( const float mass, const noVec3 &centerOfMass, const noVec3 &translation )
{
	noMat3 m;
	noVec3 newCenter;
...
noMat3 noMat3::InertiaRotate( const noMat3 &rotation ) const
{
	// NOTE: the rotation matrix is stored column-major
	return rotation.Transpose() * (*this) * rotation;
}

noMat3 & noMat3::InertiaRotateSelf( const noMat3 &rotation )
{
	// NOTE: the rotation matrix is stored column-major
...
noAngles noMat3::ToAngles( void ) const
{
	noAngles	angles;
	double		theta;
	double		cp;
	...
	if ( cp > 8192.0f * noMath::F_EPSILON ) {
...
noQuat noMat3::ToQuat( void ) const
{
	noQuat		q;
	float		trace;
	float		s;
...
noRotation noMat3::ToRotation( void ) const
{
	noRotation	r;
	float		trace;
	float		s;
	...
	r.angle = noMath::ACos( r.angle );
	if ( noMath::Fabs( r.angle ) < 1e-10f ) {
...
noVec3 noMat3::ToAngularVelocity( void ) const
{
	noRotation rotation = ToRotation();
	return rotation.GetVec() * DEG2RAD( rotation.GetAngle() );
}

const char * noMat3::ToString( int precision  ) const
{
	return idStr::FloatArrayToString( ToFloatPtr(), GetDimension(), precision );	
}
...
const char *noMat4::ToString( int precision ) const {
	return idStr::FloatArrayToString( ToFloatPtr(), GetDimension(), precision );
}
...
bool idMatX::IsPositiveSemiDefinite( const float epsilon ) const {
	...
	for ( i = 0; i < numRows; i++ ) {
		...
		for ( j = i; j < numColumns; j++ ) {
			...
			for ( k = 0; k < numRows; k++ ) {
				if ( noMath::Fabs( m[k][j] ) > epsilon ) {
					return false;
				}
				if ( noMath::Fabs( m[j][k] ) > epsilon ) {
					return false;
				}
			}
		}
...
bool idMatX::LU_Factor( int *index, float *det ) {
	...
	for ( i = 0; i < min; i++ ) {
		...
		newi = i;
		s = noMath::Fabs( (*this)[i][i] );

		if ( index ) {
			// find the largest absolute pivot
			for ( j = i + 1; j < numRows; j++ ) {
				t = noMath::Fabs( (*this)[j][i] );
				if ( t > s ) {
					newi = j;
					s = t;
				}
...
void idMatX::QR_Rotate( idMatX &R, int i, float a, float b ) {
	...
	if ( a == 0.0f ) {
		c = 0.0f;
		s = ( b >= 0.0f ) ? 1.0f : -1.0f;
	} else if ( noMath::Fabs( a ) > noMath::Fabs( b ) ) {
		f = b / a;
		c = noMath::Fabs( 1.0f / noMath::Sqrt( 1.0f + f * f ) );
		if ( a < 0.0f ) {
			c = -c;
		}
		s = f * c;
		...
		f = a / b;
		s = noMath::Fabs( 1.0f / noMath::Sqrt( 1.0f + f * f ) );
		if ( b < 0.0f ) {
			s = -s;
		}
		c = f * s;
...
bool idMatX::QR_UpdateRankOne( idMatX &R, const idVecX &v, const idVecX &w, float alpha ) {
	...
	for ( i = k-1; i >= 0; i-- ) {
		...
		if ( u[i] == 0.0f ) {
			u[i] = noMath::Fabs( u[i+1] );
		} else if ( noMath::Fabs( u[i] ) > noMath::Fabs( u[i+1] ) ) {
			f = u[i+1] / u[i];
			u[i] = noMath::Fabs( u[i] ) * noMath::Sqrt( 1.0f + f * f );
		} else {
			f = u[i] / u[i+1];
			u[i] = noMath::Fabs( u[i+1] ) * noMath::Sqrt( 1.0f + f * f );
		}
	}
...
float idMatX::Pythag( float a, float b ) const {
	...
	if ( at > bt ) {
		ct = bt / at;
		return at * noMath::Sqrt( 1.0f + ct * ct );
	} else {
		if ( bt ) {
			ct = at / bt;
			return bt * noMath::Sqrt( 1.0f + ct * ct );
		} else {
			return 0.0f;
		}
	}
...
void idMatX::SVD_BiDiag( idVecX &w, idVecX &rv1, float &anorm ) {
	...
	for ( i = 0; i < numColumns; i++ ) {
		...
		r = noMath::Fabs( w[i] ) + noMath::Fabs( rv1[i] );
		if ( r > anorm ) {
			anorm = r;
		}
	}
...
bool idMatX::SVD_Factor( idVecX &w, idMatX &V ) {
	...
	for ( k = numColumns - 1; k >= 0; k-- ) {
		for ( its = 1; its <= 30; its++ ) {
			...
			for ( l = k; l >= 0; l-- ) {
				nm = l - 1;
				if ( ( noMath::Fabs( rv1[l] ) + anorm ) == anorm /* noMath::Fabs( rv1[l] ) < noMath::FLT_EPSILON */ ) {
					flag = 0;
					break;
				}
				if ( ( noMath::Fabs( w[nm] ) + anorm ) == anorm /* noMath::Fabs( w[nm] ) < noMath::FLT_EPSILON */ ) {
					break;
				}
			}
			if ( flag ) {
				...
				for ( i = l; i <= k; i++ ) {
					...
					if ( ( noMath::Fabs( f ) + anorm ) != anorm /* noMath::Fabs( f ) > noMath::FLT_EPSILON */ ) {
						g = w[i];
						h = Pythag( f, g );
						w[i] = h;
						h = 1.0f / h;
...
void idMatX::SVD_Inverse( idMatX &inv, const idVecX &w, const idMatX &V ) const {
	...
	for ( i = 0; i < numRows; i++ ) {
		wi = w[i];
		wi = ( wi < noMath::F_EPSILON ) ? 0.0f : 1.0f / wi;
		for ( j = 0; j < numColumns; j++ ) {
			V2[j][i] *= wi;
		}
	}
...
bool idMatX::Cholesky_UpdateRankOne( const idVecX &v, float alpha, int offset ) {
	...
	for ( i = offset; i < numColumns; i++ ) {
		...
		(*this)[i][i] = newDiag = noMath::Sqrt( newDiagSqr );
...
bool idMatX::Cholesky_UpdateIncrement( const idVecX &v ) {
	...
	(*this)[numRows - 1][numRows - 1] = noMath::Sqrt( sum );
	...
	for ( i0 = 0; i0 < numRows; i0++ ) {
		for ( i1 = 0; i1 < maxIter; i1++ ) {
			for ( i2 = i0; i2 <= numRows - 2; i2++ ) {
				a = noMath::Fabs( diag[i2] ) + noMath::Fabs( diag[i2+1] );
				if ( noMath::Fabs( subd[i2] ) + a == a ) {
					break;
				}
			}
			if ( i2 == i0 ) {
			...
			for ( i3 = i2 - 1; i3 >= i0; i3-- ) {
				f = s * subd[i3];
				b = c * subd[i3];
				if ( noMath::Fabs( f ) >= noMath::Fabs( g ) ) {
					c = g / f;
					r = noMath::Sqrt( c * c + 1.0f );
					subd[i3+1] = f * r;
					s = 1.0f / r;
					c *= s;
				} else {
					s = f / g;
					r = noMath::Sqrt( s * s + 1.0f );
					subd[i3+1] = g * r;
					c = 1.0f / r;
					s *= c;
				}
...
void idMatX::HessenbergReduction( idMatX &H ) {
	...
	for ( m = low + 1; m <= high - 1; m++ ) {
		...
		scale = 0.0f;
		for ( i = m; i <= high; i++ ) {
			scale = scale + noMath::Fabs( H[i][m-1] );
		}
		if ( scale != 0.0f ) {
...
void idMatX::ComplexDivision( float xr, float xi, float yr, float yi, float &cdivr, float &cdivi ) {
	float r, d;
	if ( noMath::Fabs( yr ) > noMath::Fabs( yi ) ) {
		r = yi / yr;
		d = yr + r * yi;
		cdivr = ( xr + r * xi ) / d;
		cdivi = ( xi - r * xr ) / d;
...
bool idMatX::HessenbergToRealSchur( idMatX &H, idVecX &realEigenValues, idVecX &imaginaryEigenValues ) {
	...
	while( n >= low ) {
		...
		int l = n;
		while ( l > low ) {
			s = noMath::Fabs( H[l-1][l-1] ) + noMath::Fabs( H[l][l] );
			if ( s == 0.0f ) {
				s = norm;
			}
			if ( noMath::Fabs( H[l][l-1] ) < eps * s ) {
				break;
			}
			l--;
		}
		...
		if ( l == n ) {			// one root found
			...
			if ( q >= 0.0f ) {		// real pair
				...
				imaginaryEigenValues[n] = 0.0f;
				x = H[n][n-1];
				s = noMath::Fabs( x ) + noMath::Fabs( z );
				p = x / s;
				q = z / s;
				r = noMath::Sqrt( p * p + q * q );
				p = p / r;
				q = q / r;
			...
			if ( iter == 10 ) {
				...
				s = noMath::Fabs( H[n][n-1] ) + noMath::Fabs( H[n-1][n-2] );
				x = y = 0.75f * s;
				w = -0.4375f * s * s;
			}
			...
			for( m = n-2; m >= l; m-- ) {
				...
				q = H[m+1][m+1] - z - r - s;
				r = H[m+2][m+1];
				s = noMath::Fabs( p ) + noMath::Fabs( q ) + noMath::Fabs( r );
				p = p / s;
				q = q / s;
				r = r / s;
				if ( m == l ) {
				...
				if ( noMath::Fabs( H[m][m-1] ) * ( noMath::Fabs( q ) + noMath::Fabs( r ) ) <
						eps * ( noMath::Fabs( p ) * ( noMath::Fabs( H[m-1][m-1] ) + noMath::Fabs( z ) + noMath::Fabs( H[m+1][m+1] ) ) ) ) {
					break;
				}
			}
			...
			for ( k = m; k <= n-1; k++ ) {
				...
				if ( k != m ) {
					...
					x = noMath::Fabs( p ) + noMath::Fabs( q ) + noMath::Fabs( r );
					if ( x != 0.0f ) {
						p = p / x;
						q = q / x;
						r = r / x;
	...
	for ( n = numRows-1; n >= 0; n-- ) {
		...
		if ( q == 0.0f ) {		// real vector
			...
			for ( i = n-1; i >= 0; i-- ) {
				...
				if ( imaginaryEigenValues[i] < 0.0f ) {
					...
					if ( imaginaryEigenValues[i] == 0.0f ) {
						...
						t = ( x * s - z * r ) / q;
						H[i][n] = t;
						if ( noMath::Fabs(x) > noMath::Fabs( z ) ) {
							H[i+1][n] = ( -r - w * t ) / x;
						} else {
							H[i+1][n] = ( -s - y * t ) / z;
						}
			...
			if ( noMath::Fabs( H[n][n-1] ) > noMath::Fabs( H[n-1][n] ) ) {
				H[n-1][n-1] = q / H[n][n-1];
			...
			for ( i = n-2; i >= 0; i-- ) {
				...
				if ( imaginaryEigenValues[i] < 0.0f ) {
					...
					if ( imaginaryEigenValues[i] == 0.0f ) {
						...
						if ( vr == 0.0f && vi == 0.0f ) {
							vr = eps * norm * ( noMath::Fabs( w ) + noMath::Fabs( q ) + noMath::Fabs( x ) + noMath::Fabs( y ) + noMath::Fabs( z ) );
						}
						ComplexDivision( x * r - z * ra + q * sa, x * s - z * sa - q * ra, vr, vi, H[i][n-1], H[i][n] );
						if ( noMath::Fabs( x ) > ( noMath::Fabs( z ) + noMath::Fabs( q ) ) ) {
					...
					// overflow control
					t = Max( noMath::Fabs( H[i][n-1] ), noMath::Fabs( H[i][n] ) );
					if ( ( eps * t ) * t > 1 ) {
						for ( j = i; j <= n; j++ ) {
							H[j][n-1] = H[j][n-1] / t;
							H[j][n] = H[j][n] / t;
