...
//Bullet Physics »ç¿ëÇÔ..
//#ifdef FLT_EPSILON
//#undef FLT_EPSILON
//#endif


#define DEG2RAD(a)				( (a) * noMath::M_DEG2RAD )
#define RAD2DEG(a)				( (a) * noMath::M_RAD2DEG )

#define SEC2MS(t)				( noMath::FtoiFast( (t) * noMath::M_SEC2MS ) )
#define MS2SEC(t)				( (t) * noMath::M_MS2SEC )

#define	ANGLE2SHORT(x)			( noMath::FtoiFast( (x) * 65536.0f / 360.0f ) & 65535 )
#define	SHORT2ANGLE(x)			( (x) * ( 360.0f / 65536.0f ) )

#define	ANGLE2BYTE(x)			( noMath::FtoiFast( (x) * 256.0f / 360.0f ) & 255 )
...
public:
	...
	static bool					RealEqual( Real a, Real b, Real tolerance = std::numeric_limits<float>::epsilon());
	...
	static const float			F_EPSILON;				// smallest positive number such that 1.0+FLT_EPSILON != 1.0
...
NO_INLINE float noMath::RSqrt( float x ) {
...
NO_INLINE float noMath::InvSqrt16( float x ) {
...
NO_INLINE double noMath::InvSqrt64( float x ) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
NO_INLINE float noMath::Sqrt16( float x ) {
...
NO_INLINE double noMath::Sqrt64( float x ) {
	return x * InvSqrt64( x );
}

NO_INLINE float noMath::Sin( float a ) {
	return sinf( a );
}

NO_INLINE float noMath::Sin16( float a ) {
	float s;
...
NO_INLINE double noMath::Sin64( float a ) {
	return sin( a );
}

NO_INLINE float noMath::Cos( float a ) {
	return cosf( a );
}

NO_INLINE float noMath::Cos16( float a ) {
	float s, d;
...
NO_INLINE double noMath::Cos64( float a ) {
	return cos( a );
}

NO_INLINE void noMath::SinCos( float a, float &s, float &c ) {
#ifdef _WIN32
	_asm {
		fld		a
			fsincos
...
NO_INLINE void noMath::SinCos16( float a, float &s, float &c ) {
	float t, d;
...
NO_INLINE void noMath::SinCos64( float a, double &s, double &c ) {
#ifdef _WIN32
	_asm {
		fld		a
			fsincos
...
NO_INLINE float noMath::Tan( float a ) {
	return tanf( a );
}

NO_INLINE float noMath::Tan16( float a ) {
	float s;
	bool reciprocal;
...
NO_INLINE double noMath::Tan64( float a ) {
	return tan( a );
}

NO_INLINE float noMath::ASin( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
NO_INLINE float noMath::ASin16( float a ) {
...
NO_INLINE double noMath::ASin64( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
NO_INLINE float noMath::ACos( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
NO_INLINE float noMath::ACos16( float a ) {
	if ( FLOATSIGNBITSET( a ) ) {
		if ( a <= -1.0f ) {
			return PI;
		}
...
NO_INLINE double noMath::ACos64( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
NO_INLINE float noMath::ATan( float a ) {
	return atanf( a );
}

NO_INLINE float noMath::ATan16( float a ) {
	float s;
...
NO_INLINE double noMath::ATan64( float a ) {
	return atan( a );
}

NO_INLINE float noMath::ATan( float y, float x ) {
	return atan2f( y, x );
}

NO_INLINE float noMath::ATan16( float y, float x ) {
	float a, s;
...
NO_INLINE double noMath::ATan64( float y, float x ) {
	return atan2( y, x );
}

NO_INLINE float noMath::Pow( float x, float y ) {
	return powf( x, y );
}

NO_INLINE float noMath::Pow16( float x, float y ) {
	return Exp16( y * Log16( x ) );
}

NO_INLINE double noMath::Pow64( float x, float y ) {
	return pow( x, y );
}

NO_INLINE float noMath::Exp( float f ) {
	return expf( f );
}

NO_INLINE float noMath::Exp16( float f ) {
...
NO_INLINE double noMath::Exp64( float f ) {
	return exp( f );
}

NO_INLINE float noMath::Log( float f ) {
	return logf( f );
}

NO_INLINE float noMath::Log16( float f ) {
	int i, exponent;
	float y, y2;
...
NO_INLINE double noMath::Log64( float f ) {
	return log( f );
}

NO_INLINE int noMath::IPow( int x, int y ) {
	int r; for( r = x; y > 1; y-- ) { r *= x; } return r;
}

NO_INLINE int noMath::ILog2( float f ) {
	return ( ( (*reinterpret_cast<int *>(&f)) >> IEEE_FLT_MANTISSA_BITS ) & ( ( 1 << IEEE_FLT_EXPONENT_BITS ) - 1 ) ) - IEEE_FLT_EXPONENT_BIAS;
}

NO_INLINE int noMath::ILog2( int i ) {
	return ILog2( (float)i );
}

NO_INLINE int noMath::BitsForFloat( float f ) {
	return ILog2( f ) + 1;
}

NO_INLINE int noMath::BitsForInteger( int i ) {
	return ILog2( (float)i ) + 1;
}

NO_INLINE int noMath::MaskForFloatSign( float f ) {
	return ( (*reinterpret_cast<int *>(&f)) >> 31 );
}

NO_INLINE int noMath::MaskForIntegerSign( int i ) {
	return ( i >> 31 );
}

NO_INLINE int noMath::FloorPowerOfTwo( int x ) {
	return CeilPowerOfTwo( x ) >> 1;
}

NO_INLINE int noMath::CeilPowerOfTwo( int x ) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
NO_INLINE bool noMath::IsPowerOfTwo( int x ) {
	return ( x & ( x - 1 ) ) == 0 && x > 0;
}

NO_INLINE int noMath::BitCount( int x ) {
	x -= ( ( x >> 1 ) & 0x55555555 );
...
NO_INLINE int noMath::BitReverse( int x ) {
...
NO_INLINE int noMath::Abs( int x ) {
	int y = x >> 31;
	return ( ( x ^ y ) - y );
}
...
NO_INLINE float noMath::Floor( float f ) {
	return floorf( f );
}

NO_INLINE float noMath::Ceil( float f ) {
	return ceilf( f );
}

NO_INLINE float noMath::Rint( float f ) {
	return floorf( f + 0.5f );
}

NO_INLINE int noMath::Ftoi( float f ) {
	return (int) f;
}

NO_INLINE int noMath::FtoiFast( float f ) {
#ifdef _WIN32
	int i;
	__asm fld		f
...
NO_INLINE unsigned long noMath::Ftol( float f ) {
	return (unsigned long) f;
}

NO_INLINE unsigned long noMath::FtolFast( float f ) {
#ifdef _WIN32
...
NO_INLINE signed char noMath::ClampChar( int i ) {
	if ( i < -128 ) {
		return -128;
	}
	if ( i > 127 ) {
...
NO_INLINE signed short noMath::ClampShort( int i ) {
	if ( i < -32768 ) {
		return -32768;
	}
	if ( i > 32767 ) {
...
NO_INLINE int noMath::ClampInt( int min, int max, int value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
NO_INLINE float noMath::ClampFloat( float min, float max, float value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
NO_INLINE float noMath::AngleNormalize360( float angle ) {
...
NO_INLINE float noMath::AngleNormalize180( float angle ) {
...
NO_INLINE float noMath::AngleDelta( float angle1, float angle2 ) {
	return AngleNormalize180( angle1 - angle2 );
}

NO_INLINE int noMath::FloatHash( const float *array, const int numFloats ) {
	int i, hash = 0;
	const int *ptr;
...
NO_INLINE bool noMath::RealEqual( Real a, Real b, Real tolerance )
{
