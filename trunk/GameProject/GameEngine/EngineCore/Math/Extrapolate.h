	...
	switch( extrapolationType & ~EXTRAPOLATION_NOSTOP ) {
		...
		case EXTRAPOLATION_ACCELSINE: {
			if ( !duration ) {
				...
				deltaTime = ( time - startTime ) / duration;
				s = ( 1.0f - noMath::Cos( deltaTime * noMath::HALF_PI ) ) * duration * 0.001f * noMath::SQRT_1OVER2;
				currentValue = startValue + deltaTime * baseSpeed + s * speed;
			}
			break;
		}
		case EXTRAPOLATION_DECELSINE: {
			if ( !duration ) {
				...
				deltaTime = ( time - startTime ) / duration;
				s = noMath::Sin( deltaTime * noMath::HALF_PI ) * duration * 0.001f * noMath::SQRT_1OVER2;
				currentValue = startValue + deltaTime * baseSpeed + s * speed;
			}
			break;
		}
...
ID_INLINE type idExtrapolate<type>::GetCurrentSpeed( float time ) const {
	...
	switch( extrapolationType & ~EXTRAPOLATION_NOSTOP ) {
		...
		case EXTRAPOLATION_ACCELSINE: {
			deltaTime = ( time - startTime ) / duration;
			s = noMath::Sin( deltaTime * noMath::HALF_PI );
			return baseSpeed + s * speed;
		}
		case EXTRAPOLATION_DECELSINE: {
			deltaTime = ( time - startTime ) / duration;
			s = noMath::Cos( deltaTime * noMath::HALF_PI );
			return baseSpeed + s * speed;
		}
		default: {
			return baseSpeed;
