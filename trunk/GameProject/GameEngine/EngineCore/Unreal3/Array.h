...
public:
	...
	typedef IndexType TIndexedContainerIterator::*PrivateBooleanType;
...
public:
	...
	/** conversion to "bool" returning TRUE if the iterator has not reached the last element. */
	typedef IndexType TIndexedContainerConstIterator::*PrivateBooleanType;
	operator PrivateBooleanType() const
	{
...
public:
	...
	void RemoveSwap( INT Index, INT Count, INT NumBytesPerElement  )
	{
		assert(Count >= 0);
		assert(Index >= 0); 
		assert(Index <= ArrayNum);
		...
		if( NumToCopy )
		{
			...
			(
				(BYTE*)this->GetAllocation() + (Index      ) * NumBytesPerElement,
				(BYTE*)this->GetAllocation() + (ArrayNum-Count) * NumBytesPerElement,
				NumToCopy * NumBytesPerElement
			);
		}
		ArrayNum -= Count;
...
public:
	...
	{
		assert(i>=0);
		assert(i<ArrayNum||(i==0 && ArrayNum==0)); // (i==0 && ArrayNum==0) is workaround for &MyArray(0) abuse
		assert(ArrayMax>=ArrayNum);
	...
	{
		assert(AllocatorInstance.GetAllocation());
		assert(c<ArrayNum);
		assert(ArrayMax>=ArrayNum);
	...
	void BulkSerialize(FArchive& Ar)
	{
		...
		SerializedElementSize = sizeof(ElementType);
		...
		else 
		{
			...
			if( Ar.IsLoading() )
			{
				...
				Empty( NewArrayNum );
				Add( NewArrayNum );
				Ar.Serialize( GetData(), NewArrayNum * sizeof(ElementType) );
			}
		}
#endif
	}
	...
	void RemoveSwap( INT Index, INT Count=1 )
	{
		...
		if( NumToCopy )
		{
			...
			(
				(BYTE*)AllocatorInstance.GetAllocation() + (Index      ) * sizeof(ElementType),
				(BYTE*)AllocatorInstance.GetAllocation() + (ArrayNum-Count) * sizeof(ElementType),
				NumToCopy * sizeof(ElementType)
			);
		}
		ArrayNum -= Count;
	...
	INT RemoveItem( const ElementType& Item )
	{
		...
		assert( ((&Item) < GetTypedData()) || ((&Item) >= GetTypedData()+ArrayMax) );
	...
	{
		assert( ((&Item) < (ElementType*)AllocatorInstance.GetAllocation()) || ((&Item) >= (ElementType*)AllocatorInstance.GetAllocation()+ArrayMax) );
		const INT OriginalNum=ArrayNum;
		for( INT Index=0; Index<ArrayNum; Index++ )
		{
			if( (*this)(Index)==Item )
 ...
 * forced on/off.
 */
template<typename T,UBOOL DisableByteSwap,typename Allocator> 
class TArrayForceByteSwap : public TArray<T,Allocator>
{
public:	
	...
	friend FArchive& operator<<( FArchive& Ar, TArrayForceByteSwap& A )
	{
		...
		if( sizeof(T)==1 )
		{
			...
			{
				A.ArrayMax = A.ArrayNum;
				A.ResizeAllocation(0,A.ArrayMax,sizeof(T));
			}
			// toggle byte swapping of array elements
			const DWORD SavedForceSwap = Ar.ForceByteSwapping();
...
public:
	TArrayNoInit()
	: TArray<T, Allocator>(E_NoInit)
	{}
	TArrayNoInit& operator=( const TArrayNoInit& Other )
	{
		TArray<T, Allocator>::operator=(Other);
		return *this;
	}
	TArrayNoInit& operator=( const TArray<T, Allocator>& Other )
	{
		TArray<T, Allocator>::operator=(Other);
		return *this;
	}
...
template <typename T, typename Allocator> inline void Exchange( TArray<T, Allocator>& A, TArray<T, Allocator>& B )
{
	appMemswap( &A, &B, sizeof(TArray<T, Allocator>) );
}
...
public:
	...
	~TMRUArray()
	{
		...
		assert(this->ArrayMax>=this->ArrayNum);
...
public:
	...
	{
		assert(Index>=0);
		assert(Index<=this->ArrayNum);
		assert(Index+Count<=this->ArrayNum);
		for( INT i=Index; i<Index+Count; i++ )
		{
...
//	{
//		assert( ((&Item) < (T*)this->AllocatorInstance.GetAllocation()) || ((&Item) >= (T*)this->AllocatorInstance.GetAllocation()+this->ArrayMax) );
...
public:
	...
	void Remove(UINT BaseIndex,UINT Count = 1)
	{
		assert(Count >= 0);
		assert(BaseIndex >= 0); 
		assert(BaseIndex <= NumElements);
		assert(BaseIndex + Count <= NumElements);
		...
		assert(MaxElements >= NumElements);
	}
