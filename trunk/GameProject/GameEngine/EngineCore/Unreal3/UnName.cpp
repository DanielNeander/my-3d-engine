...
#else
	// appErrof(TEXT("Set SUPPORT_NAME_FLAGS to 1 in UnBuild.h if name flags are needed."));
	return 0;
#endif
}
/**
...
void FName::Hardcode(FNameEntry* AutoName)
{
	...
	{
		// appErrof( TEXT("Hardcoded name '%s' at index %i was duplicated. Existing entry is '%s'."), *AutoName->GetNameString(), AutoName->GetIndex(), *Names(AutoName->GetIndex())->GetNameString() );
	}
...
{
	assert(appStrlen(InName)<=NAME_SIZE);
	...
	for( FNameEntry* Hash=NameHash[iHash]; Hash; Hash=Hash->HashNext )
	{
		...
		||  (!bIsPureAnsi && Hash->IsEqual( InName )) )
		{
			...
			if (FindType == FNAME_Replace)
			{
				...
				assert(appStrlen(InName) == Hash->GetNameLength());
				...
				if( bIsPureAnsi )
				{
					appStrcpy(Hash->GetAnsiName(),Hash->GetNameLength()+1, (ANSICHAR*)AnsiName);
				}
				else
				{
					appStrcpy(Hash->GetUniName(),Hash->GetNameLength()+1, (UNICHAR*)InName);
				}
			}
...
//		appStrcpy( E.GetUniName(), Str.Len()+1, (const UNICHAR*)*Str.Left(NAME_SIZE-1) );
//	}
//	else
//	{
...
public:
	...
	FNameEntry* Allocate( INT Size )
	{
		...
		assert( CurrentPoolEnd - CurrentPoolStart >= Size );
