...
{    
	...
	float2 texcoord0to1 = (inputPatch[0].origin + inputPatch[0].size/2.0)/g_HeightFieldSize * 10.f;
	...
	float4 patch_screenspace_center = mul(float4(patch_center_realigned, 1.0), g_mWorldViewProj);
                                    ...
                                    OutputPatch<DUMMY, 1> inputPatch )
{
	...
	vertexPosition.y+=g_WaterHeight;
	// writing output params
    output.position = mul(float4(vertexPosition, 1.0), g_mWorldViewProj);
...
float4 WaterPatchPS(PSIn_Diffuse input) : SV_Target
{
	...
	disturbance_eyespace=mul(float4(microbump_normal.x,0,microbump_normal.z,0),View);
	...
	float4 projected_waveheight = mul(float4(input.positionWS.x,input.positionWS.y,input.positionWS.z,1),g_mWorldViewProj);
	float waveheight_correction=-0.5*projected_waveheight.y/projected_waveheight.w;
	projected_waveheight = mul(float4(input.positionWS.x,-0.8,input.positionWS.z,1),g_mWorldViewProj);
	...
	float4 vertex_in_viewspace=mul(float4(input.positionWS,1),View);
	...
	refraction_depth=g_ZFar*g_ZNear/(g_ZFar-refraction_depth*(g_ZFar-g_ZNear));
	vertex_in_viewspace=mul(float4(input.positionWS,1),View);
	water_depth=refraction_depth-vertex_in_viewspace.z;
