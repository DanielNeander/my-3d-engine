    ...
    break;
      ...
      PAINTSTRUCT ps;
        ...
        if (m_Buffer.size() > 1)
        {
		  ...
		  if(m_Buffer.size() >= PageSize)
		  {
			  std::vector<std::string>::iterator beg = m_Buffer.begin() + StartIndex;
			  std::vector<std::string>::iterator end = m_Buffer.begin() + StartIndex+PageSize;
		  ...
		  {
			  std::vector<std::string>::iterator beg = m_Buffer.begin() + StartIndex;
			  std::vector<std::string>::iterator end = m_Buffer.begin() + StartIndex + m_Buffer.size();
