...
namespace VCloud
{
	/** SkyX class
		...
		@remarks How to update SkyX:
				 ...
				 Both updates can be automatically performed by using listeners: the FrameListener is used
				 for the per-frame udpate, and the RenderTargetListener is used for the per-camera update.
				 For more information have a look to sample projects.
			     
     */
	class VCloudSkySystem : public Singleton<VCloudSkySystem>
	{
	public:
		/** Render queue groups
		 */
		struct RenderQueueGroups
		{
			 ...
			 */
			inline RenderQueueGroups(const uint8& s, const uint8& vc, const uint8& vclu, const uint8& vclo)
        ...
        /** Update (to be invoked per frame)
            ...
            @remarks Invoke this function only one time per frame.
					 ...
					 just register SkyX in ogre root through Root::addFrameListener(...)
         */
		...
		/** Notify camera render (to be invoked per camera and per frame)
			...
			@remarks Invoke this method manually before the camera render operation
			         ...
			         Also it's possible to use listeners, making all this process transparent,
					 just register SkyX in your render target through RenderTarget::addListener(...)
		 */
		void RenderSky(BaseCamera* c);
		 ...
		 */
		inline void setTimeMultiplier(const float& TimeMultiplier)
		{
		 ...
		 */
		inline const float& getTimeMultiplier() const
		{
		 ...
		 */
		inline SkyDomeMesh* getMeshManager()
		{
			return mMeshManager;
		}
		 ...
		 */
		inline void setInfiniteCameraFarClipDistance(const float& d)
		{
		 ...
		 */
		inline const float& getInfiniteCameraFarClipDistance() const
		{
		 ...
		 */
		inline const float& _getTimeOffset() const
		{
			return mTimeOffset;
		}

		ShaderID mSkydomeShader;		
		ShaderID	mCloudShader;
		TextureID mDiffuseTex,mNormalTex, mTileTex;
		TextureID mStarfieldTex;
		SamplerStateID	linearWarp;
		VertexFormatID	mCloudVF;	// 버텍스 버퍼 , 인덱스 버퍼는 스카이돔과 공유.
	private:
		/// Is SkyX created?
		bool mCreated;

		/// Mesh manager
		SkyDomeMesh* mMeshManager;
		/// Atmosphere manager
		...
		noVec3 mLastCameraPosition;
		/// Last camera far clip distance
		float mLastCameraFarClipDistance;
		/// Infinite camera far clip distance
		float mInfiniteCameraFarClipDistance;
