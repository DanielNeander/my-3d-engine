/*$T navigationcell.h GC 1.140 01/03/12 01:39:30 */

/*
 * Copyright (C) Greg Snook, 2000. All rights reserved worldwide. This software is
 * provided "as is" without express or implied warranties. You may freely copy and
 * compile this source into applications you distribute provided that the
...
/**
 *  \ NavigationCell.h NavigationCell component interface for the Navimesh sample program. Included as part of the Game
 *  Programming Gems sample code. Created 3/18/00 by Greg Snook greg@mightystudios.com Notes/Revisions: \
 */
#ifndef noPlane_H
...
/*
 * NavigationCell A NavigationCell represents a single triangle within a
 * NavigationMesh. It contains functions for testing a path against the cell, and
 * various ways to resolve collisions with the cell walls. Portions of the A* path
 * finding algorythm are provided within this class as well, but the path finding
 * process is managed by the parent Navigation Mesh.
 */
...
public:
	...
	/* MUTATORS */
	void						Initialize(const noVec3 &PointA, const noVec3 &PointB, const noVec3 &PointC);
	void						ComputeCellData();
	bool						RequestLink(const noVec3 &PointA, const noVec3 &PointB, NavigationCell *Caller);
	void						SetLink(CELL_SIDE Side, NavigationCell *Caller);

	PATH_RESULT					ClassifyPathToCell
								(
									const Line2D	&MotionPath,
									NavigationCell	**pNextCell,
									CELL_SIDE		&Side,
									noVec2			*pPointOfIntersection
								) const;

	void						ProjectPathOnCellWall(CELL_SIDE SideNumber, Line2D &MotionPath) const;

	void						MapVectorHeightToCell(noVec3 &MotionPoint) const;

	bool						ForcePointToCellCollumn(noVec3 &TestPoint) const;
	bool						ForcePointToCellCollumn(noVec2 &TestPoint) const;
	bool						ForcePointToWallInterior(CELL_SIDE SideNumber, noVec2 &TestPoint) const;
	bool						ForcePointToWallInterior(CELL_SIDE SideNumber, noVec3 &TestPoint) const;

	bool						ProcessCell(NavigationHeap *pHeap);
	bool						QueryForPath(NavigationHeap *pHeap, NavigationCell *Caller, float arrivalcost);

	/* ACCESSORS */
	bool						IsPointInCellCollumn(const noVec3 &TestPoint) const;
	bool						IsPointInCellCollumn(const noVec2 &TestPoint) const;
	const noVec3				&Vertex(int Vert) const;
	const noVec3				&CenterPoint() const;
	NavigationCell				*Link(int Side) const;

	const noVec3				&LinkPoint() const;
	...
	const noVec3				WallMidpoint(int Side) const;
...
private:
	...
	/* DATA */
	noPlane			m_CellnoPlane;	/* A noPlane containing the cell triangle */
	noVec3			m_Vertex[3];	/* pointers to the verticies of this triangle held in the NavigationMesh's vertex
									 * pool */
	noVec3			m_CenterPoint;	/* The center of the triangle */
	...
	noVec3			m_WallMidpoint[3];	/* the pre-computed midpoint of each wall. */
	float			m_WallDistance[3];	/* the distances between each wall midpoint of sides (0-1, 1-2, 2-0) */

	/* HELPER FUNCTIONS */
	void			ComputeHeuristic(const noVec3 &Goal);	/* estimates the distance to the goal for A* */
...
inline NavigationCell &NavigationCell::operator=(const NavigationCell &Src)
{
	if(this != &Src)
	{
		m_CellnoPlane = Src.m_CellnoPlane;
...
/*
 ...
 =======================================================================================================================
 */
inline void NavigationCell::Initialize(const noVec3 &PointA, const noVec3 &PointB, const noVec3 &PointC)
{
...
inline void NavigationCell::ComputeCellData()
{
	...
	noVec2 Point1(m_Vertex[VERT_A].x, m_Vertex[VERT_A].z);
	noVec2 Point2(m_Vertex[VERT_B].x, m_Vertex[VERT_B].z);
	noVec2 Point3(m_Vertex[VERT_C].x, m_Vertex[VERT_C].z);
	...
	m_CellnoPlane.FromPoints(m_Vertex[VERT_A], m_Vertex[VERT_B], m_Vertex[VERT_C]);
...
/*
 ...
 =======================================================================================================================
 */
inline bool NavigationCell::RequestLink(const noVec3 &PointA, const noVec3 &PointB, NavigationCell *Caller)
{
...
/*
 =======================================================================================================================
    ...
    * Uses the X and Z information of the vector to calculate Y on the cell noPlane ;
    * ://
 =======================================================================================================================
 */
inline void NavigationCell::MapVectorHeightToCell(noVec3 &MotionPoint) const
{
	/*
	 * Ax + By + Cz + D = 0 ;
	 * By = -(Ax + Cz + D) ;
	 * y = -(Ax + Cz + D)/B
	 */
	if(m_CellnoPlane.Normal().y != 0.f)
	{
		MotionPoint.y =(-(m_CellnoPlane.Normal().x * MotionPoint.x + m_CellnoPlane.Normal().z * MotionPoint.z + -m_CellnoPlane.Dist()) / 
			m_CellnoPlane.Normal().y );
	}
}

/*
 =======================================================================================================================
    ACCESSORS : IsPointInCellCollumn ;
 ...
 =======================================================================================================================
 */
inline bool NavigationCell::IsPointInCellCollumn(const noVec2 &TestPoint) const
{

	/*
	 * we are "in" the cell if we are on the right hand side of all edge lines of the
	 * cell
	 */
	int InteriorCount = 0;
...
/*
 ...
 =======================================================================================================================
 */
inline bool NavigationCell::IsPointInCellCollumn(const noVec3 &TestPoint) const
{
	noVec2 TestPoint2D(TestPoint.x, TestPoint.z);
...
/*
 ...
 =======================================================================================================================
 */
inline const noVec3 &NavigationCell::Vertex(int Vert) const
{
...
/*
 ...
 =======================================================================================================================
 */
inline const noVec3 &NavigationCell::CenterPoint() const
{
...
/*
 ...
 =======================================================================================================================
 */
inline const noVec3 NavigationCell::WallMidpoint(int Side) const
{
