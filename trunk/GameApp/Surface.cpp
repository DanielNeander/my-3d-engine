...
Surface::Surface( int a_Width, int a_Height, unsigned long* a_Buffer, int a_Pitch ) :
m_Width( a_Width ),
	m_Height( a_Height ),
	m_Buffer( a_Buffer ),
	m_Pitch( a_Pitch )
...
m_Width( a_Width ),
	...
	m_Pitch( a_Width )
{
	m_Buffer = new unsigned long[a_Width * a_Height];
}
...
void Surface::Clear( unsigned long a_Color )
{
	unsigned long* a = m_Buffer;
...
{
	unsigned long* dst = a_Dst->GetBuffer();
	unsigned long* src = m_Buffer;
	if ((src) && (dst)) 
	{
...
void Surface::Print( char* a_String, int x1, int y1, unsigned long color )
{
	unsigned long* t = m_Buffer + x1 + y1 * m_Pitch;
	int i;
...
void Surface::Line( float x1, float y1, float x2, float y2, unsigned long c )
{
...
void Surface::Plot( int x, int y, unsigned long c )
{
	if ((x >= 0) && (y >= 0) && (x < GetApp()->getWidth()) && (y < GetApp()->getHeight()))
	{
		m_Buffer[x + y * GetApp()->getWidth()] = c;
	}
}
