...
UBOOL noAnimNode::bNodeSearching = FALSE;
INT noAnimNode::CurrentSearchTag = 0;
 ...
 */
void noAnimNode::InitAnim( USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent )
{
 ...
 */
void noAnimNode::FillWithRefPose(TArray<FBoneAtom>& OutAtoms, const TArray<BYTE>& DesiredBones, const TArray<struct FMeshBone>& RefSkel)
{
	assert( OutAtoms.Num() == RefSkel.Num() );
 ...
 */
void noAnimNode::EnsureParentsPresent(TArray<BYTE>& BoneIndices, USkeletalMesh* SkelMesh)
{
	...
	while( i<BoneIndices.Num() )
	{
		...
		if( BoneIndex > 0 )
		{
			...
			{
				// debugf(TEXT("noAnimNode::EnsureParentsPresent, BoneIndex >= SkelMesh->RefSkeleton.Num()."));
				i++;
				continue;
			}
 ...
 */
void noAnimNode::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
 ...
 */
UBOOL noAnimNode::GetCachedResults(TArray<FBoneAtom>& OutAtoms, FBoneAtom& OutRootMotionDelta, INT& bOutHasRootMotion)
{
...
UBOOL noAnimNode::ShouldSaveCachedResults()
{
...
void noAnimNode::SaveCachedResults(const TArray<FBoneAtom>& NewAtoms, const FBoneAtom& NewRootMotionDelta, INT bNewHasRootMotion)
{
	assert(SkelComponent);
 ...
 */
void noAnimNode::GetNodesInternal(TArray<noAnimNode*>& Nodes)
{
	// make sure we're only called from inside GetNodes() or another GetNodesInternal()
	assert(noAnimNode::bNodeSearching);

	if (SearchTag != noAnimNode::CurrentSearchTag)
	{
		SearchTag = noAnimNode::CurrentSearchTag;
		Nodes.AddItem(this);
	}
}
...
//void noAnimNode::GetNodesByClass(TArray<noAnimNode*>& Nodes, UClass* BaseClass)
//{
//	TArray<noAnimNode*> AllNodes;
//	GetNodes(AllNodes);
//
...
void noAnimNode::GetAnimSeqNodes(TArray<noAnimNodeSequence*>& Nodes, FName InSynchGroupName)
{
	TArray<noAnimNode*> AllNodes;
	GetNodes(AllNodes);
	...
	for (INT i = 0; i < AllNodes.Num(); i++)
	{
		noAnimNodeSequence* SeqNode = (noAnimNodeSequence*)(AllNodes(i));
...
noAnimNode* noAnimNode::FindAnimNode(FName InNodeName)
{
	TArray<noAnimNode*> Nodes;
	this->GetNodes( Nodes );
...
INT noAnimNode::CountNumParentsTicked()
{
...
UBOOL noAnimNode::IsChildOf(noAnimNode* Node)
{
	if( Node == NULL )
	{
		return FALSE;
...
void noAnimNode::PlayAnim(UBOOL bLoop, FLOAT Rate, FLOAT StartTime)
{}

void noAnimNode::StopAnim()
{}
 ...
 * @param Parent Parent blend node (will be NULL for root note).
 */
void noAnimNodeBlendBase::InitAnim( USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent)
{
...
void noAnimNodeBlendBase::BuildTickArray(TArray<noAnimNode*>& OutTickArray)
{
	// Then iterate over calling BuildTickArray on any children who have had all parents updated.
	for(INT i=0; i<Children.Num(); i++)
	{
		noAnimNode* ChildNode = Children(i).Anim;
		if( ChildNode )
		{
...
void noAnimNodeBlendBase::SetChildrenTotalWeightAccumulator(const INT Index)
{
...
void noAnimNodeBlendBase::OnCeaseRelevant()
{
 ...
 */
void noAnimNodeBlendBase::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	...
	if( bRelevant )
	{
		...
		{
			noAnimNode* ChildNode = Children(ChildIndex).Anim;
			if( ChildNode )
			{
 ...
 * @param Nodes Output array of AnimNode pointers.
 */
void noAnimNodeBlendBase::GetNodesInternal(TArray<noAnimNode*>& Nodes)
{
	// make sure we're only called from inside GetNodes() or another GetNodesInternal()
	assert(noAnimNode::bNodeSearching);
...
FORCEINLINE FLOAT noAnimNodeBlendBase::GetBlendWeight(FLOAT ChildWeight)
{
#if USEANIMBLENDTYPES
	switch( BlendType )
	{
	case ABT_Sinusoidal			: return Clamp<FLOAT>((appSin(ChildWeight * noMath::PI - noMath::HALF_PI) + 1.f) / 2.f, 0.f, 1.f); 
...
FORCEINLINE void noAnimNodeBlendBase::SetBlendTypeWeights()
{
#if USEANIMBLENDTYPES
	...
	for(INT i=0; i<Children.Num(); i++)
	{
		...
		if( !Children(i).bIsAdditive && Children(i).Weight > ZERO_ANIMWEIGHT_THRESH )
		{
			...
			assert(Children(i).BlendWeight >= 0 && Children(i).BlendWeight <= 1.f);
 ...
 */
void noAnimNodeBlendBase::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	//START_GETBONEATOM_TIMER
	...
	if( Abs(GetChildWeightTotal() - 1.f) > ZERO_ANIMWEIGHT_THRESH )
	{
		...
		for(INT i=0; i<Children.Num(); i++)
		{
			// debugf( TEXT("Child: %d Weight: %f bIsAdditive: %d"), i, Children(i).Weight, Children(i).bIsAdditive );
	...
	for(INT i=0; i<Children.Num(); i++)
	{
 	...
 	assert(LastChildNonAddIndex != INDEX_NONE);
 ...
 */
void noAnimNodeBlendBase::GetMirroredBoneAtoms(TArray<FBoneAtom>& Atoms, INT ChildIndex, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
#if 0
...

		{
			...
			for(INT i=0; i<DesiredBones.Num(); i++)
			{
				...
				if( BoneIndex == 0 )
				{
					...
					if( bHasRootMotion && (!RMD.Translation.IsZero() || Square(RMD.Rotation.w) < 1.f - DELTA * DELTA) )
					{
						FQuatRotationTranslationMatrix RootTM(RMD.Rotation, RMD.Translation);
 ...
 */
FLOAT noAnimNodeBlendBase::GetChildWeightTotal()
{
	FLOAT TotalWeight = 0.f;
 ...
 */ 
void noAnimNodeBlendBase::OnChildAnimEnd(noAnimNodeSequence* Child, FLOAT PlayedTime, FLOAT ExcessTime) 
{ 
	for(INT i=0; i<ParentNodes.Num(); i++)
	{
...
void noAnimNodeBlendBase::PlayAnim(UBOOL bLoop, FLOAT Rate, FLOAT StartTime)
{
...
void noAnimNodeBlendBase::StopAnim()
{
	// pass on the call to our children
	for (INT i = 0; i < Children.Num(); i++)
	{
...
void noAnimNodeBlendBase::RenameChildConnectors()
{
	for(INT i=0; i<Children.Num(); i++)
	{
...
void noAnimNodeBlendBase::OnAddChild(INT ChildNum)
{
...
void noAnimNodeBlendBase::OnRemoveChild(INT ChildNum)
{
	// Make sure name matches position in array.
	RenameChildConnectors();
}
...
/** @see noAnimNode::TickAnim */
void noAnimNodeBlend::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	...
	noAnimNodeBlendBase::TickAnim(DeltaSeconds, TotalWeight);
}
 ...
 */
void noAnimNodeBlend::SetBlendTarget(FLOAT BlendTarget, FLOAT BlendTime)
{
...
/** @see noAnimNode::InitAnim */
void noAnimNodeCrossfader::InitAnim( USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent )
{
	noAnimNodeBlend::InitAnim( meshComp, Parent );
	
	noAnimNodeSequence	*ActiveChild = GetActiveChild();
...
/** @see noAnimNode::TickAnim */
void noAnimNodeCrossfader::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
	...
	// Tick AnimNodeBlend
	noAnimNodeBlend::TickAnim( DeltaSeconds, TotalWeight );
}

/** @see AnimCrossfader::GetAnimName */
FName noAnimNodeCrossfader::GetAnimName()
{
	noAnimNodeSequence	*ActiveChild = GetActiveChild();
	if( ActiveChild )
	{
 ...
 */
noAnimNodeSequence *noAnimNodeCrossfader::GetActiveChild()
{
	...
	return (noAnimNodeSequence*)(Child2WeightTarget < 0.5f ? Children(0).Anim : Children(1).Anim);
}

/** @see AnimNodeCrossFader::PlayOneShotAnim */
//void noAnimNodeCrossfader::execPlayOneShotAnim( FFrame& Stack, RESULT_DECL )
//{
...
//	}
//
//	noAnimNodeSequence*	Child = (noAnimNodeSequence*)(Child2WeightTarget < 0.5f ? Children(1).Anim : Children(0).Anim);
//
//	if( Child )
//	{
...
//void noAnimNodeCrossfader::execBlendToLoopingAnim( FFrame& Stack, RESULT_DECL )
//{
//	P_GET_NAME(AnimSeqName);
//	P_GET_FLOAT_OPTX(BlendInTime,0.f);
//	P_GET_FLOAT_OPTX(Rate,1.f);
 ...
 */
void noAnimNodeBlendPerBone::SetChildrenTotalWeightAccumulator(const INT Index)
{
	if( Index == 0 )
	{
		// Update the weight of this connection
...
//void noAnimNodeBlendPerBone::PostEditChange(UProperty* PropertyThatChanged)
//{
...
void noAnimNodeBlendPerBone::BuildWeightList()
{
...
void noAnimNodeBlendPerBone::InitAnim(USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent)
{
	noAnimNodeBlend::InitAnim(meshComp, Parent);
...
/** @see noAnimNode::GetBoneAtoms. */
void noAnimNodeBlendPerBone::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	START_GETBONEATOM_TIMER
	...
	for(INT i=0; i<DesiredBones.Num(); i++)
	{
		...
		if( !bForceLocalSpaceBlend && bDoComponentSpaceBlend )
		{
			...
			FMatrix ParentTM = mat4_identity;
			if( BoneIndex > 0 )
			{
 ...
 * @see noAnimNode::TickAnim
 */
void noAnimNodeBlendDirectional::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
	assert(Children.Num() == 4);

	// Calculate DirAngle based on player velocity.
	GameObject* actor = SkelComponent->GetGameObject(); // Get the actor to use for acceleration/look direction etc.
	if( actor )
	{
		FLOAT TargetDirAngle = 0.f;
		FVector	VelDir = actor->GetVelocity();
		VelDir.z = 0.0f;
		...
		else
		{
			...
			FVector LookDir = actor->GetRotation().mat[0];
			LookDir.z = 0.f;
			LookDir = LookDir.SafeNormal();

			FVector LeftDir = LookDir.Cross(FVector(0.f,0.f,1.f));
		...
		if( DeltaDir != 0.f )
		{
			FLOAT MaxDelta = DeltaSeconds * DirDegreesPerSecond * (noMath::PI/180.f);
	...
	if(SkelComponent->PredictedLODLevel < SingleAnimAtOrAboveLOD)
	{
		...
		if(DirAngle < -0.5f*noMath::PI) // Back and left
		{
			Children(2).Weight = (DirAngle/(0.5f*noMath::PI)) + 2.f;
		...
		{
			Children(2).Weight = -DirAngle/(0.5f*noMath::PI);
			Children(3).Weight = 0.f;

			Children(0).Weight = 1.f - Children(2).Weight;
			Children(1).Weight = 0.f;
		}
		else if(DirAngle < 0.5f*noMath::PI) // Forward and right
		{
			Children(2).Weight = 0.f;
			Children(3).Weight = DirAngle/(0.5f*noMath::PI);
		...
		{
			Children(2).Weight = 0.f;
			Children(3).Weight = (-DirAngle/(0.5f*noMath::PI)) + 2.f;
	...
	else
	{
		...
		if(DirAngle < -0.75f*noMath::PI) // Back
		{
			Children(1).Weight = 1.f;
		}
		else if(DirAngle < -0.25f*noMath::PI) // Left
		{
			Children(2).Weight = 1.f;
		}
		else if(DirAngle < 0.25f*noMath::PI) // Forward
		{
			Children(0).Weight = 1.f;
		}
		else if(DirAngle < 0.75f*noMath::PI) // Right
		{
 ...
 * @see noAnimNode::InitAnim
 */
void noAnimNodeBlendList::InitAnim( USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent )
{
	noAnimNodeBlendBase::InitAnim(meshComp, Parent);
...
/** @see noAnimNode::TickAnim */
void noAnimNodeBlendList::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	assert(Children.Num() == TargetWeight.Num());
 ...
 */
void noAnimNodeBlendList::SetActiveChild( INT ChildIndex, FLOAT BlendTime )
{
	assert(Children.Num() == TargetWeight.Num());
	
	if( ChildIndex < 0 || ChildIndex >= Children.Num() )
	{
		// debugf( TEXT("noAnimNodeBlendList::SetActiveChild : %s ChildIndex (%d) outside number of Children (%d)."), *GetName(), ChildIndex, Children.Num() );
		ChildIndex = 0;
	}
	...
	if( bPlayActiveChild )
	{
		...
		noAnimNodeSequence* AnimSeq = (noAnimNodeSequence*)(Children(ActiveChildIndex).Anim);
		if( AnimSeq )
		{
...
FLOAT noAnimNodeBlendList::GetSliderPosition(INT SliderIndex, INT ValueIndex)
{
	assert(0 == SliderIndex && 0 == ValueIndex);
	return SliderPosition;
}

void noAnimNodeBlendList::HandleSliderMove(INT SliderIndex, INT ValueIndex, FLOAT NewSliderValue)
{
	assert(0 == SliderIndex && 0 == ValueIndex);
...
FString noAnimNodeBlendList::GetSliderDrawValue(INT SliderIndex)
{
	assert(0 == SliderIndex);
...
void noAnimNodeBlendList::OnAddChild(INT ChildNum)
{
	noAnimNodeBlendBase::OnAddChild(ChildNum);
...
void noAnimNodeBlendList::OnRemoveChild(INT ChildNum)
{
	noAnimNodeBlendBase::OnRemoveChild(ChildNum);
 ...
 */
void noAnimNodeBlendByPosture::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
...
/************************************************************************************
 * noAnimNodeBlendBySpeed
 ***********************************************************************************/

/**
 * Resets the last channel on becoming active.	
 */
void noAnimNodeBlendBySpeed::OnBecomeRelevant()
{
	//noAnimNodeBlendList::OnBecomeRelevant();
	LastChannel = -1;
}
 ...
 * @param DeltaSeconds	Time since last tick in seconds.
 */
void noAnimNodeBlendBySpeed::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
	...
	{
		// debugf(TEXT("noAnimNodeBlendBySpeed::TickAnim - Need at least two children"));
	}
	else if( !SufficientConstraints )
	{
		// debugf(TEXT("noAnimNodeBlendBySpeed::TickAnim - Number of constraints (%i) is lower than number of children! (%i)"), Constraints.Num(), NumChannels);
	}
	
	// Update AnimNodeBlendList
	noAnimNodeBlendList::TickAnim(DeltaSeconds, TotalWeight);				
}
 ...
 */
FLOAT noAnimNodeBlendBySpeed::CalcSpeed()
{
...
void noAnimNodeMirror::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	START_GETBONEATOM_TIMER
...
void UAnimTree::InitAnim(USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent)
{
	noAnimNodeBlendBase::InitAnim(meshComp, Parent);
 ...
 */
static inline FLOAT GetNodeRelativePosition(noAnimNodeSequence* SeqNode)
{
 ...
 * Taking into account node's relative offset.
 */
static inline FLOAT FindNodePositionFromRelative(noAnimNodeSequence* SeqNode, FLOAT RelativePosition)
{
...
void UAnimTree::RepopulateAnimGroups()
{
	if( Children(0).Anim )
	{
		// get all noAnimNodeSequence children and cache them to avoid over casting
		TArray<noAnimNodeSequence*> Nodes;
		...
		for( INT GroupIdx=0; GroupIdx<AnimGroups.Num(); GroupIdx++ )
		{
			...
			if( AnimGroup.GroupName != NAME_None )
			{
				for( INT i=0; i<Nodes.Num(); i++ )
				{
					noAnimNodeSequence* SeqNode = Nodes(i);
...
static inline UBOOL IsAnimNodeRelevantForSynchGroup(const noAnimNodeSequence* SeqNode)
{
...
static inline UBOOL IsAnimNodeRelevantForNotifyGroup(const noAnimNodeSequence* SeqNode)
{
...
void UAnimTree::UpdateMasterNodesForGroup(FAnimGroup& AnimGroup)
{
	...
	noAnimNodeSequence* OldSynchMaster = AnimGroup.SynchMaster;
	...
	{
		noAnimNodeSequence* Candidate = AnimGroup.SeqNodes(i);
...
void UAnimTree::UpdateAnimNodeSeqGroups(FLOAT DeltaSeconds)
{
	...
	for(INT GroupIdx=0; GroupIdx<GroupsCount; GroupIdx++)
	{
		...
		if( !IsAnimNodeRelevantForSynchGroup(AnimGroup.SynchMaster) )
		{
			...
			{
				noAnimNodeSequence* SeqNode = AnimGroup.SeqNodes(i);
		...
		{
			noAnimNodeSequence* SynchMaster	= AnimGroup.SynchMaster;
...
UBOOL UAnimTree::SetAnimGroupForNode(class noAnimNodeSequence* SeqNode, FName GroupName, UBOOL bCreateIfNotFound)
{
	if( !SeqNode )
	{	
		return FALSE;
...
noAnimNodeSequence* UAnimTree::GetGroupSynchMaster(FName GroupName)
{
	const INT GroupIndex = GetGroupIndex(GroupName);
	if( GroupIndex != INDEX_NONE )
	{
...
noAnimNodeSequence* UAnimTree::GetGroupNotifyMaster(FName GroupName)
{
	const INT GroupIndex = GetGroupIndex(GroupName);
	if( GroupIndex != INDEX_NONE )
	{
...
//void UAnimTree::CopyAnimNodes(const TArray<noAnimNode*>& SrcNodes, UObject* NewOuter, TArray<noAnimNode*>& DestNodes, TMap<noAnimNode*,noAnimNode*>& SrcToDestNodeMap)
//{
...
//	{
//		noAnimNode* NewNode = ConstructObject<noAnimNode>( SrcNodes(i)->GetClass(), NewOuter, NAME_None, 0, SrcNodes(i) );
...
//	{
//		// Only noAnimNodeBlendBase classes have references to other AnimNodes.
//		noAnimNodeBlendBase* NewBlend = Cast<noAnimNodeBlendBase>( DestNodes(i) );
//		if(NewBlend)
//		{
...
//				{
//					noAnimNode** NewNode = SrcToDestNodeMap.Find(NewBlend->Children(j).Anim);
//					if(NewNode)
//					{
//						assert(*NewNode);
...
//	TArray<noAnimNode*> SrcNodes;
//	GetNodes(SrcNodes);
//	verify(SrcNodes.RemoveItem(this) > 0);
//
//	// Duplicate all the AnimNodes	
//	TArray<noAnimNode*> DestNodes; // Array of newly created nodes.
//	TMap<noAnimNode*,noAnimNode*> SrcToDestNodeMap; // Mapping table from src node to newly created node.
...
//	assert(NewTree->Children.Num() == 1);
//	if(NewTree->Children(0).Anim)
//	{
//		noAnimNode** NewNode = SrcToDestNodeMap.Find(NewTree->Children(0).Anim);
//		assert(NewNode && *NewNode); // When we copy the entire tree, we should always find the node we want.
 ...
 * Set a full weight on source, because it's potentially always feeding animations into the final blend.
 */
void noAnimNodeBlendMultiBone::SetChildrenTotalWeightAccumulator(const INT Index)
{
	if( Index == 0 )
	{
		// Update the weight of this connection
...
/** @see noAnimNode::InitAnim */
void noAnimNodeBlendMultiBone::InitAnim( USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent )
{
	noAnimNodeBlendBase::InitAnim(meshComp, Parent);
...
//void noAnimNodeBlendMultiBone::PostEditChange(UProperty* PropertyThatChanged)
//{
//	for( INT Idx = 0; Idx < BlendTargetList.Num(); Idx++ )
//	{
//		if( PropertyThatChanged && 
 ...
 */
void noAnimNodeBlendMultiBone::SetTargetStartBone( INT TargetIdx, FName StartBoneName, FLOAT PerBoneIncrease )
{
	...
	else
	{
		...
		{
			// debugf( TEXT("noAnimNodeBlendPerBone::SetTargetStartBone : StartBoneName (%s) not found."), *StartBoneName.ToString() );
			return;
		}
		...
		assert(PerBoneIncrease >= 0.0f && PerBoneIncrease <= 1.0f); // rather aggressive...
...
//void noAnimNodeBlendMultiBone::execSetTargetStartBone( FFrame& Stack, RESULT_DECL )
//{
//	P_GET_INT(TargetIdx);
...
/** @see noAnimNode::GetBoneAtoms. */
void noAnimNodeBlendMultiBone::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	START_GETBONEATOM_TIMER
...
void noAnimNodeAimOffset::PostAnimNodeInstance(noAnimNode* SourceNode)
{
	TemplateNode = dynamic_cast<noAnimNodeAimOffset*>(SourceNode);
	assert(TemplateNode);
...
//void noAnimNodeAimOffset::PostEditChange(UProperty* PropertyThatChanged)
//{
//	Super::PostEditChange(PropertyThatChanged);
//
...
void noAnimNodeAimOffset::InitAnim(USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent)
{
	noAnimNodeBlendBase::InitAnim(meshComp, Parent);
...
void noAnimNodeAimOffset::UpdateListOfRequiredBones()
{
	...
	noAnimNode::EnsureParentsPresent(RequiredBones, SkelComponent->SkeletalMesh);
}

FAimOffsetProfile* noAnimNodeAimOffset::GetCurrentProfile()
{
...
void noAnimNodeAimOffset::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	START_GETBONEATOM_TIMER
...
FLOAT noAnimNodeAimOffset::GetSliderPosition(INT SliderIndex, INT ValueIndex)
{
	assert(SliderIndex == 0);
...
void noAnimNodeAimOffset::HandleSliderMove(INT SliderIndex, INT ValueIndex, FLOAT NewSliderValue)
{
	assert(SliderIndex == 0);
	assert(ValueIndex == 0 || ValueIndex == 1);
...
FString noAnimNodeAimOffset::GetSliderDrawValue(INT SliderIndex)
{
	assert(SliderIndex == 0);
...
FQuat noAnimNodeAimOffset::GetBoneAimQuaternion(INT CompIndex, EAnimAimDir InAimDir)
{
	FAimOffsetProfile* P = GetCurrentProfile();
	if(!P)
	{
...
FVector noAnimNodeAimOffset::GetBoneAimTranslation(INT CompIndex, EAnimAimDir InAimDir)
{
	FAimOffsetProfile* P = GetCurrentProfile();
	if(!P)
	{
...
void noAnimNodeAimOffset::SetBoneAimQuaternion(INT CompIndex, EAnimAimDir InAimDir, FQuat InQuat)
{
	FAimOffsetProfile* P = GetCurrentProfile();
	if(!P)
	{
...
void noAnimNodeAimOffset::SetBoneAimTranslation(INT CompIndex, EAnimAimDir InAimDir, FVector InTrans)
{
	FAimOffsetProfile* P = GetCurrentProfile();
	if(!P)
	{
...
UBOOL noAnimNodeAimOffset::ContainsBone(const FName &BoneName)
{
	FAimOffsetProfile* P = GetCurrentProfile();
	if(!P)
	{
...
void noAnimNodeAimOffset::BakeOffsetsFromAnimations()
{
	if( !SkelComponent || !SkelComponent->SkeletalMesh )
	{
	...
	noAnimNodeSequence*	SeqNode = new noAnimNodeSequence;
...
void noAnimNodeAimOffset::ExtractOffsets(TArray<FBoneAtom>& RefBoneAtoms, TArray<FBoneAtom>& BoneAtoms, EAnimAimDir InAimDir)
{
	...
	for(INT i=0; i<BoneAtoms.Num(); i++)
	{
		...
		FMatrix TargetTranslationM = mat4_identity;
		TargetTranslationM.SetOrigin( TargetTM(i).GetOrigin() );

		FMatrix SourceTranslationM = mat4_identity;
...
INT noAnimNodeAimOffset::GetComponentIdxFromBoneIdx(const INT BoneIndex, UBOOL bCreateIfNotFound)
{
 ...
 */
UBOOL noAnimNodeAimOffset::ExtractAnimationData(noAnimNodeSequence *SeqNode, FName AnimationName, TArray<FBoneAtom>& BoneAtoms)
{
	//Set Animation
 ...
 */
void noAnimNodeAimOffset::SetActiveProfileByName(FName ProfileName)
{
	if(TemplateNode)
	{
 ...
 */
void noAnimNodeAimOffset::SetActiveProfileByIndex(INT ProfileIndex)
{
...
void noAnimNodeSynch::PostLoad()
{
	//noAnimNodeBlendBase::PostLoad();
}


/** Add a node to an existing group */
void noAnimNodeSynch::AddNodeToGroup(class noAnimNodeSequence* SeqNode, FName GroupName)
{
...
void noAnimNodeSynch::RemoveNodeFromGroup(class noAnimNodeSequence* SeqNode, FName GroupName)
{
	if( !SeqNode || GroupName == NAME_None )
	{
		return;
...
/** Force a group at a relative position. */
void noAnimNodeSynch::ForceRelativePosition(FName GroupName, FLOAT RelativePosition)
{
	for( INT GroupIdx=0; GroupIdx<Groups.Num(); GroupIdx++ )
	{
		FSynchGroup& SynchGroup = Groups(GroupIdx);
		if( SynchGroup.GroupName == GroupName )
		{
			...
			{
				noAnimNodeSequence* SeqNode = SynchGroup.SeqNodes(i);
				if( SeqNode && SeqNode->AnimSeq )
				{
					SeqNode->SetPosition(FindNodePositionFromRelative(SeqNode, RelativePosition), FALSE);
				}
...
/** Get the relative position of a group. */
FLOAT noAnimNodeSynch::GetRelativePosition(FName GroupName)
{
	for( INT GroupIdx=0; GroupIdx<Groups.Num(); GroupIdx++ )
	{
		FSynchGroup& SynchGroup = Groups(GroupIdx);
		if( SynchGroup.GroupName == GroupName )
		{
			...
			{
				// debugf(TEXT("noAnimNodeSynch::GetRelativePosition, no master node for group %s"), *SynchGroup.GroupName.ToString());
			}
		}
	}
...
noAnimNodeSequence* noAnimNodeSynch::GetMasterNodeOfGroup(FName GroupName)
{
	for(INT GroupIdx=0; GroupIdx<Groups.Num(); GroupIdx++ )
	{
...
/** Force a group at a relative position. */
void noAnimNodeSynch::SetGroupRateScale(FName GroupName, FLOAT NewRateScale)
{
	for(INT GroupIdx=0; GroupIdx<Groups.Num(); GroupIdx++ )
	{
		if( Groups(GroupIdx).GroupName == GroupName )
...
void noAnimNodeSynch::InitAnim(USkeletalMeshComponent* MeshComp, noAnimNodeBlendBase* Parent)
{
	// Rebuild cached list of nodes belonging to each group
	RepopulateGroups();
...
/** Look up AnimNodeSequences and cache Group lists */
void noAnimNodeSynch::RepopulateGroups()
{
	if( Children(0).Anim )
	{
		// get all noAnimNodeSequence children and cache them to avoid over casting
...
void noAnimNodeSynch::UpdateMasterNodeForGroup(FSynchGroup& SynchGroup)
{
	// start with our previous node. see if we can find better!
	noAnimNodeSequence*	MasterNode		= SynchGroup.MasterNode;	
...
/** The main synchronization code... */
void noAnimNodeSynch::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	noAnimNodeBlendBase::TickAnim(DeltaSeconds, TotalWeight);
	...
	for(INT GroupIdx=0; GroupIdx<Groups.Num(); GroupIdx++)
	{
		...
		if( SynchGroup.MasterNode && SynchGroup.MasterNode->AnimSeq )
		{
				...
				MasterNode->AnimSeq->SequenceLength > 0.f )
			{
				...
				for(INT i=0; i<SynchGroup.SeqNodes.Num(); i++)
				{
					noAnimNodeSequence *SlaveNode = SynchGroup.SeqNodes(i);
...
void noAnimNodeRandom::InitAnim(USkeletalMeshComponent* meshComp, noAnimNodeBlendBase* Parent)
{
#if DEBUG_ANIMNODERANDOM
	// debugf(TEXT("%3.2f noAnimNodeRandom::InitAnim"), GWorld->GetTimeSeconds());
#endif

	noAnimNodeBlendList::InitAnim(meshComp, Parent);	
...
void noAnimNodeRandom::OnAddChild(INT ChildNum)
{
	noAnimNodeBlendList::OnAddChild(ChildNum);
...
void noAnimNodeRandom::OnRemoveChild(INT ChildNum)
{
	noAnimNodeBlendList::OnRemoveChild(ChildNum);
...
void noAnimNodeRandom::OnChildAnimEnd(noAnimNodeSequence* Child, FLOAT PlayedTime, FLOAT ExcessTime)
{
#if DEBUG_ANIMNODERANDOM
	// debugf(TEXT("%3.2f noAnimNodeRandom::OnChildAnimEnd %s"), GWorld->GetTimeSeconds(), *Child->AnimSeqName.ToString());
#endif

	noAnimNodeBlendList::OnChildAnimEnd(Child, PlayedTime, ExcessTime);
...
INT noAnimNodeRandom::PickNextAnimIndex()
{
#if DEBUG_ANIMNODERANDOM
	// debugf(TEXT("%3.2f noAnimNodeRandom::PickNextAnimIndex"), GWorld->GetTimeSeconds());
#endif
...
void noAnimNodeRandom::PlayPendingAnimation(FLOAT BlendTime, FLOAT StartTime)
{
#if DEBUG_ANIMNODERANDOM
	// debugf(TEXT("%3.2f noAnimNodeRandom::PlayPendingAnimation. PendingChildIndex: %d, BlendTime: %f, StartTime: %f"), GWorld->GetTimeSeconds(), PendingChildIndex, BlendTime, StartTime);
#endif
...
#if DEBUG_ANIMNODERANDOM
		// debugf(TEXT("%3.2f noAnimNodeRandom::PlayPendingAnimation. PendingChildIndex not valid, picked: %d"), GWorld->GetTimeSeconds(), PendingChildIndex);
#endif
		// if our pending child index is STILL not valid, abort
		if( !(PendingChildIndex >= 0 && PendingChildIndex < Children.Num() && PendingChildIndex < RandomInfo.Num() && Children(PendingChildIndex).Anim) )
		{
#if DEBUG_ANIMNODERANDOM
			// debugf(TEXT("%3.2f noAnimNodeRandom::PlayPendingAnimation. PendingChildIndex still not valid, abort"), GWorld->GetTimeSeconds());
#endif
			return;
		}
	}
	...
	// Play the animation if this child is a sequence
	PlayingSeqNode = (noAnimNodeSequence*)(Children(ActiveChildIndex).Anim);
	if( PlayingSeqNode )
	{
		FRandomAnimInfo& Info = RandomInfo(ActiveChildIndex);
	...
	PendingChildIndex = PickNextAnimIndex();
#if DEBUG_ANIMNODERANDOM
	// debugf(TEXT("%3.2f noAnimNodeRandom::PlayPendingAnimation. Picked PendingChildIndex: %d"), GWorld->GetTimeSeconds(), PendingChildIndex);
#endif
}

void noAnimNodeRandom::OnBecomeRelevant()
{
	//noAnimNodeBlendList::OnBecomeRelevant();
	...
	if( ActiveChildIndex >=0 && ActiveChildIndex < RandomInfo.Num() )
	{
		...
		{
#if DEBUG_ANIMNODERANDOM
			// debugf(TEXT("%3.2f noAnimNodeRandom::OnBecomeRelevant. Starting a new animation."), GWorld->GetTimeSeconds());
#endif
...
void noAnimNodeRandom::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	...
	if( ActiveChildIndex >=0 && ActiveChildIndex < RandomInfo.Num() )
	{
		...
		if( ActiveChildIndex != PendingChildIndex )
		{
			...
			if( Info.BlendInTime > 0.f && PlayingSeqNode && PlayingSeqNode->AnimSeq )
			{
				...
				{
#if DEBUG_ANIMNODERANDOM
					// debugf(TEXT("%3.2f noAnimNodeRandom::TickAnim. Blending to pending animation. TimeLeft: %f"), GWorld->GetTimeSeconds(), TimeLeft);
#endif
 ...
 * @param DeltaSeconds	Time since last tick in seconds.
 */
void noAnimNodeBlendByPhysics::TickAnim( FLOAT DeltaSeconds, FLOAT TotalWeight )
{
...
/************************************************************************************
 * noAnimNodeBlendByBase
 ***********************************************************************************/

void noAnimNodeBlendByBase::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
...
/************************************************************************************
 * noAnimNodeScalePlayRate
 ***********************************************************************************/

void noAnimNodeScalePlayRate::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	if( Children(0).Anim )
	{
		TArray<noAnimNodeSequence*> SeqNodes;
...
FLOAT noAnimNodeScalePlayRate::GetScaleValue()
{
	return ScaleByValue;
}
...
/************************************************************************************
 * noAnimNodeScaleRateBySpeed
 ***********************************************************************************/

FLOAT noAnimNodeScaleRateBySpeed::GetScaleValue()
{
	/*AActor* Owner = SkelComponent ? SkelComponent->GetOwner() : NULL;
...
/************************************************************************************
 * noAnimNodePlayCustomAnim
 ***********************************************************************************/

/**
 * Main tick
 */
void noAnimNodePlayCustomAnim::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	// check for custom animation timing, to blend out before it is over.
	if( bIsPlayingCustomAnim && CustomPendingBlendOutTime >= 0.f )
	{
		noAnimNodeSequence *ActiveChild = (noAnimNodeSequence*)(Children(1).Anim);
		if( ActiveChild && ActiveChild->AnimSeq )
		{
 ...
 */
FLOAT noAnimNodePlayCustomAnim::PlayCustomAnim(FName AnimName, FLOAT Rate, FLOAT BlendInTime, FLOAT BlendOutTime, UBOOL bLooping, UBOOL bOverride)
{
	// Pre requisites
	...
	noAnimNodeSequence* Child = (noAnimNodeSequence*)(Children(1).Anim);
	if( Child )
	{
 ...
 * @param	bOverride		play same animation over again only if bOverride is set to true.
 */
void noAnimNodePlayCustomAnim::PlayCustomAnimByDuration(FName AnimName, FLOAT Duration, FLOAT BlendInTime, FLOAT BlendOutTime, UBOOL bLooping, UBOOL bOverride)
{
	// Pre requisites
 ...
 */
void noAnimNodePlayCustomAnim::StopCustomAnim(FLOAT BlendOutTime)
{
...
void noAnimNodeSlot::UpdateWeightsForAdditiveAnimations()
{
	...
	Children(0).Weight = 1.f - NonAdditiveWeight;
}
 ...
 * @see noAnimNode::InitAnim
 */
void noAnimNodeSlot::InitAnim(USkeletalMeshComponent* MeshComp, noAnimNodeBlendBase* Parent)
{
	noAnimNodeBlendBase::InitAnim(MeshComp, Parent);
...
void noAnimNodeSlot::MAT_SetAnimPosition(INT ChannelIndex, FName InAnimSeqName, FLOAT InPosition, UBOOL bFireNotifies, UBOOL bLooping)
{
	const INT ChildNum = ChannelIndex + 1;

	if( ChildNum >= Children.Num() )
	{
		// debugf(TEXT("noAnimNodeSlot::MAT_SetAnimPosition, invalid ChannelIndex: %d"), ChannelIndex);
		return;
	}

	noAnimNodeSequence* SeqNode = (noAnimNodeSequence*)(Children(ChildNum).Anim);
	if( SeqNode )
	{
...
void noAnimNodeSlot::MAT_SetAnimWeights(const FAnimSlotInfo& SlotInfo)
{
...
/** Rename all child nodes upon Add/Remove, so they match their position in the array. */
void noAnimNodeSlot::RenameChildConnectors()
{
 ...
 */
INT noAnimNodeSlot::FindBestChildToPlayAnim(FName AnimToPlay)
{
	...
	//{
	//	AActor* AOwner = SkelComponent ? SkelComponent->GetOwner() : NULL;
	//	// debugf(TEXT("noAnimNodeSlot::FindBestChildToPlayAnim - Best Index %d with a weight of %f, for Anim: %s and Owner: %s"), 
 ...
 * @param	bOverride		play same animation over again only if bOverride is set to true.
 */
FLOAT noAnimNodeSlot::PlayCustomAnim(FName AnimName, FLOAT Rate, FLOAT BlendInTime, FLOAT BlendOutTime, UBOOL bLooping, UBOOL bOverride)
{
	// Pre requisites
	if( AnimName == NAME_None || Rate <= 0.f )
	{
	...
	{
		// debugf(TEXT("noAnimNodeSlot::PlayCustomAnim, CustomChildIndex %d is out of bounds."), CustomChildIndex);
		return 0.f;
	}

	noAnimNodeSequence* SeqNode = (noAnimNodeSequence*)(Children(CustomChildIndex).Anim);
	if( SeqNode )
	{
		UBOOL bSetAnim = TRUE;
...
#if 0 // DEBUG
		noAnimNodeSequence* SeqNode = GetCustomAnimNodeSeq();
	...
	{
		// debugf(TEXT("noAnimNodeSlot::PlayCustomAnim, Child %d, is not hooked up to a AnimNodeSequence."), CustomChildIndex);
	}
 ...
 * @param	bOverride		play same animation over again only if bOverride is set to true.
 */
void noAnimNodeSlot::PlayCustomAnimByDuration(FName AnimName, FLOAT Duration, FLOAT BlendInTime, FLOAT BlendOutTime, UBOOL bLooping, UBOOL bOverride)
{
	// Pre requisites
	if( AnimName == NAME_None || Duration <= 0.f )
	{
	...
	{
		// debugf(TEXT("noAnimNodeSlot::PlayAnim - AnimSequence for %s not found"), *AnimName.ToString());
	}
}

/** Returns the Name of the currently played animation or NAME_None otherwise. */
FName noAnimNodeSlot::GetPlayedAnimation()
{
	noAnimNodeSequence* SeqNode = GetCustomAnimNodeSeq();
	if( SeqNode )
	{	
 ...
 * Used for blending out of a looping custom animation.
 */
void noAnimNodeSlot::StopCustomAnim(FLOAT BlendOutTime)
{
	if( bIsPlayingCustomAnim )
	{
#if 0 // DEBUG
 ...
 * Used for blending out of a looping custom animation.
 */
void noAnimNodeSlot::SetCustomAnim(FName AnimName)
{
	if( bIsPlayingCustomAnim )
	{
		noAnimNodeSequence* SeqNode = GetCustomAnimNodeSeq();
 ...
 */
void noAnimNodeSlot::SetActorAnimEndNotification(UBOOL bNewStatus)
{
	// Set all childs but the active one to FALSE. 
	// Active one is set to bNewStatus
	for(INT i=1; i<Children.Num(); i++)
	{
		noAnimNodeSequence* SeqNode = (noAnimNodeSequence*)(Children(i).Anim);
 ...
 */
noAnimNodeSequence* noAnimNodeSlot::GetCustomAnimNodeSeq()
{
	if( CustomChildIndex > 0 )
	{
		return (noAnimNodeSequence*)(Children(CustomChildIndex).Anim);
	}
 ...
 */
void noAnimNodeSlot::SetRootBoneAxisOption(BYTE AxisX, BYTE AxisY, BYTE AxisZ)
{
	noAnimNodeSequence* SeqNode = GetCustomAnimNodeSeq();
 ...
 */
void noAnimNodeSlot::AddToSynchGroup(FName GroupName)
{
	noAnimNodeSequence* SeqNode = GetCustomAnimNodeSeq();
	...
	if (SynchNode == NULL)
	{
		...
		for(INT i = 0; i < Nodes.Num(); i++)
		{
			SynchNode = (noAnimNodeSynch*)(Nodes(i));
			if (SynchNode != NULL)
			{
				break;
			}
...
/**  */
void noAnimNodeSlot::TickAnim(FLOAT DeltaSeconds, FLOAT TotalWeight)
{
	//if( bIsPlayingCustomAnim   )
	//{
	//	noAnimNodeSequence *SeqNode = GetCustomAnimNodeSeq();
 ...
 * @param BlendTime How long for child to take to reach weight of 1.0.
 */
void noAnimNodeSlot::SetActiveChild(INT ChildIndex, FLOAT BlendTime)
{
	assert(Children.Num() == TargetWeight.Num());
...
/** Called when we add a child to this node. We reset the TargetWeight array when this happens. */
void noAnimNodeSlot::OnAddChild(INT ChildNum)
{
	noAnimNodeBlendBase::OnAddChild(ChildNum);
...
/** Called when we remove a child to this node. We reset the TargetWeight array when this happens. */
void noAnimNodeSlot::OnRemoveChild(INT ChildNum)
{
	noAnimNodeBlendBase::OnRemoveChild(ChildNum);
...
/************************************************************************************
 * noAnimNodeAdditiveBlending
 ***********************************************************************************/

void noAnimNodeAdditiveBlending::GetBoneAtoms(TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	START_GETBONEATOM_TIMER
	...
	if( Children(0).Weight >= (1.f - ZERO_ANIMWEIGHT_THRESH) || !Children(1).Anim || (bPassThroughWhenNotRendered && !SkelComponent->bRecentlyRendered && !GIsEditor) )
	{
		if( Children(0).Anim )
		{
			EXCLUDE_CHILD_TIME
...
/** Get bone atoms from child node (if no child - use ref pose). */
void noAnimNodeAdditiveBlending::GetChildAtoms(INT ChildIndex, TArray<FBoneAtom>& Atoms, const TArray<BYTE>& DesiredBones, FBoneAtom& RootMotionDelta, INT& bHasRootMotion)
{
	if( Children(ChildIndex).Anim )
	{
 ...
 */
void noAnimNodeAdditiveBlending::SetChildrenTotalWeightAccumulator(const INT Index)
{
	if( Index == 0 )
	{
		// Update the weight of this connection
