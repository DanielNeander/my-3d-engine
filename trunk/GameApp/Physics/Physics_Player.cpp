...
float idPhysics_Player::CmdScale( const usercmd_t &cmd ) const {
	...
	total = noMath::Sqrt( (float) forwardmove * forwardmove + rightmove * rightmove + upmove * upmove );
...
==============
*/
void idPhysics_Player::Accelerate( const noVec3 &wishdir, const float wishspeed, const float accel ) {
#if 1
	// q2 style
...
bool idPhysics_Player::SlideMove( bool gravity, bool stepUp, bool stepDown, bool push ) {
	...
	noVec3		dir, planes[MAX_CLIP_PLANES];
	noVec3		end, stepEnd, primal_velocity, endVelocity, endClipVelocity, clipVelocity;
	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
		...
		if ( push && trace.c.entityNum != ENTITYNUM_WORLD ) {
			...
			if ( totalMass > 0.0f ) {
				...
				current.velocity *= 1.0f - noMath::ClampFloat( 0.0f, 1000.0f, totalMass - 20.0f ) * ( 1.0f / 950.0f );
				pushed = true;
			}
	...
	if ( oldVelocity * current.velocity > 0.0f ) {
		...
		if ( newVel > 1.0f ) {
			...
			if ( oldVel > 1.0f ) {
				...
				current.velocity *= noMath::Sqrt( oldVel / newVel );
			}
		}
	}
...
bool idPhysics_Player::CheckJump( void ) {
	...
	addVelocity *= noMath::Sqrt( addVelocity.Normalize() );
...
================
*/
void idPhysics_Player::SetPlayerInput( const usercmd_t &cmd, const noAngles &newViewAngles ) {
	command = cmd;
...
================
*/
void idPhysics_Player::GetImpactInfo( const int id, const noVec3 &point, impactInfo_t *info ) const {
...
================
*/
void idPhysics_Player::ApplyImpulse( const int id, const noVec3 &point, const noVec3 &impulse ) {
	if ( current.movementType != PM_NOCLIP ) {
...
================
*/
void idPhysics_Player::SetOrigin( const noVec3 &newOrigin, int id ) {
	noVec3 masterOrigin;
	noMat3 masterAxis;
...
================
*/
const noVec3 & idPhysics_Player::PlayerGetOrigin( void ) const {
...
================
*/
void idPhysics_Player::SetAxis( const noMat3 &newAxis, int id ) {
...
================
*/
void idPhysics_Player::Translate( const noVec3 &translation, int id ) {
...
================
*/
void idPhysics_Player::Rotate( const noRotation &rotation, int id ) {
	noVec3 masterOrigin;
	noMat3 masterAxis;
...
================
*/
void idPhysics_Player::SetLinearVelocity( const noVec3 &newLinearVelocity, int id ) {
...
================
*/
const noVec3 &idPhysics_Player::GetLinearVelocity( int id ) const {
...
void idPhysics_Player::SetPushed( int deltaTime ) {
	...
	velocity = ( current.origin - saved.origin ) / ( deltaTime * noMath::M_MS2SEC );
...
================
*/
const noVec3 &idPhysics_Player::GetPushedLinearVelocity( const int id ) const {
...
const int	PLAYER_VELOCITY_EXPONENT_BITS	= noMath::BitsForInteger( noMath::BitsForFloat( PLAYER_VELOCITY_MAX ) ) + 1;
