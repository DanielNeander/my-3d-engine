...
public:
	...
	const noPlane &			GetPlane( void ) const { return plane; }
	void					SetPlane( const noPlane &p ) { plane = p; }
	void					SetTextureMatrix( const noVec3 mat[2] ) { texMat[0] = mat[0]; texMat[1] = mat[1]; }
	void					GetTextureMatrix( noVec3 &mat1, noVec3 &mat2 ) { mat1 = texMat[0]; mat2 = texMat[1]; }
	void					GetTextureVectors( noVec4 v[2] ) const;
...
public:
	...
	static idMapBrush *		Parse( idLexer &src, const noVec3 &origin, bool newFormat = true, float version = CURRENT_MAP_VERSION );
	static idMapBrush *		ParseQ3( idLexer &src, const noVec3 &origin );
	bool					Write( idFile *fp, int primitiveNum, const noVec3 &origin ) const;
...
public:
							...
							~idMapPatch( void ) { }
	static idMapPatch *		Parse( idLexer &src, const noVec3 &origin, bool patchDef3 = true, float version = CURRENT_MAP_VERSION );
	bool					Write( idFile *fp, int primitiveNum, const noVec3 &origin ) const;
	const char *			GetMaterial( void ) const { return material; }
	void					SetMaterial( const char *p ) { material = p; }
