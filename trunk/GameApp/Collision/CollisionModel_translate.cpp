...
================
*/
ID_INLINE int idCollisionModelManagerLocal::TranslateEdgeThroughEdge( noVec3 &cross, idPluecker &l1, idPluecker &l2, float *fraction ) {
...
float CM_TranslationPlaneFraction( noPlane &plane, noVec3 &start, noVec3 &end ) {
	float d1, d2;
...
================
*/
void idCollisionModelManagerLocal::TranslateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly, cm_vertex_t *v, noVec3 &endp, idPluecker &pl ) {
	int i, edgeNum;
	float f;
	cm_trmEdge_t *edge;
...
bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	if ( noMath::Fabs(d) > tw->maxDistFromHeartPlane2 ) {
		return false;
	}
...
void idCollisionModelManagerLocal::Translation( trace_t *results, const noVec3 &start, const noVec3 &end,
	...
	noVec3 dir1, dir2, dir;
	noMat3 invModelAxis, tmpAxis;
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( noMath::Fabs( tw.size[0][i] ) > noMath::Fabs( tw.size[1][i] ) ) {
			tw.extents[i] = noMath::Fabs( tw.size[0][i] ) + CM_BOX_EPSILON;
		} else {
			tw.extents[i] = noMath::Fabs( tw.size[1][i] ) + CM_BOX_EPSILON;
		}
	}
	...
	for ( vert = tw.vertices, i = 0; i < tw.numVerts; i++, vert++ ) {
		...
		dist = noMath::Fabs( tw.heartPlane1.Distance( vert->p ) );
		if ( dist > tw.maxDistFromHeartPlane1 ) {
			tw.maxDistFromHeartPlane1 = dist;
		}
		dist = noMath::Fabs( tw.heartPlane2.Distance( vert->p ) );
