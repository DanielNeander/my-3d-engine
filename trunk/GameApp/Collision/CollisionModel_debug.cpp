...
================
*/
void idCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const noVec3 &origin, const noMat3 &axis ) {
	int side;
	cm_edge_t *edge;
	noVec3 start, end, mid;
	bool isRotated;
...
================
*/
void idCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const noVec3 &origin, const noMat3 &axis, const noVec3 &viewOrigin ) {
	int i, edgeNum;
	cm_edge_t *edge;
	noVec3 center, end, dir;
...
void idCollisionModelManagerLocal::DrawNodePolygons( cm_model_t *model, cm_node_t *node,
										   const noVec3 &origin, const noMat3 &axis,
...
================
*/
void idCollisionModelManagerLocal::DrawModel( cmHandle_t handle, const noVec3 &modelOrigin, const noMat3 &modelAxis,
					const noVec3 &viewOrigin, const float radius ) {
...
static noVec3 start;
static noVec3 *testend;

//#include "../sys/sys_public.h"

void idCollisionModelManagerLocal::DebugOutput( const noVec3 &origin ) {
	int i, k, t;
	char buf[128];
	noVec3 end;
	noAngles boxAngles;
	noMat3 modelAxis, boxAxis;
	idBounds bounds;
	trace_t trace;
	...
	testend = (noVec3 *) Mem_Alloc( cm_testTimes.GetInteger() * sizeof(noVec3) );
	...
	if ( cm_testRotation.GetBool() ) {
		...
		noVec3 vec( random.CRandomFloat(), random.CRandomFloat(), random.RandomFloat() );
		vec.Normalize();
		noRotation rotation( vec3_origin, vec, cm_testAngle.GetFloat() );
